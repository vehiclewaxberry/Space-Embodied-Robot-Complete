#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""V5 Loop1E live probe: negative-direction creation for joint2.

Probe V5_PROBE_LOOP1E_J2_RECREATE (20260812T074440Z) proved AddMate3 rejects
a NEGATIVE creation Angle (error 0, no additions) while 0 and +0.5 deg
create fine.  joint2/joint3 need an inward (negative) probe because their
accepted upper bound is 0.  SolidWorks represents direction via Flip /
alignment, not a negative Angle.

In-memory only, NO SAVE, production state:
  V1  joint2 mate created with Flip=True, Angle=+0.0087266 -> measure the
      link1<-link2 relative rotation projected on the accepted joint2 axis
      (want NEGATIVE ~-0.0087);
  V2  if V1 is positive: Align=SW_ALIGN_ANTI_ALIGNED(1), Flip=False,
      Angle=+0.0087266 -> measure again.
  V3  discard WITHOUT saving; close; session census.
...
def attempt(model: Any, assembly: Any, joint: Dict[str, Any], angle: float, align: int, flip: bool, types: Any, pythoncom_module: Any, swap: bool = False, baseline: Any = None) -> Dict[str, Any]:
    ...
    first_plane = m.component_side_plane(male if swap else female, base, types, pythoncom_module)
    second_plane = m.component_side_plane(female if swap else male, base, types, pythoncom_module)
    ...
    out: Dict[str, Any] = {"angle_arg": angle, "align": align, "flip": flip, "swap": swap, "error_status": error, "additions": len(additions)}
    if mate_raw is not None and additions:
        if baseline is not None:
            try:
                out["delta"] = relative_delta(assembly, joint, baseline, types, pythoncom_module)
            except Exception as exc:  # noqa: BLE001
                out["delta_exception"] = repr(exc)
        delete_feature(model, additions[0])
        model.ForceRebuild3(True)
    return out


def relative_delta(assembly: Any, joint: Dict[str, Any], before: List[List[float]], types: Any, pythoncom_module: Any) -> Dict[str, Any]:
    """Delta rotation between a saved baseline and the current pose,
    projected on the accepted axis (production sign-probe semantics)."""
    ...
    current = mul(tr(rot(t16(parent))), rot(t16(child)))
    delta = mul(current, tr(before))
    cos_angle = (delta[0][0] + delta[1][1] + delta[2][2] - 1.0) / 2.0
    angle = _math.acos(max(-1.0, min(1.0, cos_angle)))
    if abs(angle) < 1.0e-12:
        vector = [0.0, 0.0, 0.0]
    else:
        denom = 2.0 * _math.sin(angle)
        vector = [angle * (delta[2][1] - delta[1][2]) / denom, angle * (delta[0][2] - delta[2][0]) / denom, angle * (delta[1][0] - delta[0][1]) / denom]
    expected = m.rpy_rotation(joint["origin_rpy"])
    axis = [sum(expected[r][i] * joint["axis_xyz"][i] for i in range(3)) for r in range(3)]
    norm = _math.sqrt(sum(v * v for v in axis))
    axis = [v / norm for v in axis]
    projected = sum(vector[i] * axis[i] for i in range(3))
    transverse = _math.sqrt(max(0.0, sum(v * v for v in vector) - projected * projected))
    return {"projected": projected, "transverse": transverse}


def baseline_relative(assembly: Any, joint: Dict[str, Any], types: Any, pythoncom_module: Any) -> List[List[float]]:
    index = int(joint["joint"].replace("joint", ""))
    parent_leaf = "B51_REF_base_link_LINKLOCAL-1" if index == 1 else f"B51_REF_link{index - 1}_LINKLOCAL-1"
    child_leaf = f"B51_REF_link{index}_LINKLOCAL-1"
    parent = find_leaf(assembly, parent_leaf, types, pythoncom_module)
...
def main() -> int:
    result: Dict[str, Any] = {
        "schema": "V5_PROBE_LOOP1E_J2_NEGDIRECTION_V1",
    ...
    try:
        ...
        baseline = baseline_relative(assembly, joint2, types, pythoncom_module)
        v1 = attempt(model, assembly, joint2, probe, 0, True, types, pythoncom_module, baseline=baseline)
        result["phases"]["V1_flip_true"] = v1
        log(f"V1 flip=True: additions={v1.get('additions')} delta={v1.get('delta')}")
        v3 = attempt(model, assembly, joint2, probe, 0, False, types, pythoncom_module, swap=True, baseline=baseline)
        result["phases"]["V3_swapped_order"] = v3
        log(f"V3 swap+pos: additions={v3.get('additions')} delta={v3.get('delta')}")

        remaining = close_everything(sw)
        result["cleanup_remaining"] = remaining
        log(f"cleanup remaining={remaining}")
        result["verdict"] = "V5_PROBE_LOOP1E_J2_NEGDIRECTION_PASS"
    except Exception:  # noqa: BLE001
        result["exception"] = traceback.format_exc()
        result["verdict"] = "V5_PROBE_LOOP1E_J2_NEGDIRECTION_FAIL"
