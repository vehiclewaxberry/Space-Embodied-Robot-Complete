#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""V5 Loop1E live probe: ModifyDefinition priming hypothesis.

Observed pattern (2026-08-11/12, same assembly, same state):
  * 23:25 DRIVE_FRESH_READS: setup ModifyDefinition RAN and SUCCEEDED, then
    the +0.5 deg drive ModifyDefinition SUCCEEDED (joint moved exactly);
  * 05:47 MODIFY_CONVERGE: creation readback already exact, setup write
    SKIPPED, and the direct drive ModifyDefinition FAILED x3 (direct /
    fresh retry / delete+recreate), 1.03 GiB free;
  * 06:04 production: setup write skipped by verify-first; drive failed
    twice, 2.27 GiB free — memory alone does not discriminate.
Hypothesis: a freshly created mate is not committable until an initial
ModifyDefinition "primes" it; the drive write then succeeds.

In-memory only, NO SAVE, production state (full dependent-reload + 默认 gate):
  A  create M1; drive ModifyDefinition(+0.5 deg) unprimed (expect fail);
  B  delete M1; create M2; PRIME ModifyDefinition(unchanged values), then
     drive ModifyDefinition(+0.5 deg); measure base_link<-link1 rotation;
  C  if B fails: delete M2; create M3 with AddMate3 Angle=+0.5 deg at
     creation; measure rotation (creation-angle drive test);
  D  discard WITHOUT saving; close; session census.
...
def modify(model: Any, feature: Any, joint: Dict[str, Any], angle: float, types: Any, pythoncom_module: Any) -> Dict[str, Any]:
    ...
    out: Dict[str, Any] = {"modified": modified, "rebuilt": rebuilt, "target_angle": angle}
    if modified:
...
def main() -> int:
    result: Dict[str, Any] = {
        "schema": "V5_PROBE_LOOP1E_MODIFY_PRIME_V1",
    ...
    try:
        ...
        a_drive = modify(model, m1, joint, probe, types, pythoncom_module)
        result["phases"]["A_unprimed_drive"] = a_drive
        log(f"A unprimed drive: modified={a_drive['modified']}")

        # B: delete, recreate, PRIME, then drive
        deleted = delete_feature(model, m1)
        m2 = create_mate(model, assembly, joint, 0.0, types, pythoncom_module)
        prime = modify(model, m2, joint, 0.0, types, pythoncom_module)
        b_drive = None
        rotvec_after = None
        if prime["modified"]:
            b_drive = modify(model, m2, joint, probe, types, pythoncom_module)
            rotvec_after = relative_rotation(assembly, types, pythoncom_module)
        result["phases"]["B_prime_then_drive"] = {"deleted_m1": deleted, "prime": prime, "drive": b_drive, "rotvec": rotvec_after}
        log(f"B: deleted={deleted} prime={prime['modified']} drive={None if b_drive is None else b_drive['modified']} rotvec={rotvec_after}")

        # C: creation-angle drive fallback
        if b_drive is None or not b_drive.get("modified"):
            delete_feature(model, m2)
            ...
            c_angle = float(base.value(definition, "Angle"))
            c_rotvec = relative_rotation(assembly, types, pythoncom_module)
            result["phases"]["C_creation_angle_drive"] = {"angle_after": c_angle, "rotvec": c_rotvec}
            log(f"C creation-angle: angle={c_angle} rotvec={c_rotvec}")
        ...
        result["verdict"] = "V5_PROBE_LOOP1E_MODIFY_PRIME_PASS"
    except Exception:  # noqa: BLE001
        result["exception"] = traceback.format_exc()
        result["verdict"] = "V5_PROBE_LOOP1E_MODIFY_PRIME_FAIL"
