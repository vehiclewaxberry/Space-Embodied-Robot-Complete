#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""V5 Loop1E live probe: joint1 angle-mate drive truth experiment.

Production run 20260811T2251 created the joint1 angle mate cleanly but the
+0.5 deg dimension drive produced EXACTLY zero relative rotation
(ANGLE_SIGN_PROBE_KINEMATIC_FAIL with rotation_vector all zeros).  The
mate/dimension topology is proven healthy (25 donor mates: lock/concentric/
coincident only; FEMALE fixed to base_link, MALE fixed to link1; angle mate
spans FEMALE x MALE across the free joint DOF).

In-memory only, NO SAVE, on the staged tree:
  S1  production state at joint1 (hybrid prebind, 默认 active, dependent-
      reload FEMALE/MALE/link1/base_link);
  S2  recreate the production angle mate (AddMate3 type 6, aligned, angle 0,
      limits) exactly as add_native_angle_driver does;
  S3  enumerate the mate's display dimensions: count, FullName, current
      system value, and drive dimension(0) to +0.5 deg in 默认;
  S4  read back: mate data Angle, dimension system value, and the
      base_link<-link1 relative rotation vector (fresh handles);
  S5  if the angle did not move: try ModifyDefinition(data.Angle=+0.5 deg)
      and read back again;
  S6  discard WITHOUT saving; close; session census.
...
def relative_rotation(assembly: Any, types: Any, pythoncom_module: Any) -> List[float]:
    ...
    rp = rot(transform16(parent))
    rc = rot(transform16(child))
    relative = mul(tr(rp), rc)
    return rotvec(relative)


def main() -> int:
    result: Dict[str, Any] = {
        "schema": "V5_PROBE_LOOP1E_JOINT1_DRIVE_TRUTH_V1",
    ...
    try:
        ...
        # S1: production-verbatim state (hybrid prebind, census ending in
        # STOWED, dependent-reload of ALL non-gripper occurrences, then the
        # 默认 activation gate) — replicates the 20260811T2251 run setup.
        ...
        unresolved = [str(base.value(c, "Name2")) for c in (base.wrap(r2, "IComponent2", types, pythoncom_module) for r2 in base.as_list(assembly.GetComponents(True))) if str(base.value(c, "Name2")).split("/")[-1] != m.ARM_LEGACY_GRIPPER_LEAF and int(base.value(c, "GetSuppression2")) != SW_COMPONENT_FULLY_RESOLVED]
        if unresolved:
            raise RuntimeError(f"dependent-reload readback failed: {unresolved}")
        ...
        log("S1 ready (production-verbatim state)")

        # S2: create the joint1 driver via the PRODUCTION function verbatim
        ...
        result["phases"]["S2_mate_created"] = creation
        log("S2 mate created via production add_native_angle_driver")

        # S3: display dimensions + drive dimension(0)
        dimension = m.angle_mate_dimension(feature, base, types, pythoncom_module)
        full_name = str(base.value(dimension, "FullName"))
        before = [float(v) for v in base.as_list(dimension.GetSystemValue3(SW_SET_VALUE_IN_SPECIFIC_CONFIGS, [m.ARM_CONFIGS[0]]))]
        ...
        after_drive = [float(v) for v in base.as_list(dimension.GetSystemValue3(SW_SET_VALUE_IN_SPECIFIC_CONFIGS, [m.ARM_CONFIGS[0]]))]
        mate_angle = float(base.value(m.angle_mate_data(feature, base, types, pythoncom_module), "Angle"))
        current = relative_rotation(assembly, types, pythoncom_module)
        result["phases"]["S3S4_drive"] = {
            "dimension_full_name": full_name,
            "value_before": before,
            "status": status,
            "rebuilt": rebuilt,
            "value_after": after_drive,
            "mate_angle_after": mate_angle,
            "baseline_rotvec": baseline,
            "current_rotvec": current,
        }
        log(f"S4 drive: status={status} value {before}->{after_drive} mate_angle={mate_angle} rotvec={current}")

        # S5: fallback ModifyDefinition drive if needed
        if abs(mate_angle - probe) > 1.0e-9:
            data2 = m.angle_mate_data(feature, base, types, pythoncom_module)
            data2.Angle = probe
            modified = bool(feature.ModifyDefinition(data2, model, None))
            rebuilt2 = bool(model.ForceRebuild3(True))
            mate_angle2 = float(base.value(m.angle_mate_data(feature, base, types, pythoncom_module), "Angle"))
            current2 = relative_rotation(assembly, types, pythoncom_module)
            result["phases"]["S5_modifydefinition_drive"] = {"modified": modified, "rebuilt": rebuilt2, "mate_angle_after": mate_angle2, "current_rotvec": current2}
            log(f"S5 modify: mate_angle={mate_angle2} rotvec={current2}")

        remaining = close_everything(sw)
        result["cleanup_remaining"] = remaining
        log(f"cleanup remaining={remaining}")
        result["verdict"] = "V5_PROBE_LOOP1E_JOINT1_DRIVE_TRUTH_PASS"
    except Exception:  # noqa: BLE001
        result["exception"] = traceback.format_exc()
        result["verdict"] = "V5_PROBE_LOOP1E_JOINT1_DRIVE_TRUTH_FAIL"
