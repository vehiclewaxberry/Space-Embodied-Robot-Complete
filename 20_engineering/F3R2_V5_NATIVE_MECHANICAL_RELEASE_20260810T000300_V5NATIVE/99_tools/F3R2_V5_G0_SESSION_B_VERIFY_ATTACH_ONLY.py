...
def configure(run_dir: Path) -> None:
    """Bind this verifier to a fresh G0 smoke run directory."""
    global RUN_DIR, PART, SESSION_A_RECEIPT, SESSION_A_ADDENDUM
    global SESSION_A_PID, SESSION_A_START_LOCAL, SESSION_A_RECEIPT_SHA256
    global PART_BYTES, PART_SHA256, PREDELETE_RECEIPT, FINAL_RECEIPT
    resolved = Path(run_dir).resolve()
    if resolved.parent != ENGINEERING.resolve():
    ...
    PREDELETE_RECEIPT = RUN_DIR / "G0_SMOKE_SESSION_B_PREDELETE_RECEIPT.json"
    FINAL_RECEIPT = RUN_DIR / "G0_SOLIDWORKS_NATIVE_EXECUTION_READY.json"
    if not SESSION_A_RECEIPT.is_file() or not SESSION_A_ADDENDUM.is_file():
        raise GateError("SESSION_A_EVIDENCE_MISSING", "Session-A receipt/addendum are missing in the run directory")
    receipt = json.loads(SESSION_A_RECEIPT.read_text(encoding="utf-8"))
    addendum = json.loads(SESSION_A_ADDENDUM.read_text(encoding="utf-8"))
    SESSION_A_PID = int(addendum["solidworks_process_id"])
    SESSION_A_START_LOCAL = str(addendum["solidworks_process_start_local"])
    SESSION_A_RECEIPT_SHA256 = sha256(SESSION_A_RECEIPT)
    PART_BYTES = int(receipt["native_part"]["output_bytes"])
    PART_SHA256 = str(receipt["native_part"]["output_sha256"])
...
def memory_gate() -> List[float]:
    ...
    for index in range(10):
        ...
        if index < 9:
            time.sleep(0.25)
    if not mo.override_allowed() and not all(value >= MIN_AVAILABLE_GIB for value in samples):
        raise GateError(
            "RESOURCE_GATE_HOLD",
...
def main() -> int:
    ...
    try:
        ...
        if not mo.override_allowed() and available_memory_gib() < MIN_AVAILABLE_GIB:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Strict Session-B cold-reopen verifier for a fresh G0 smoke run")
    parser.add_argument("run_dir", type=Path, help="_MFINAL_G0_SMOKE_* directory directly under 20_engineering")
