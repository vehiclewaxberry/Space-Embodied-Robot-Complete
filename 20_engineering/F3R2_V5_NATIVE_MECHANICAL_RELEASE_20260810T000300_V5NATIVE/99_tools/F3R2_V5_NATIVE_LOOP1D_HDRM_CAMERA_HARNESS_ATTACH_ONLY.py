...
FIXED_INPUTS: Tuple[Tuple[Path, str, int], ...] = (
    (BASE_HELPER, "E44BC52CFBDECCC107E361ACB0EDD993EFC46248EB663A994564B77BDA30F906", 53775),
    (LOOP1A_HELPER, "25369CB867685EBA0C374636F8F540867D3D8B5890F9CE8622A6842E6518EC42", 63256),
    (BASE_HELPER_COPY, "E44BC52CFBDECCC107E361ACB0EDD993EFC46248EB663A994564B77BDA30F906", 53775),
...
def insert_component(
    ...
    try:
        ...
        # Warning 128 is a transient same-session open warning for these
        # freshly generated parts; a manual cold open returns 0.
        if raw_part is None or errors != 0 or warnings not in (0, 128) or not bool(base.value(part, "IsOpenedReadOnly")):
        ...
        # Occurrence Name2 is read-only in this SW2024 session; the role is
        # tracked by the returned component object, not by the display name.
        occurrence_name = str(base.value(component, "Name2"))
        if occurrence_name != str(component_spec["role"]):
            result_note = {"expected_role": component_spec["role"], "actual_occurrence_name": occurrence_name}
            if not hasattr(insert_component, "occurrence_name_notes"):
                insert_component.occurrence_name_notes = []
            insert_component.occurrence_name_notes.append(result_note)
        model.ClearSelection2(True)
...
def expected_component_map_by_path(spec: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
    return {norm(Path(str(item["path"]))): item for item in spec["components"]}


def role_by_path(spec: Dict[str, Any]) -> Dict[str, str]:
    return {norm(Path(str(item["path"]))): str(item["role"]) for item in spec["components"]}


def path_by_role(spec: Dict[str, Any]) -> Dict[str, str]:
    return {str(item["role"]): norm(Path(str(item["path"]))) for item in spec["components"]}
...
def expected_name2_map(spec: Dict[str, Any]) -> Dict[str, str]:
    """Deterministic occurrence names: SW2024 numbers repeated inserts of one
    part file per assembly in insertion order (part-stem-N), as evidenced by
    the live fail-receipt ledger name2 values."""
    counters: Dict[str, int] = {}
    out: Dict[str, str] = {}
    for item in spec["components"]:
        stem = Path(str(item["path"])).stem
        counters[stem] = counters.get(stem, 0) + 1
        out[str(item["role"])] = f"{stem}-{counters[stem]}"
    return out


def live_components_by_role(model: Any, spec: Dict[str, Any], types: Any, pythoncom: Any) -> Dict[str, Any]:
    """Fresh per-configuration component handles keyed by role.

    Component handles fetched before a configuration switch go stale on this
    SW2024 SP05 session (probe V5_PROBE_LOOP1D_MATE_SUPPRESSION P4: stale
    SetSuppression2 returned 0 and GetSuppression2 read back -1; Loop1C1
    measured the same staleness for Transform2).  Every per-configuration
    drive/readback therefore re-resolves the exact occurrence set against the
    now-active configuration.
    """
    expected_names = expected_name2_map(spec)
    ...
    for raw in base.as_list(assembly.GetComponents(True)):
        ...
        name2 = str(base.value(component, "Name2"))
        if name2 in fresh:
            raise GateError("LIVE_COMPONENT_NAME_FAIL", "duplicate occurrence name in the live assembly", {"name2": name2})
        fresh[name2] = component
    if set(fresh) != set(expected_names.values()) or len(fresh) != len(expected_names):
        raise GateError("LIVE_COMPONENT_SET_FAIL", "live occurrence set is not the exact expected set", {"expected": sorted(expected_names.values()), "actual": sorted(fresh)})
    return {role: fresh[name2] for role, name2 in expected_names.items()}


def show_configuration(model: Any, config: str, types: Any, pythoncom: Any) -> None:
    """Activate a configuration under the measured SW2024 SP05 semantics.

    Probe V5_PROBE_LOOP1D_COLD_REOPEN_DIAG showed that on a read-only document
    ShowConfiguration2 returns False when the named configuration is already
    active (a no-op), while a real switch returns True.  Success is therefore
    defined as "the named configuration is active after the call", verified by
    reading the active configuration name back; this stays fail-closed.
    """
    ...
    active_name = "" if raw_active is None else str(base.value(base.wrap(raw_active, "IConfiguration", types, pythoncom), "Name"))
    if active_name != config:
        raise GateError("CONFIGURATION_ACTIVATE_FAIL", "configuration is not active after ShowConfiguration2", {"config": config, "returned": None if returned is None else bool(returned), "active": active_name})


def mate_feature_handles(model: Any, types: Any, pythoncom: Any) -> List[Any]:
    """Fresh IFeature handles for every mate under the MateGroup."""
    handles: List[Any] = []
    feature = base.value(model, "FirstFeature")
    while feature is not None:
        typed = base.wrap(feature, "IFeature", types, pythoncom)
...
def drive_mate_suppression(model: Any, config: str, active_roles: set, name2_to_role: Dict[str, str], types: Any, pythoncom: Any) -> None:
    """Explicit per-configuration mate suppression drive.

    Probe V5_PROBE_LOOP1D_MATE_SUPPRESSION P2 proved that component suppression
    does not cascade IFeature.IsSuppressed onto referencing lock mates in this
    SW2024 SP05 session, while P3/P4/P5 proved an explicit
    IFeature.SetSuppression2(state, swThisConfiguration) drive sticks per
    configuration and persists across live configuration switches; probe
    V5_PROBE_LOOP1D_COLD_REOPEN_DIAG proved the explicit states persist through
    save + cold read-only reopen.  This drive makes mate suppression follow
    the active state-proxy endpoints exactly, as the contract requires.
    """
    for feature in mate_feature_handles(model, types, pythoncom):
        mate = base.wrap(base.value(feature, "GetSpecificFeature2"), "IMate2", types, pythoncom)
        endpoint_roles: set = set()
        for index in range(int(base.value(mate, "GetMateEntityCount"))):
            ...
            raw_component = base.value(entity, "ReferenceComponent")
            if raw_component is None:
                raise GateError("MATE_ENDPOINT_COMPONENT_FAIL", "mate endpoint has no component", {"config": config, "index": index})
            name2 = str(base.value(base.wrap(raw_component, "IComponent2", types, pythoncom), "Name2"))
            if name2 not in name2_to_role:
                raise GateError("MATE_ENDPOINT_OCCURRENCE_FAIL", "mate endpoint occurrence is not a controlled role", {"config": config, "name2": name2})
            endpoint_roles.add(name2_to_role[name2])
        should_suppress = not endpoint_roles.issubset(active_roles)
        state = 0 if should_suppress else 1  # swSuppressFeature / swUnSuppressFeature
        returned = feature.SetSuppression2(state, 1, None)  # swThisConfiguration
        readback = bool(base.value(feature, "IsSuppressed"))
        if readback != should_suppress:
            raise GateError("MATE_SUPPRESSION_DRIVE_FAIL", "explicit per-configuration mate suppression did not read back", {"config": config, "feature_name": str(base.value(feature, "Name")), "endpoint_roles": sorted(endpoint_roles), "should_suppress": should_suppress, "returned": None if returned is None else repr(returned), "readback": readback})


def validate_mate_contract(ledger: Sequence[Dict[str, Any]], spec: Dict[str, Any], config: str, components: Dict[str, Any]) -> None:
    name2_to_role = {str(base.value(component, "Name2")): role for role, component in components.items()}
    actual_pairs = Counter(tuple(sorted(name2_to_role[item["name2"]] for item in row["endpoints"])) for row in ledger)
    ...
    for row in ledger:
        endpoints = {name2_to_role[item["name2"]] for item in row["endpoints"]}
        should_suppress = not endpoints.issubset(active_roles)
        if bool(row["suppressed"]) != should_suppress:
            raise GateError("MATE_CONFIGURATION_SUPPRESSION_FAIL", "mate suppression does not follow active state-proxy endpoints", {"config": config, "row": row, "active_roles": sorted(active_roles), "name2_to_role": name2_to_role, "endpoint_roles": sorted(endpoints)})
...
def configure_document(model: Any, components: Dict[str, Any], spec: Dict[str, Any], types: Any, pythoncom: Any) -> List[Dict[str, Any]]:
    ...
    for config in configs:
        show_configuration(model, config, types, pythoncom)
        active_roles = expected_active_roles(spec, config)
        # Handles fetched before a configuration switch are stale; re-resolve
        # the exact occurrence set for every drive pass (see the staleness
        # note on live_components_by_role).
        for pass_code in ("COMPONENT_SUPPRESSION_FAIL", "COMPONENT_SUPPRESSION_REAPPLY_FAIL"):
            live = live_components_by_role(model, spec, types, pythoncom)
            for role, component in live.items():
                ...
                if readback != wanted:
                    raise GateError(pass_code, "SetSuppression2 did not read back the intended state", {"config": config, "role": role, "returned": returned, "wanted": wanted, "readback": readback})
        ...
        # Component suppression does not cascade onto lock-mate features in
        # this session, so mate suppression is driven explicitly per
        # configuration after the component states settle (see
        # drive_mate_suppression for the probe evidence).
        live = live_components_by_role(model, spec, types, pythoncom)
        name2_to_role = {str(base.value(component, "Name2")): role for role, component in live.items()}
        drive_mate_suppression(model, config, active_roles, name2_to_role, types, pythoncom)
        if not bool(model.ForceRebuild3(True)):
            raise GateError("CONFIGURATION_REBUILD_FAIL", "configuration rebuild after mate suppression drive failed", {"config": config})
        state_rows.append(configuration_snapshot(model, spec, config, types, pythoncom, live_components_by_role(model, spec, types, pythoncom)))
    show_configuration(model, configs[0], types, pythoncom)
    return state_rows


def configuration_snapshot(model: Any, spec: Dict[str, Any], config: str, types: Any, pythoncom: Any, components: Dict[str, Any]) -> Dict[str, Any]:
    assembly = base.wrap(model, "IAssemblyDoc", types, pythoncom)
    raw_components = base.as_list(assembly.GetComponents(True))
    live_components = [base.wrap(item, "IComponent2", types, pythoncom) for item in raw_components]
    states = sorted((component_state(item) for item in live_components), key=lambda row: row["name2"])
    expected = expected_component_map(spec)
    name2_to_role = {str(base.value(component, "Name2")): role for role, component in components.items()}
    if {row["name2"] for row in states} != set(name2_to_role) or len(states) != len(name2_to_role):
        raise GateError("CONFIG_COMPONENT_SET_FAIL", "configuration component occurrence set is not exact", {"config": config, "expected": sorted(name2_to_role), "actual": states})
    states_by_name = {row["name2"]: row for row in states}
    active_roles = expected_active_roles(spec, config)
    for role in expected:
        component = components[role]
        name2 = str(base.value(component, "Name2"))
        row = states_by_name[name2]
        contract = expected[role]
        wanted_suppression = SW_COMPONENT_RESOLVED if role in active_roles else SW_COMPONENT_SUPPRESSED
    ...
    validate_mate_contract(ledger, spec, config, components)
...
def assembly_brep_bbox_mm(model: Any, types: Any, pythoncom: Any) -> Tuple[List[float], List[Dict[str, Any]]]:
    ...
    IBody2.GetExtremePoint is called without explicit out arguments: the
    makepy-managed byref form returns (ok, x, y, z) directly, while passing
    explicit VT_BYREF VARIANTs raises TypeError on this install (probe
    V5_PROBE_LOOP1D_EXTREME_POINT_API, forms E1 vs E2/E3).
    """
    assembly = base.wrap(model, "IAssemblyDoc", types, pythoncom)
        ...
        for body in bodies:
            ...
            for axis in range(3):
                for sign in (-1.0, 1.0):
                    ...
                    returned = body.GetExtremePoint(direction[0], direction[1], direction[2])
                    if not isinstance(returned, tuple) or len(returned) != 4 or not bool(returned[0]):
                        raise GateError("BREP_EXTREME_POINT_FAIL", "IBody2.GetExtremePoint did not return (True, x, y, z)", {"name2": str(base.value(component, "Name2")), "axis": axis, "sign": sign, "returned": repr(returned)})
                    world = transform_point(array, [float(returned[1]), float(returned[2]), float(returned[3])])
                    if sign < 0.0:
...
def fixed_datum_witness(snapshot: Dict[str, Any], spec: Dict[str, Any], fixed_role: str) -> Dict[str, Any]:
    expected = expected_component_map(spec)
    # Occurrence Name2 is read-only in this SW2024 session, so live components
    # keep the deterministic per-path insertion-order names (part-stem-N);
    # resolve the fixed role to its occurrence name through the same contract
    # used by the live/cold verifiers.
    wanted_name2 = expected_name2_map(spec).get(fixed_role) if fixed_role in expected else None
...
def resolve_cold_active_components(model: Any, config: str, types: Any, pythoncom: Any) -> None:
    """Resolve lightweight components in place on the cold read-only document.

    Cold read-only opens load active components lightweight on this machine
    (probe V5_PROBE_LOOP1D_COLD_REOPEN_DIAG C0: GetSuppression2()=2), and
    IAssemblyDoc.ResolveAllLightWeightComponents(True) is a no-op returning
    False on a read-only document (same probe, C5), while per-component
    SetSuppression2(swComponentResolved) resolves them in place (probe
    V5_PROBE_LOOP1D_LIGHTWEIGHT_RESOLVE L2/L3, with exact per-config
    suppression patterns and Transform2 readbacks preserved).  This in-memory
    load-mode change never touches the saved per-configuration
    suppressed/active pattern under verification: suppressed occurrences (0)
    are left untouched and active occurrences only move lightweight (2) ->
    resolved (1), which is exactly what the live snapshots recorded.  The
    SetSuppression2 return value is unreliable on this install (observed 2
    with a resolved readback), so the gate reads the state back.
    """
    assembly = base.wrap(model, "IAssemblyDoc", types, pythoncom)
    for raw in base.as_list(assembly.GetComponents(True)):
        component = base.wrap(raw, "IComponent2", types, pythoncom)
        ...
        if before == SW_COMPONENT_SUPPRESSED:
            continue
        returned = component.SetSuppression2(SW_COMPONENT_RESOLVED)
        readback = int(base.value(component, "GetSuppression2"))
        if readback != SW_COMPONENT_RESOLVED:
            raise GateError("COLD_COMPONENT_RESOLVE_FAIL", "cold lightweight component did not resolve in place", {"config": config, "name2": name2, "before": before, "returned": None if returned is None else repr(returned), "readback": readback})
...
def verify_assembly_cold(sw: Any, types: Any, pythoncom: Any, name: str, spec: Dict[str, Any], expected_snapshots: Optional[Sequence[Dict[str, Any]]] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        for config in spec["configs"]:
            show_configuration(cold, config, types, pythoncom)
            if not bool(cold.ForceRebuild3(True)):
                raise GateError("ASSEMBLY_COLD_CONFIG_FAIL", "cold configuration rebuild failed", {"name": name, "config": config})
            resolve_cold_active_components(cold, config, types, pythoncom)
            snapshots.append(configuration_snapshot(cold, spec, config, types, pythoncom, live_components_by_role(cold, spec, types, pythoncom)))
        ...
        show_configuration(cold, spec["configs"][0], types, pythoncom)
        if not bool(cold.ForceRebuild3(True)):
            raise GateError("FRAME_WITNESS_CONFIG_RESTORE_FAIL", "cannot restore controlled frame-witness configuration", {"name": name})
        resolve_cold_active_components(cold, spec["configs"][0], types, pythoncom)
...
def build_assembly(sw: Any, types: Any, pythoncom: Any, name: str, spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        # Insert-time component handles can disconnect after later inserts/
        # solves (observed CO_E_OBJNOTCONNECTED -2147417848 on a Name2 readback
        # after the 12-insert HDRM sequence).  Re-resolve the exact occurrence
        # set against the live assembly; live_components_by_role enforces the
        # deterministic occurrence-name contract (set equality against
        # expected_name2_map) and returns fresh handles for mate creation.
        components = live_components_by_role(model, spec, types, pythoncom)
        mate_results = []
...
def continuation_script_sha() -> str:
    """Walk the write-once continuation chain to the checkpoint-bound hash.

    Each ``00_authority/V5_LOOP1D_SCRIPT_CONTINUATION*.json`` file bridges one
    script revision to its predecessor (current_script_sha256 ->
    previous_script_sha256).  The terminal hash of the chain is the revision
    that wrote the part checkpoints, which is what checkpoint_binding must
    present for a verified resume.  A cycle or a missing bridge terminates the
    walk, so an undocumented patch can never silently bind old checkpoints.
    """
    bridges: Dict[str, str] = {}
    for path in sorted(RUN_ROOT.glob("00_authority/V5_LOOP1D_SCRIPT_CONTINUATION*.json")):
        try:
            payload = json.loads(path.read_text(encoding="utf-8"))
        except Exception:
            continue
        if payload.get("schema") != "F3R2_V5_LOOP1D_SCRIPT_CONTINUATION_V1":
            continue
        current = payload.get("current_script_sha256")
        previous = payload.get("previous_script_sha256")
        if isinstance(current, str) and isinstance(previous, str) and current and previous:
            bridges[current] = previous
    terminal = sha256(Path(__file__))
    seen = set()
    while terminal in bridges and terminal not in seen:
        seen.add(terminal)
        terminal = bridges[terminal]
    return terminal


def checkpoint_binding() -> Dict[str, Any]:
    return {
        "script_sha256": continuation_script_sha(),
...
def execute() -> int:
    ...
    try:
        ...
        if int(session.get("pid")) != int(sbin.resolve_pid(42276)) or session.get("revision") != "32.5.0" or session.get("document_count") != 0 or session.get("active_doc_is_null") is not True:
        ...
        only_assembly = os.environ.get("V5_LOOP1D_ONLY", "")
        for name, spec in ASSEMBLY_SPECS.items():
            if only_assembly and name != only_assembly:
                continue
            target = Path(spec["target"])
            if target.exists():
