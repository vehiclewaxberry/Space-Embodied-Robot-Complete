...
body is bound geometrically by volume, bounding box, centroid, and jaw-axis
station/range before Copy2; GetBodies2 enumeration order is never consulted.
...
JAW_AXIS = (-0.000007, 0.906306, 0.422622)
JAW_ORIGIN_MM = (399.7007, 92.2233, -197.7734)
...
MAPPING_BBOX_COORD_TOL_MM = 0.75
MAPPING_BBOX_CENTROID_TOL_MM = 0.75
MAPPING_MASS_CENTROID_TOL_MM = 1.0
MAPPING_JAW_TOL_MM = 0.75
...
SCRIPT_COPY = RUN_ROOT / "99_tools/F3R2_V5_NATIVE_LOOP1C_GRIPPER_ATTACH_ONLY_R2.py"
...
def normalized_jaw_axis() -> Tuple[float, float, float]:
    length = math.sqrt(vector_dot(JAW_AXIS, JAW_AXIS))
    if length <= 0.0:
        raise GateError("JAW_AXIS_FAIL", "fixed jaw axis has zero length")
    return tuple(float(value) / length for value in JAW_AXIS)
...
def bbox_metrics(box_mm: Sequence[float]) -> Dict[str, Any]:
    ...
    axis = normalized_jaw_axis()
    relative_centre = [centre[index] - JAW_ORIGIN_MM[index] for index in range(3)]
    projected = [
        vector_dot([corner[index] - JAW_ORIGIN_MM[index] for index in range(3)], axis)
    ...
    return {
        ...
        "jaw_s_mm": vector_dot(relative_centre, axis),
        "jaw_u_min_mm": min(projected),
        "jaw_u_max_mm": max(projected),
    }
...
def load_geometry_contract() -> Dict[str, Any]:
    ...
    axis = tuple(round(value, 6) for value in normalized_jaw_axis())
    recorded_axis = tuple(float(value) for value in binding_payload.get("jaw_axis", {}).get("world_direction", []))
    recorded_origin = tuple(float(value) for value in binding_payload.get("jaw_axis", {}).get("origin_mm", []))
    if len(recorded_axis) != 3 or len(recorded_origin) != 3 or vector_distance(axis, recorded_axis) > 2.0e-6 or vector_distance(JAW_ORIGIN_MM, recorded_origin) > 1.0e-6:
        raise GateError("JAW_AUTHORITY_FAIL", "fixed historical binding does not contain the pinned jaw axis/origin", {"recorded_axis": recorded_axis, "recorded_origin_mm": recorded_origin, "expected_axis": axis, "expected_origin_mm": JAW_ORIGIN_MM})
    ...
    for label in PHYSICAL_LABELS:
        ...
        metrics = bbox_metrics(mesh.get("box_mm", []))
    ...
    union_box = [min(item["bbox_mm"][index] for item in physical.values()) for index in range(3)] + [max(item["bbox_mm"][index] for item in physical.values()) for index in range(3, 6)]
    ...
    with ASSIGNMENT_CSV.open("r", encoding="utf-8-sig", newline="") as stream:
        ...
        for row_number, raw in enumerate(reader, start=2):
            ...
            item = {
                ...
                "volume_mm3": volume,
                "faces": faces,
                "historical_jaw_s_mm": historical_s,
                "historical_jaw_u_min_mm": historical_u_min,
                "historical_jaw_u_max_mm": historical_u_max,
...
def audit_expected_mapping_uniqueness(groups: Dict[str, List[Dict[str, Any]]]) -> Dict[str, Any]:
    ...
    for contract in contracts:
        ...
        synthetic_actual = {
            ...
            "bbox_mm": geometry["bbox_mm"],
            "bbox_centroid_mm": geometry["bbox_centroid_mm"],
            "mass_centroid_mm": geometry["stl_mass_centroid_mm"],
            "jaw_s_mm": geometry["jaw_s_mm"],
            "jaw_u_min_mm": geometry["jaw_u_min_mm"],
            "jaw_u_max_mm": geometry["jaw_u_max_mm"],
        }
        candidates = []
        for other in contracts:
            deltas = geometry_match_deltas(synthetic_actual, other)
            if geometry_match_passes(synthetic_actual, other, deltas):
                candidates.append(other["source_name"])
        row = {"source_name": contract["source_name"], "candidate_count": len(candidates), "candidates": candidates, "pass": candidates == [contract["source_name"]]}
        rows.append(row)
    if not all(row["pass"] for row in rows):
        raise GateError("EXPECTED_MAPPING_NOT_UNIQUE", "fixed 57-object signatures are ambiguous under runtime mapping tolerances", {"rows": rows})
    return {
        "physical_contracts": len(rows),
        "all_contracts_uniquely_identifiable": True,
        "mapping_fields": ["volume_mm3", "bbox_mm", "bbox_centroid_mm", "stl_mass_centroid_mm", "jaw_s_mm", "jaw_u_min_mm", "jaw_u_max_mm"],
        "tolerances": {
            "volume_abs_mm3": MAPPING_VOLUME_ABS_TOL_MM3,
            "volume_relative": MAPPING_VOLUME_REL_TOL,
            "bbox_coordinate_mm": MAPPING_BBOX_COORD_TOL_MM,
            "bbox_centroid_mm": MAPPING_BBOX_CENTROID_TOL_MM,
            "mass_centroid_mm": MAPPING_MASS_CENTROID_TOL_MM,
            "jaw_s_u_mm": MAPPING_JAW_TOL_MM,
        },
    }
...
def checkpoint_state(key: str) -> Dict[str, Any]:
    ...
    passed = (
        payload.get("schema") == "F3R2_V5_LOOP1C0_PART_CHECKPOINT_V2_AGGREGATE_CORRECTION"
...
def static_audit() -> Dict[str, Any]:
    ...
    return {
        "schema": "F3R2_V5_LOOP1C0_STATIC_AUDIT_V2_AGGREGATE_CORRECTION",
...
def geometry_match_deltas(actual: Dict[str, Any], contract: Dict[str, Any]) -> Dict[str, Any]:
    expected = contract["geometry"]
    return {
        "volume_mm3": abs(float(actual["volume_mm3"]) - float(expected["volume_mm3"])),
        "bbox_max_abs_mm": max(abs(float(a) - float(b)) for a, b in zip(actual["bbox_mm"], expected["bbox_mm"])),
        "bbox_centroid_distance_mm": vector_distance(actual["bbox_centroid_mm"], expected["bbox_centroid_mm"]),
        "mass_centroid_distance_mm": vector_distance(actual["mass_centroid_mm"], expected["stl_mass_centroid_mm"]),
        "jaw_s_abs_mm": abs(float(actual["jaw_s_mm"]) - float(expected["jaw_s_mm"])),
        "jaw_u_min_abs_mm": abs(float(actual["jaw_u_min_mm"]) - float(expected["jaw_u_min_mm"])),
        "jaw_u_max_abs_mm": abs(float(actual["jaw_u_max_mm"]) - float(expected["jaw_u_max_mm"])),
    }


def geometry_match_passes(actual: Dict[str, Any], contract: Dict[str, Any], deltas: Dict[str, Any]) -> bool:
    expected_volume = float(contract["geometry"]["volume_mm3"])
    return (
        mapping_volume_matches(float(actual["volume_mm3"]), expected_volume)
        and deltas["bbox_max_abs_mm"] <= MAPPING_BBOX_COORD_TOL_MM
        and deltas["bbox_centroid_distance_mm"] <= MAPPING_BBOX_CENTROID_TOL_MM
        and deltas["mass_centroid_distance_mm"] <= MAPPING_MASS_CENTROID_TOL_MM
        and max(deltas["jaw_s_abs_mm"], deltas["jaw_u_min_abs_mm"], deltas["jaw_u_max_abs_mm"]) <= MAPPING_JAW_TOL_MM
    )


def geometry_match_score(actual: Dict[str, Any], contract: Dict[str, Any], deltas: Dict[str, Any]) -> float:
    volume_tol = max(MAPPING_VOLUME_ABS_TOL_MM3, abs(float(contract["geometry"]["volume_mm3"])) * MAPPING_VOLUME_REL_TOL)
    return max(
        deltas["volume_mm3"] / volume_tol,
        deltas["bbox_max_abs_mm"] / MAPPING_BBOX_COORD_TOL_MM,
        deltas["bbox_centroid_distance_mm"] / MAPPING_BBOX_CENTROID_TOL_MM,
        deltas["mass_centroid_distance_mm"] / MAPPING_MASS_CENTROID_TOL_MM,
        deltas["jaw_s_abs_mm"] / MAPPING_JAW_TOL_MM,
        deltas["jaw_u_min_abs_mm"] / MAPPING_JAW_TOL_MM,
        deltas["jaw_u_max_abs_mm"] / MAPPING_JAW_TOL_MM,
    )
...
def map_source_bodies(model: Any, groups: Dict[str, List[Dict[str, Any]]], types: Any, pythoncom: Any) -> Tuple[Dict[str, Any], List[Dict[str, Any]]]:
    ...
    for raw in raw_bodies:
        ...
        reported_normalized = normalize_body_name(facts["name"])
        if reported_normalized == normalize_body_name(AGGREGATE_LABEL):
            raise GateError("AGGREGATE_BODY_PRESENT", "SolidWorks source unexpectedly exposes aggregate wrapper 057 as a physical body", facts)
        candidates: List[Tuple[Dict[str, Any], Dict[str, Any]]] = []
        ranked: List[Dict[str, Any]] = []
        for contract in expected.values():
            deltas = geometry_match_deltas(facts, contract)
            score = geometry_match_score(facts, contract, deltas)
            ranked.append({"source_name": contract["source_name"], "score": score, "deltas": deltas})
            if geometry_match_passes(facts, contract, deltas):
                candidates.append((contract, deltas))
        if len(candidates) != 1:
            raise GateError("SOURCE_GEOMETRY_MAPPING_NOT_UNIQUE", "volume+bbox+centroid+jaw signature did not identify exactly one 000..056 row", {"actual": facts, "candidate_count": len(candidates), "candidates": [{"source_name": contract["source_name"], "deltas": deltas} for contract, deltas in candidates], "nearest": sorted(ranked, key=lambda row: row["score"])[:5]})
        contract, deltas = candidates[0]
        normalized = contract["normalized_name"]
        if normalized in mapped:
            raise GateError("SOURCE_GEOMETRY_MAPPING_DUPLICATE", "two SolidWorks bodies mapped to the same physical CSV/STL row", {"source_name": contract["source_name"], "actual": facts})
        # Body names are evidence, not the mapping key.  If SolidWorks happens
        # to report one of the historical labels, it must agree with geometry.
        if reported_normalized in expected and reported_normalized != normalized:
            raise GateError("SOURCE_REPORTED_NAME_CONTRADICTS_GEOMETRY", "SolidWorks body name contradicts its unique geometric signature", {"reported_name": facts["name"], "geometry_source_name": contract["source_name"]})
        runtime = {**facts, "mapped_source_name": contract["source_name"], "mapping_deltas": deltas}
        contract["runtime"] = runtime
        mapped[normalized] = body
        evidence.append({**runtime, "assigned_body": contract["assigned_body"], "historical_csv_volume_mm3": contract["volume_mm3"], "historical_step_faces": contract["faces"], "mapping_authority": "VOLUME_BBOX_CENTROID_JAW_S_U_NOT_GETBODIES2_ORDER"})
...
def set_part_properties(model: Any, key: str, items: Sequence[Dict[str, Any]]) -> None:
    ...
    properties = {
        ...
        "GEOMETRY_BINDING": "VOLUME_BBOX_CENTROID_JAW_S_U",
...
def audit_target_model(model: Any, key: str, items: Sequence[Dict[str, Any]], types: Any, pythoncom: Any) -> Dict[str, Any]:
    ...
    for raw in raw_bodies:
        ...
        copy_deltas = {
            ...
            "jaw_s_abs_mm": abs(float(facts["jaw_s_mm"]) - float(runtime["jaw_s_mm"])),
            "jaw_u_min_abs_mm": abs(float(facts["jaw_u_min_mm"]) - float(runtime["jaw_u_min_mm"])),
            "jaw_u_max_abs_mm": abs(float(facts["jaw_u_max_mm"]) - float(runtime["jaw_u_max_mm"])),
        }
        if (
            ...
            or max(copy_deltas["jaw_s_abs_mm"], copy_deltas["jaw_u_min_abs_mm"], copy_deltas["jaw_u_max_abs_mm"]) > 1.0e-4
        ):
...
def write_checkpoint(key: str, result: Dict[str, Any], cold: Dict[str, Any]) -> Dict[str, Any]:
    target = Path(GROUPS[key]["target"])
    payload = {
        "schema": "F3R2_V5_LOOP1C0_PART_CHECKPOINT_V2_AGGREGATE_CORRECTION",
        "timestamp_utc": utc_now(),
        "group": key,
        "role": GROUPS[key]["role"],
        "binding": binding(),
...
def execute_parts() -> int:
    ...
    try:
        ...
        final = {
            "schema": "F3R2_V5_LOOP1C0_GRIPPER_NATIVE_PART_RECEIPT_V2_AGGREGATE_CORRECTION",
            ...
            "source_open": {"path": posix(SOURCE_PART), "read_only": True, "physical_body_count": 57, "actual_face_count": sum(int(row["faces"]) for row in source_evidence), "actual_volume_mm3": sum(float(row["volume_mm3"]) for row in source_evidence), "mapping_authority": "VOLUME_BBOX_CENTROID_JAW_S_U_NOT_GETBODIES2_ORDER", "evidence": source_evidence},
            "assignment": assignment_summary(groups),
            "aggregate_exclusion": geometry["aggregate"],
        ...
        failure = {
            "schema": "F3R2_V5_LOOP1C0_FAILURE_RECEIPT_V2_AGGREGATE_CORRECTION",
            "timestamp_utc": utc_now(),
            "code": code,
            "message": str(exc),
            "detail": detail,
