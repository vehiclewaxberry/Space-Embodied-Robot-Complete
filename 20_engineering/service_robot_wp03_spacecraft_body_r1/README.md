...
R17 修复后（2026-09-19）`export_parts_and_bom.py` 内部强制执行顺序：有效输入哈希校验 → 构建/回执 → `dynamics_handoff.py` 子进程生成 HANDOFF → HANDOFF 逐项校验 → 最后写 BOM/INTERFACES；任何一项失配即 fail-closed 拒绝并恢复旧交付物字节。`dynamics_handoff.py` 仍可独立复算（如 `--self-test`），但不再需要在 BOM 之前手动先跑；`--bom-only` 为纯读取入口（不导入 CAD 模块），同样先校验后写出，旧交接配新 CAD 会被拒绝。
