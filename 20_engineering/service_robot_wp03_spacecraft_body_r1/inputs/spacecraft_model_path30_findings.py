...
def build(state='service',view='complete',include_arm=True,write_parts=False):
    ...
    for k,row in enumerate(CONTACT['stations']):
        ...
        for xx in [-10,10]:
            add(cylinder(4,70,(xx,0,shoe-z0-35)),f'hold_shoe_guide_{k}_{xx}',parent='ONBOARD_RETENTION',mount=f'hold_fold_mast_{k}',T=T,density=7.85e-6,color=DARK,motion_group=f'cradle_{k}')
