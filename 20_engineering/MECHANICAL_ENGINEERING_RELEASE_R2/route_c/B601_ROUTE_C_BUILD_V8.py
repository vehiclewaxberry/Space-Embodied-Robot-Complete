# B601 Route-C Guided Dress Pack V8 candidate-2 - headless FreeCAD build script.
...
# Run: FreeCADCmd.exe B601_ROUTE_C_BUILD_V8.py
...
OUT_FCSTD = os.path.join(SCRIPT_DIR, "B601_ROUTE_C_GUIDED_DRESS_PACK_V8.FCStd")
OUT_STEP = os.path.join(SCRIPT_DIR, "B601_ROUTE_C_GUIDED_DRESS_PACK_V8.step")
OUT_CENTERLINE = os.path.join(SCRIPT_DIR, "B601_ROUTE_C_HARNESS_CENTERLINE_V8.json")
OUT_CLAMPS = os.path.join(SCRIPT_DIR, "B601_ROUTE_C_CLAMP_AND_GUIDE_REGISTER_V8.csv")
OUT_MASS = os.path.join(SCRIPT_DIR, "B601_ROUTE_C_MASS_DELTA_BY_LINK_CANDIDATES_V8.json")
OUT_RECEIPT = os.path.join(SCRIPT_DIR, "B601_ROUTE_C_BUILD_RECEIPT_V8.json")
...
# V8 D2-A: enlarge the J1 service-loop radius without changing the joint axis,
...
M4 = (M4[0], YC, 195.1)
# V8 D1 candidate-3: retain K2, then move the link4 bypass south/outboard and
# upward before entering J5.  Candidate-2's east leg crossed the link5 high
# return at [166,43,277] mm and is preserved as an immutable negative result.
K2 = (115.0, YC - 30.0, 270.0)
E1_J5 = (180.0, -60.0, 320.0)
E2_J5 = (160.0, -180.0, 260.0)
J4_HOST_TRANSITION_ARC, J4_HOST_TRANSITION_T1, J4_HOST_BOUNDARY = \
    canonical_fillet_section(j4_end, M4, K2, 55.0)
...
SEGMENTS = [
    ...
    },
    {
        ...
        "note": "V8 D2-A protected annular service loop: 1.2-turn CW coil R66 pitch -12; tangent exit to J2",
    },
    {
    ...
    },
    {
        ...
        "sections": [
            ...
            ("poly", [J3_HOST_BOUNDARY, F4, T1_J4], [55.0]),
        ],
        ...
        "note": ("festoon take-up via moving carriage (2x110=220), R63 guide saddle, "
                 "and explicit R55 J3 host transition; post-boundary channel is link3-only"),
    },
    {
        ...
        "sections": [
            ...
            ("poly", [j4_end, J4_HOST_TRANSITION_T1], []),
            J4_HOST_TRANSITION_ARC,
            ("poly", [J4_HOST_BOUNDARY, K2, E1_J5, E2_J5, T1_J5],
             [55.0, 55.0, 55.0]),
        ],
        "take_up_required_mm": 55.0*3.44,
        "take_up_capacity_mm": 55.0*math.radians(abs(j4_sweep)),
        "note": ("V8 candidate-3 D1: J4 loop and explicit R55 host-transition fillet are "
                 "dual-host compliance; post-boundary south/outboard high bypass is link4-only"),
    },
    {
        ...
        "note": "V8 D1-A external J5 wrap R55 CW 185 deg and R55 high return dogleg to the frozen J6 entry tangent",
    },
    {
...
# FreeCAD build
# ==========================================================================
doc = App.newDocument("B601_ROUTE_C_GUIDED_DRESS_PACK_V8")
part_registry = []
part_objects = {}
receipt_parts = []
part_axis_registry = {}
...
def refresh_part_metrics(name):
    ...
    for rec in receipt_parts:
        if rec.get("name") == name:
            ...
            rec["mass_g_estimate"] = vol * DENS[mat] if mat in DENS else None
            return
...
J3_CARR_PT, J3_CARR_TAN = route_station_near("SEG-02_J2_GUIDED_U_LOOP_AND_LINK2_CHANNEL",
                                             (-140.0, YC, 147.0))
_l3_seed_dir = v_norm(v_sub(F4, J3_HOST_BOUNDARY))
_l3_seed_1 = v_add(J3_HOST_BOUNDARY,
                    v_mul(_l3_seed_dir, v_dist(J3_HOST_BOUNDARY, F4)/3.0))
_l3_seed_2 = v_add(J3_HOST_BOUNDARY,
                    v_mul(_l3_seed_dir, 2.0*v_dist(J3_HOST_BOUNDARY, F4)/3.0))
...
J4_FIX_PT, J4_FIX_TAN = route_station_near("SEG-03_J3_CARRIER_HYBRID_WRAP", F4)
_l4_seed = v_add(J4_HOST_BOUNDARY,
                 v_mul(v_norm(v_sub(K2, J4_HOST_BOUNDARY)),
                       0.544*v_dist(J4_HOST_BOUNDARY, K2)))
L4_01_PT, L4_01_TAN = route_station_near("SEG-04_J4_LOOP_LINK4_CHANNEL_RISER", _l4_seed)
L4_E1_PT, L4_E1_TAN = route_station_near("SEG-04_J4_LOOP_LINK4_CHANNEL_RISER", E1_J5)
L4_E2_PT, L4_E2_TAN = route_station_near("SEG-04_J4_LOOP_LINK4_CHANNEL_RISER", E2_J5)
...
clamp_saddle("RC-CLP-J1-RELIEF-A1", j1_relief_a1, j1_relief_tan_a1,
             "polymer", "base_link", "V8 D3 first-pass 190deg relief-entry retainer", width=10.0)
clamp_saddle("RC-CLP-J1-RELIEF-B1", j1_relief_b1, j1_relief_tan_b1,
             "polymer", "base_link", "V8 D3 first-pass 100deg relief-exit retainer", width=10.0)
clamp_saddle("RC-CLP-J1-RELIEF-A2", j1_relief_a2, j1_relief_tan_a2,
             "polymer", "base_link", "V8 D3 second-pass 190deg relief-entry retainer", width=10.0)
...
clamp_saddle("RC-CLP-J3-HOST-BOUNDARY", J3_HOST_BOUNDARY,
             v_norm(v_sub(F4, J3_HOST_BOUNDARY)), "aluminum", "link3",
             "CM-J3-HB physical host-boundary clamp on link3")
channel_section("RC-CHN-L3", J3_HOST_BOUNDARY, F4, 12.0, 12.0, "aluminum", "link3",
                "link3-only fixed channel after the J3 host boundary")
l3_dir = v_norm(v_sub(F4, J3_HOST_BOUNDARY))
l3_p1 = v_add(J3_HOST_BOUNDARY, v_mul(l3_dir, v_dist(J3_HOST_BOUNDARY, F4)/3.0))
l3_p2 = v_add(J3_HOST_BOUNDARY, v_mul(l3_dir, 2.0*v_dist(J3_HOST_BOUNDARY, F4)/3.0))
...
clamp_saddle("RC-CLP-J4-FIX", J4_FIX_PT, J4_FIX_TAN, "aluminum", "link3", "CF-J4-F fixed clamp on link3 side of J4 loop")
...
clamp_saddle("RC-CLP-J4-MOV", J4_HOST_BOUNDARY, v_norm(v_sub(K2, M4)), "aluminum", "link4",
             "CM-J4-M moving clamp and physical host boundary on link4")

# link4 tilted channel (rises to J5 wrap plane) + clamps
channel_section("RC-CHN-L4", J4_HOST_BOUNDARY, K2, 12.0, 12.0, "aluminum", "link4",
                "link4-only fixed channel after the J4 host boundary")
l4_clamp_pt = L4_01_PT
clamp_saddle("RC-CLP-L4-01", L4_01_PT, L4_01_TAN, "aluminum", "link4", "CF-L4-01 fixed clamp at channel mid")
guide_tube_poly("RC-GDE-J5-HIGH-BYPASS", [K2, E1_J5, E2_J5, T1_J5],
                [55.0, 55.0], 6.0, "aluminum", "link4",
                "V8 D1-A external R55 high-bypass guide", "guide_channel")
guide_tube_poly("RC-GDE-J5-HIGH-BYPASS-LINER", [K2, E1_J5, E2_J5, T1_J5],
                [55.0, 55.0], 7.5, "polymer", "link4",
                "V8 D1-A high-bypass low-friction liner", "liner")
guide_support_saddle("RC-GSP-L4-HIGH-E1", L4_E1_PT, L4_E1_TAN, "aluminum", "link4",
                     "V8 candidate-2 high-bypass conduit support E1")
guide_support_saddle("RC-GSP-L4-HIGH-E2", L4_E2_PT, L4_E2_TAN, "aluminum", "link4",
                     "V8 candidate-2 high-bypass conduit support E2")
...
guide_tube_poly("RC-GDE-J5-HIGH-RETURN", [j5_end, W2, W3, A6, H0],
                [55.0, 55.0, 55.0], 6.0, "aluminum", "link5",
                "V8 D1-A R55 high return guide to J6 entry", "guide_channel")
guide_tube_poly("RC-GDE-J5-HIGH-RETURN-LINER", [j5_end, W2, W3, A6, H0],
                [55.0, 55.0, 55.0], 7.5, "polymer", "link5",
                "V8 D1-A high return low-friction liner", "liner")
guide_support_saddle("RC-GSP-J5-HIGH-W2", J5_W2_PT, J5_W2_TAN, "aluminum", "link5",
                     "V8 candidate-2 high-return conduit support W2")
guide_support_saddle("RC-GSP-J5-HIGH-W3", J5_W3_PT, J5_W3_TAN, "aluminum", "link5",
                     "V8 candidate-2 high-return conduit support W3")
guide_support_saddle("RC-GSP-J5-HIGH-A6", J5_A6_PT, J5_A6_TAN, "aluminum", "link5",
                     "V8 candidate-2 J6-entry conduit support A6")
...
CLAMPS = [
    ...
    ("CF-J1-REL-A1", "base_link", j1_relief_a1, "RELIEF_END", "2xM3 on V8 annulus cheek", "first-pass 190deg entry on actual helix"),
    ("CF-J1-REL-B1", "base_link", j1_relief_b1, "RELIEF_END", "2xM3 on V8 annulus cheek", "first-pass 100deg exit on actual helix"),
    ("CF-J1-REL-A2", "base_link", j1_relief_a2, "RELIEF_END", "2xM3 on V8 annulus cheek", "second-pass 190deg entry on actual helix"),
    ...
    ("CF-J4-F", "link3", J4_FIX_PT, "FIXED", "2xM3 on RC-CHN-L3", "J4 loop fixed clamp (link3 side)"),
    ("CM-J4-M", "link4", J4_HOST_BOUNDARY, "MOVING", "2xM3 on RC-CHN-L4", "J4 physical host-boundary clamp on link4"),
    ("CF-L4-01", "link4", l4_clamp_pt, "FIXED", "2xM3 on RC-CHN-L4", "link4 channel end clamp"),
    ("GS-L4-HIGH-E1", "link4", L4_E1_PT, "GUIDE_SUPPORT", "2xM3 on V8 high-bypass guide", "conduit-axis support E1; not a cable clamp"),
    ("GS-L4-HIGH-E2", "link4", L4_E2_PT, "GUIDE_SUPPORT", "2xM3 on V8 high-bypass guide", "conduit-axis support E2; not a cable clamp"),
    ("CF-J5-F", "link4", T1_J5, "FIXED", "2xM3 on V8 high-bypass guide", "V8 external J5 wrap entry fixed clamp"),
    ("CM-J5-M", "link5", j5_end, "MOVING", "2xM3 on link5 derived pad", "J5 wrap exit moving clamp"),
    ("GS-J5-HIGH-W2", "link5", J5_W2_PT, "GUIDE_SUPPORT", "2xM3 on V8 high-return guide", "conduit-axis support W2; not a cable clamp"),
    ("GS-J5-HIGH-W3", "link5", J5_W3_PT, "GUIDE_SUPPORT", "2xM3 on V8 high-return guide", "conduit-axis support W3; not a cable clamp"),
    ("GS-J5-HIGH-A6", "link5", J5_A6_PT, "GUIDE_SUPPORT", "2xM3 on V8 high-return guide", "conduit-axis support A6; not a cable clamp"),
...
mass_delta_doc = {
    "schema": "B601_ROUTE_C_MASS_DELTA_BY_LINK_CANDIDATES_V8",
...
centerline_doc = {
    "schema": "B601_ROUTE_C_HARNESS_CENTERLINE_V8",
    "generated_utc": "DETERMINISTIC_REPLAY_NO_WALLCLOCK",
    "design_candidate_sequence": 3,
    "frame_convention": {
...
receipt = {
    "schema": "B601_ROUTE_C_BUILD_RECEIPT_V8",
    "generated_utc": "DETERMINISTIC_REPLAY_NO_WALLCLOCK",
    "design_candidate_sequence": 3,
    "supersedes_candidate": "V8_LOOP2_HOST_SPLIT_WITH_EAST_LEG (rejected, immutable archive)",
    ...
    "candidate_3_changes": {
        "D1": "explicit J4 host split plus south/outboard/upward B1-B2 bypass replacing the rejected east leg",
        "D2": "R66 J1 annular service loop retained from candidate-1",
        "D3": "actual two-layer helix relief crossings and three fixed relief retainers plus moving exit retainer",
        "J3_host_boundary": "explicit R55 transition and physical CM-J3-HB clamp; only the downstream section is link3-only",
        "support_semantics": "high-route hardware reclassified as conduit GUIDE_SUPPORT with registered bore/axis",
        "clamp_semantics": "all cable clamps use 6 mm bore radius against 5 mm upper bundle radius plus 0.5 mm installation allowance"
    },
}
