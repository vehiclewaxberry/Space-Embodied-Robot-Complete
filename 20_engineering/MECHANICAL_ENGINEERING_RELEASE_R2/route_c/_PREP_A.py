...
stub = {"schema": "ROUTE_C_SWEEP_MESH_PACK_%s_MANIFEST" % VARIANT,
        "variant": VARIANT, "stage": "A_vendor_solar_rails",
        "freecad_version": ".".join(App.Version()[0:3]),
        "arm_entries": manifest_arm,
        "solar_tris": int(len(solar_tris)),
        "rail_entries": rail_entries,
        "rail_frame_valid": rail_frame_valid,
        "palm_med_mm": palm_med, "palm_p90_mm": palm_p90}
with open(os.path.join(PACK_DIR, "_prep_A_receipt.json"), "w", encoding="utf-8", newline="
") as f:
    _json.dump(stub, f, indent=2, sort_keys=True)
print("PREP_A_COMPLETE")
