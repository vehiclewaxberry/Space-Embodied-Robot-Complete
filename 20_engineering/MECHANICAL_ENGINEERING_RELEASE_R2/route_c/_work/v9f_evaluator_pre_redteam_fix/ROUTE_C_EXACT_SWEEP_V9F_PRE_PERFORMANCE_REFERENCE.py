...
def main():
    ...
    def eval_clearance(q, pinch_only=False):
        ...
        for hi, h in enumerate(HOSTS):
            for altflag in (0, 1):
                ...
                j4_si_local = seg_id_list.index(J4_SEGMENT_ID)
                j4_dynamic = ((seg_of_pts == j4_si_local)
                ...
                # Rigid own-host pairs are precomputed once at q=0.  V9F J4
                # sections 1/2/3/5 change in the link3 frame and therefore must
                # be checked against link3 at every pose.
                for lname, fld in vendor_fields.items():
                    own_dynamic = (lname == h) & j4_dynamic
                    if lname == h and not own_dynamic.any():
                        continue
                ...
                for (pname, phost, pkind, motion_class, fld) in rc_parts:
                    ...
                    okm = ok & ~intended
                    comparison_counts["route_c_hardware"] += int(okm.sum())
                    if not okm.any():
                        continue
                    gated = c[okm] - dr[okm]
                    k = int(np.argmin(gated))
    ...
    if FAST_MODE:
        ...
        for q in mission_q_samples:
            ...
            j4s = j4_dynamic_state(q, T)
            dx_car = 0.5*J3_SADDLE_R*(float(q[2])-Q3_DATUM)
            dx_car = max(-55.0, min(55.0, dx_car))
            for pname, phost, pkind, motion_class, fld_part in rc_parts:
                Mpart = rc_part_pose_S(pname, phost, motion_class, fld_part,
                                       T, q, dx_car, j4s)
                P = xform_batch(Mpart, fld_part.v0)
