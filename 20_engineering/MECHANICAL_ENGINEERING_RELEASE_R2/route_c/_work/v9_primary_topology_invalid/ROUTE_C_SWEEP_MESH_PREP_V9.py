...
#
# Run: FreeCADCmd.exe ROUTE_C_SWEEP_MESH_PREP_V9.py
import os
import json
import shutil
import hashlib
...
for obj in list(doc.Objects):
    ...
    if meta.get("kind") == "bundle_envelope":
        ...
        if name in {"RC-CAR-J4-CARRIAGE", "RC-GDE-J4-CARRIER-SADDLE",
                    "RC-GDE-J4-CARRIER-SADDLE-LINER"}:
            rc_entries[-1]["motion_class"] = "J4_CARRIAGE_RIGID_TRANSLATION"
            rc_entries[-1]["motion_law"] = "dx_mm=27.5*q4_rad"
        elif name.startswith("RC-CHN-J4-PLANE-A-LINK-"):
            x0 = float(cg[0])
            rc_entries[-1]["motion_class"] = "J4_PLANE_A_DISTRIBUTED"
            rc_entries[-1]["q0_x_mm"] = x0
            rc_entries[-1]["station_fraction"] = (x0 - 20.0) / (164.125 - 20.0)
        elif name.startswith("RC-CHN-J4-PLANE-B-LINK-"):
            x0 = float(cg[0])
            rc_entries[-1]["motion_class"] = "J4_PLANE_B_DISTRIBUTED"
            rc_entries[-1]["q0_x_mm"] = x0
            rc_entries[-1]["station_fraction"] = (x0 - 75.0) / (164.125 - 75.0)
    rc_mass_rows.append({
