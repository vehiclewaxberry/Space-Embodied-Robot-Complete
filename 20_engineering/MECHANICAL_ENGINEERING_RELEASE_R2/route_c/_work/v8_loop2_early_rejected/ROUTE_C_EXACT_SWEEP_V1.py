...
# time (the mesh pack is pre-built and hash-pinned by ROUTE_C_SWEEP_MESH_PREP_V1.py).
#
# Run:  python ROUTE_C_EXACT_SWEEP_V1.py            (from this directory)
...
#
# See ROUTE_C_EXACT_SWEEP_V1.json "kinematic_attachment_model" and
...
if VARIANT not in ("V1", "V2", "V3", "V4", "V5", "V6", "V7", "V8", "VF"):
    raise SystemExit("RC_VARIANT must be one of V1..V8 or VF")

# VF is the final-gate alias of the V7 hardware definition: it consumes the V7
# centerline / clamp register / mesh pack but writes VF-named authoritative
# outputs.  All V1..V8 artifacts are preserved (no overwrite).
...
SEG_INTENDED_PARTS = {
    ...
    "SEG-03_J3_CARRIER_HYBRID_WRAP": {
        ...
        "RC-CHN-L3-WALL-B", "RC-CHN-L3-LINER", "RC-CLP-L3-01", "RC-CLP-L3-02"},
    "SEG-04_J4_LOOP_LINK4_CHANNEL_RISER": {
        "RC-GDE-J4-MANDREL", "RC-GDE-J4-MANDREL-LINER", "RC-CLP-J4-FIX",
        "RC-CLP-J4-MOV", "RC-CHN-L4-BASE", "RC-CHN-L4-WALL-A", "RC-CHN-L4-WALL-B",
        "RC-CHN-L4-LINER", "RC-CLP-L4-01", "RC-GDE-J5-HIGH-BYPASS",
...
def main():
    ...
    if not pins["accepted_urdf"]["match"]:
        print("ABORT: accepted URDF hash mismatch", flush=True)
        raise SystemExit(3)
    ...
    host_plan = {
        ...
        ("SEG-04_J4_LOOP_LINK4_CHANNEL_RISER", 1): ("link3", "link4"),
        ("SEG-04_J4_LOOP_LINK4_CHANNEL_RISER", 2): ("link3", "link4"),
        ("SEG-04_J4_LOOP_LINK4_CHANNEL_RISER", 3): ("link4", None),
    ...
    seg_records = []
    all_pts, all_host, all_alt, all_seg, all_sec = [], [], [], [], []
    min_routes = []
    ...
    rc_parts = []   # (name, host, kind, TriField)
    rc_verts_by_host = {}
    for grp in manifest["files"]:
        if grp["group"] == "route_c_parts":
            for e in grp["entries"]:
                ...
                rc_parts.append((e["name"], e["host_link"], e["kind"], fld))
    ...
    seg_takeup = [
        ...
        ("SEG-04_J4_LOOP_LINK4_CHANNEL_RISER", "joint4", _loop_R("SEG-04_J4_LOOP_LINK4_CHANNEL_RISER", "joint4"),
         _seg_by_id["SEG-04_J4_LOOP_LINK4_CHANNEL_RISER"]["take_up_capacity_mm"]),
    ...
    # Adjacent segment guide hardware legitimately overlaps the cable only in a
    # bounded transition neighborhood.  This is not a whole-part/global
    # allowlist: each rule is tied to the shared q=0 boundary and a 65 mm
    # centerline window, so remote contact remains gated.
    def _segment_endpoint(si, first):
        pts = P_A0[H_SEG == si]
        return pts[0] if first else pts[-1]

    def _shared_boundary(si_left, si_right):
        return 0.5*(_segment_endpoint(si_left, False) +
                    _segment_endpoint(si_right, True))

    INTERFACE_WINDOW_MM = 65.0
    interface_rules = [
        (0, _shared_boundary(0, 1), {"RC-GDE-J1-ANNULUS-LOW", "RC-GDE-J1-ANNULUS-UP",
                                    "RC-GDE-J1-ANNULUS-WALL", "RC-GDE-J1-LINER"}),
        (1, _shared_boundary(1, 2), {"RC-GDE-J2-MANDREL", "RC-GDE-J2-MANDREL-LINER",
                                    "RC-CLP-J2-FIX"}),
        (2, _shared_boundary(2, 3), {"RC-GDE-J3-SADDLE", "RC-GDE-J3-SADDLE-LINER"}),
        (3, _shared_boundary(3, 4), {"RC-GDE-J4-MANDREL", "RC-GDE-J4-MANDREL-LINER",
                                    "RC-CLP-J4-FIX"}),
        (4, _shared_boundary(4, 5), {"RC-GDE-J5-WRAP", "RC-GDE-J5-WRAP-LINER",
                                    "RC-CLP-J5-MOV"}),
        (5, _shared_boundary(4, 5), {"RC-GDE-J5-HIGH-BYPASS",
                                    "RC-GDE-J5-HIGH-BYPASS-LINER", "RC-CLP-J5-FIX"}),
        (5, _shared_boundary(5, 6), {"RC-GDE-J6-RING-0", "RC-CLP-J6-FIX"}),
        (6, _shared_boundary(5, 6), {"RC-GDE-J5-HIGH-RETURN",
                                    "RC-GDE-J5-HIGH-RETURN-LINER",
                                    "RC-GSP-J5-HIGH-A6"}),
    ]
    interface_rules_by_seg = {}
    for si, boundary, parts in interface_rules:
        interface_rules_by_seg.setdefault(si, []).append((boundary, parts))
    section_intended_parts = {
        # The J5 joint-crossing wrap slides through the upstream link4 conduit
        # across its allocated wrap range.  Restrict the exclusion to section 0
        # (the declared joint-crossing arc); the link5 return section remains
        # fully gated against the upstream conduit.
        (seg_id_list.index("SEG-05_J5_WRIST_WRAP"), 0): {
            "RC-GDE-J5-HIGH-BYPASS", "RC-GDE-J5-HIGH-BYPASS-LINER",
            "RC-CLP-J5-FIX", "RC-GSP-L4-HIGH-E2"
        },
    }
    ...
    def eval_clearance(q, pinch_only=False):
        ...
        T_off_car = tr(dx_car, 0.0, 0.0)
        for (pname, phost, pkind, fld) in rc_parts:
            Th = T.get(phost, np.eye(4))
            iT0h = inv_T0.get(phost, np.eye(4))
            if pname in CARRIAGE_PARTS:
                Th = Th @ T_off_car
            aabb_rc[pname] = _field_aabb_S(fld, arm.mount @ Th @ iT0h)
        ...
        for hi, h in enumerate(HOSTS):
            for altflag in (0, 1):
                ...
                for (pname, phost, pkind, fld) in rc_parts:
                    ...
                    if pname in CARRIAGE_PARTS:
                        Th = Th @ T_off_car
                    Mloc = np.linalg.inv(Th @ iT0h) @ arm.inv_mount
                    ...
                    intended = np.array([pname in seg_intended_idx[int(s)]
                                         for s in seg_of_pts])
                    for key, parts in section_intended_parts.items():
                        if pname in parts:
                            intended |= ((seg_of_pts == key[0]) & (sec_of_pts == key[1]))
                    if not intended.all():
                        P0_subset = P_A0[m]
                        for si in set(int(x) for x in seg_of_pts):
                            for boundary, parts in interface_rules_by_seg.get(si, []):
                                if pname not in parts:
                                    continue
                                ms = seg_of_pts == si
                                intended[ms] |= (np.linalg.norm(P0_subset[ms] - boundary,
                                                               axis=1)
                                                   <= INTERFACE_WINDOW_MM)
                    okm = ok & ~intended
                    if not okm.any():
