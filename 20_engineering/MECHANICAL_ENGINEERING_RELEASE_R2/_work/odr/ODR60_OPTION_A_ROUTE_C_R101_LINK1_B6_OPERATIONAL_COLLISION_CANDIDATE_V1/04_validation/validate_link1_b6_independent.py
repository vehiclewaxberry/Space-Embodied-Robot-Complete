#!/usr/bin/env python3
"""Independent link1 B6 validator. It deliberately imports no builder module."""
...
REEXTRACTOR = PACKAGE / "04_validation/reextract_link1_b6_independent.py"
GEOMETRY_INDEX = PACKAGE / "05_results/LINK1_B6_GEOMETRY_INDEX_V1.json"
RUNTIME_RECEIPT = PACKAGE / "05_results/LINK1_B6_RUNTIME_SIDECAR_RECEIPT_V1.json"
POSE_RECEIPT = PACKAGE / "05_results/LINK1_B6_POSE_ADAPTER_SAMPLES_V1.json"
OUTPUT_GEOMETRY = PACKAGE / "05_results/INDEPENDENT_LINK1_B6_VALIDATION_V1.json"
...
def independent_fk(urdf: Path) -> tuple[dict[str, Any], Any]:
    ...
    matches = [item for item in document.findall("joint") if item.get("name") == "joint1"]
    require(len(matches) == 1, "independent joint1 count drift")
    item = matches[0]
    require(item.get("type") == "revolute" and item.find("parent").get("link") == "base_link" and item.find("child").get("link") == "link1", "independent joint1 topology drift")
    xyz_raw = item.find("origin").get("xyz").split(); rpy_raw = item.find("origin").get("rpy").split(); axis_raw = item.find("axis").get("xyz").split()
    xyz = np.asarray([float(v) for v in xyz_raw]); angles = np.asarray([float(v) for v in rpy_raw]); axis = np.asarray([float(v) for v in axis_raw])
    lower_raw, upper_raw = item.find("limit").get("lower"), item.find("limit").get("upper")
    lower, upper = float(lower_raw), float(upper_raw)
    origin = np.eye(4); origin[:3, :3] = rpy(*angles); origin[:3, 3] = xyz
    def fk(q: float) -> np.ndarray:
        require(math.isfinite(q) and lower <= q <= upper, "independent q1 limit rejection")
        motion = np.eye(4); motion[:3, :3] = rodrigues(axis, q)
        return origin @ motion
    raw = {"origin_xyz_m": xyz_raw, "origin_rpy_rad": rpy_raw, "axis": axis_raw, "limit_lower_rad": lower_raw, "limit_upper_rad": upper_raw}
    return raw, fk
...
def main() -> None:
    ...
    require(len(contract["objects"]) == 6 and lock["source_pin_count"] == 10 and len(lock["source_pins"]) == 10, "contract/source count drift")
    paths = {}
    ...
    exact = {row["object_id"]: row for row in registry["objects"] if row["category"] == "R" and row["parent_frame"] == "link1"}
    require(set(exact) == {row["object_id"] for row in contract["objects"]}, "independent exact link1 mapping drift")
    raw_joint, fk = independent_fk(paths["accepted_urdf"])
    ...
    t0 = fk(0.0); inv0 = np.linalg.inv(t0)
    ...
    object_rows = []
    with tempfile.TemporaryDirectory(prefix="route_c_link1_b6_independent_") as directory_name:
        ...
        extraction = json.loads(completed.stdout); require(extraction["verdict"] == "INDEPENDENT_SOURCE_REEXTRACTION_6_OF_6_PASS" and extraction["object_placement_reapplied"] is False, "independent source extraction verdict drift")
        ...
        for spec in contract["objects"]:
            ...
            expected = transform(source, inv0_mm); expected_metric = solid_check(expected, f"{object_id} expected link1")
            ...
            object_rows.append({
                ...
                "expected_link1_bbox_mm": expected_metric["bbox_mm"].tolist(), "step_bbox_mm": step_metric["bbox_mm"].tolist(),
    ...
    geometry_document = {
        "schema": "ROUTE_C_LINK1_B6_INDEPENDENT_VALIDATION_V1", "generated_utc": "DETERMINISTIC_VALIDATION_NO_WALLCLOCK",
        "validator_imports_builder": False, "source_pin_count_verified": len(paths), "exact_registry_link1_object_count": len(exact),
        ...
        "verdict": "INDEPENDENT_LINK1_B6_STEP_BREP_RUNTIME_VALIDATION_PASS__LOCAL_CANDIDATE_ONLY",
    }
    pose_samples = []
    sample_by_name = {row["sample"]: row for row in pose_receipt["samples"]}
    for sample in ledger["pose_samples"]:
        q = float(sample["q1_rad"]); expected = mount @ fk(q); actual = np.asarray(sample_by_name[sample["sample"]]["T_S_link1_m"], dtype=np.float64)
        ...
        pose_samples.append({"sample": sample["sample"], "q1_rad": q, "matrix_max_abs_residual": residual})
    q0_closure = float(np.max(np.abs((mount @ fk(0.0)) @ inv0 - mount)))
    require(q0_closure <= contract["tolerances"]["fk_matrix_max_abs"], "independent q0 S closure drift")
    require(raw_joint == pose_receipt["joint1_raw"], "builder/independent raw URDF numeric spelling mismatch")
    pose_document = {
        "schema": "ROUTE_C_LINK1_B6_INDEPENDENT_POSE_ADAPTER_VALIDATION_V1", "generated_utc": "DETERMINISTIC_VALIDATION_NO_WALLCLOCK",
        "validator_imports_builder": False, "accepted_urdf_raw_numeric_spelling": raw_joint,
        "execution_mount_12dp_strings_consumed_directly": strings, "execution_mount_reconstructed_from_25deg": False,
        "historical_D6_mount_used": False, "q0_source_A0_to_link1_to_S_closure_max_abs": q0_closure,
        "samples": pose_samples, "side_effect_free": True, "system_authority_changed": False,
        "verdict": "INDEPENDENT_ACCEPTED_URDF_JOINT1_FK_AND_12DP_MOUNT_POSE_ADAPTER_PASS",
    }
    if args.write:
