#!/usr/bin/env python3
"""Build the exact Route-C R95 link2 B12 as STEP-first host-local candidates."""
...
from link2_pose_adapter import link2_transform_A0_link2, load_adapter, pose_matrix_S_link2, raw_joint
...
EXTRACTOR = PACKAGE / "02_builder/extract_v9f_link2_b12_brep.py"
CAD_DIR = PACKAGE / "01_cad"
RUNTIME_DIR = PACKAGE / "02_runtime"
RESULTS_DIR = PACKAGE / "05_results"
GEOMETRY_INDEX = RESULTS_DIR / "LINK2_B12_GEOMETRY_INDEX_V1.json"
RUNTIME_RECEIPT = RESULTS_DIR / "LINK2_B12_RUNTIME_SIDECAR_RECEIPT_V1.json"
POSE_RECEIPT = RESULTS_DIR / "LINK2_B12_POSE_ADAPTER_SAMPLES_V1.json"
BUILD_RECEIPT = RESULTS_DIR / "TWELVE_STEP_BUILD_RECEIPT_V1.json"
...
def run_extractor(directory: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    require(document["verdict"] == "LINK2_B12_TRANSIENT_BREP_EXTRACTION_PASS__OBJECT_PLACEMENT_NOT_REAPPLIED", "extractor verdict drift")
...
def build_artifacts() -> tuple[dict[Path, bytes], dict[str, Any]]:
    ...
    mount = adapter["mount"]
    t_a0_l2_0_m = link2_transform_A0_link2(0.0, 0.0, joints)
    t_l2_a0_mm = matrix_m_to_mm(np.linalg.inv(t_a0_l2_0_m))
    t_a0_l2_0_mm = matrix_m_to_mm(t_a0_l2_0_m)
    specs = contract["objects"]
    require(len(specs) == 12, "contract must contain exactly twelve objects")
    require(len({row["object_id"] for row in specs}) == 12, "object id uniqueness drift")
    require(len({row["source_label"] for row in specs}) == 12, "source label uniqueness drift")
    require(len({row["output"] for row in specs}) == 12, "output uniqueness drift")

    registry = strict_json(paths["m01_registry"])
    all_link2_rows = {row["object_id"]: row for row in registry["objects"] if row["category"] == "R" and row["parent_frame"] == "link2"}
    selected_ids = {row["object_id"] for row in specs}
    excluded_ids = set(contract["excluded_j3_carriage_objects"])
    require(len(all_link2_rows) == 20, "registry link2 population drift")
    require(len(excluded_ids) == contract["requirements"]["excluded_j3_carriage_count"] == 8, "J3 carriage exclusion count drift")
    require(selected_ids.isdisjoint(excluded_ids) and selected_ids | excluded_ids == set(all_link2_rows), "link2 selected/excluded partition drift")
    registry_rows = {object_id: all_link2_rows[object_id] for object_id in selected_ids}
    ...
    geometry_rows = []
    runtime_rows = []
    with tempfile.TemporaryDirectory(prefix="route_c_link2_b12_brep_") as directory_name:
        ...
        require(sum(not row["source_object_placement_is_identity"] for row in extraction["objects"]) == 7, "selected nonidentity source placement count drift")
        for spec in specs:
            ...
            require(receipt_row["host_link"] == "link2" and receipt_row["kind"] == spec["kind"], f"receipt mapping drift: {object_id}")
            require(mass_row["host_link"] == "link2" and mass_row["kind"] == spec["kind"], f"mass mapping drift: {object_id}")
            ...
            local_shape = transform_shape(source_shape, t_l2_a0_mm)
            local_metric = validate_single_solid(local_shape, context=f"{object_id} link2 local")
            recovered_shape = transform_shape(local_shape, t_a0_l2_0_mm)
            ...
            require(step_volume_residual <= contract["tolerances"]["step_reopen_volume_relative"], f"STEP volume drift: {object_id}: {step_volume_residual}")
            ...
            metadata = {
                "schema": "ROUTE_C_LINK2_LOCAL_RUNTIME_MESH_V1",
                "object_id": object_id,
                "frame": "link2",
                "joint_state": "q1=q2=0",
                "length_unit": "m",
            ...
            local_cg_mm = transform_points(np.asarray([mass_row["cg_mm_A0"]]), t_l2_a0_mm)[0]
            geometry_rows.append({
                "object_id": object_id,
                "source_label": spec["source_label"],
                "source_internal_name": spec["source_internal_name"],
                "registry_parent_frame": "link2",
                ...
                "source_object_placement_is_identity": source_row["source_object_placement_is_identity"],
                ...
                "output_frame": "link2",
                "output_joint_state": "q1=q2=0",
                "output_unit": "mm",
                "transform": "inv(T_A0_link2(0,0))_from_accepted_URDF_raw_numbers",
                ...
                "output_cg_mm_link2": local_cg_mm.tolist(),
    ...
    q0_matrix_closure = float(np.max(np.abs(pose_matrix_S_link2(0.0, 0.0, joints, mount) @ np.linalg.inv(t_a0_l2_0_m) - mount)))
    ...
    require([(row["q1_rad"], row["q2_rad"]) for row in ledger["pose_samples"]] == expected_pose_grid, "nine-point q1/q2 Cartesian pose grid drift")
    sample_rows = []
    for sample in ledger["pose_samples"]:
        q1 = float(sample["q1_rad"])
        q2 = float(sample["q2_rad"])
        matrix = pose_matrix_S_link2(q1, q2, joints, mount)
        sample_rows.append({
            "sample": sample["sample"],
            "q1_rad_raw": sample["q1_rad"],
            "q2_rad_raw": sample["q2_rad"],
            "q3_to_q6_rad": [0, 0, 0, 0],
            "T_S_link2_m": [[float(value) for value in row] for row in matrix],
    ...
    pose_document = {
        "schema": "ROUTE_C_LINK2_B12_POSE_ADAPTER_SAMPLES_V1",
        ...
        "joint_chain_raw": {name: raw_joint(joint) for name, joint in sorted(joints.items())},
        "q0_source_to_link2_to_S_matrix_closure_max_abs": q0_matrix_closure,
        "sample_count": len(sample_rows),
        "sample_grid": ledger["pose_sample_grid"],
        "samples": sample_rows,
        "side_effect_free": True,
        "system_authority_changed": False,
        "verdict": "JOINT1_JOINT2_ACCEPTED_URDF_RAW_FK_AND_12DP_EXECUTION_MOUNT_ADAPTER_SAMPLES_PASS",
    }
    pose_bytes = stable_json_bytes(pose_document)
    artifacts[POSE_RECEIPT] = pose_bytes
    geometry_document = {
        "schema": "ROUTE_C_LINK2_B12_GEOMETRY_INDEX_V1",
        "generated_utc": "DETERMINISTIC_BUILD_NO_WALLCLOCK",
        "scope": "R95_NEXT_SAFE_BATCH_LINK2_B12",
        ...
        "verdict": "LINK2_B12_STEP_FIRST_HOST_LOCAL_GEOMETRY_BUILT__LOCAL_CANDIDATE_ONLY",
    }
    geometry_bytes = stable_json_bytes(geometry_document)
    artifacts[GEOMETRY_INDEX] = geometry_bytes
    runtime_document = {
        "schema": "ROUTE_C_LINK2_B12_RUNTIME_SIDECAR_RECEIPT_V1",
        "generated_utc": "DETERMINISTIC_BUILD_NO_WALLCLOCK",
        ...
        "verdict": "12_OF_12_RUNTIME_SIDECAR_SETS_DERIVED_FROM_REOPENED_LINK2_LOCAL_STEP_IN_METRES",
    }
    runtime_bytes = stable_json_bytes(runtime_document)
    artifacts[RUNTIME_RECEIPT] = runtime_bytes
    build_document = {
        "schema": "ROUTE_C_LINK2_B12_TWELVE_STEP_BUILD_RECEIPT_V1",
        "generated_utc": "DETERMINISTIC_BUILD_NO_WALLCLOCK",
        "scope": "R95_NEXT_SAFE_BATCH_LINK2_B12",
        "contract_files": {
            ...
            "link2_b12_contract": byte_record(CONTRACT, CONTRACT.read_bytes()),
        },
        ...
        "verdict": "LINK2_B12_PRIMARY_STEP_RUNTIME_AND_POSE_ADAPTER_BUILD_PASS__SYSTEM_AUTHORITY_UNCHANGED",
    }
    build_bytes = stable_json_bytes(build_document)
    artifacts[BUILD_RECEIPT] = build_bytes
    return artifacts, {"objects": 12, "steps": 12, "runtime_sidecars": 36, "artifacts": len(artifacts), "verdict": build_document["verdict"]}
