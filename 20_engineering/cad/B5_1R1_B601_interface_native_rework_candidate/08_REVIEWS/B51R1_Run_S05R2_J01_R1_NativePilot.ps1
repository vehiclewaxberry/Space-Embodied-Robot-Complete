...
$sessionRoot = Join-Path $root '07_VERIFICATION\AUTONOMOUS\S05R2_J01_R1'
...
$toolSource = Join-Path $PSScriptRoot 'B51R1_S05R2_J01_R1_NativePilotTool.cs'
$toolDll = Join-Path $PSScriptRoot 'B51R1_S05R2_J01_R1_NativePilotTool.dll'
$toolSourceBytes = 78738
$toolSourceSha256 = '3D64AD6ED6F51274FDD435A915789F1BDABA7528F10727D90ED4C0B717350FFC'
$toolDllBytes = 52736
$toolDllSha256 = '6174AC9BA6740EEF555CAFB9784C460B2051BEC6FBCC948AB6C4DA0E5F73D768'
...
$j00Assembly = Join-Path $checkpointDir 'B51R1_CARRIER_J00_ROOT_PILOT_R1.SLDASM'
$pilotAssembly = Join-Path $pilotDir 'B51R1_CARRIER_J01_R1_NATIVE_PILOT.SLDASM'
$j00Checkpoint = Join-Path $sessionRoot 'B51R1_S05R2_J01_R1_J00_BASELINE_CHECKPOINT.json'
$createReceipt = Join-Path $sessionRoot 'B51R1_S05R2_J01_R1_CREATE_RECEIPT.json'
$createProgress = Join-Path $sessionRoot 'B51R1_S05R2_J01_R1_CREATE_PROGRESS.log'
$createGate = Join-Path $sessionRoot 'B51R1_S05R2_J01_R1_CREATE_GATE.json'
$coldReceipt = Join-Path $sessionRoot 'B51R1_S05R2_J01_R1_COLD_REOPEN_RECEIPT.json'
$coldProgress = Join-Path $sessionRoot 'B51R1_S05R2_J01_R1_COLD_REOPEN_PROGRESS.log'
$finalGate = Join-Path $sessionRoot 'B51R1_S05R2_J01_R1_NATIVE_1R_GATE.json'
...
Assert-File $toolCompiler $toolCompilerBytes $toolCompilerSha256 'J01 R1 frozen-tool compiler provenance'
...
$readinessReceipt = Join-Path $sessionRoot "B51R1_S05R2_J01_R1_$($Mode.ToUpperInvariant())_READINESS_RECEIPT.json"
$inputLock = Join-Path $sessionRoot "B51R1_S05R2_J01_R1_$($Mode.ToUpperInvariant())_INPUT_LOCK.json"
...
if ($Mode -eq 'ColdVerify') {
    ...
    if ($created.schema -ne 'B51R1_S05R2_J01_R1_NATIVE_1R_CREATE_GATE_V1' -or
        $created.status -ne 'S05R2_J01_R1_NATIVE_1R_PILOT_CREATE_PASS' -or
        $created.gate_pass -isnot [bool] -or $created.gate_pass -ne $true) {
        throw 'J01 R1 Create Gate is not PASS'
    }
    if ($created.cold_reopen_authorized -isnot [bool] -or $created.cold_reopen_authorized -ne $true) {
        throw 'J01 R1 Create Gate does not authorize cold reopen'
    }
    ...
    if ($createdReceipt.schema -ne 'B51R1_S05R2_J01_R1_NATIVE_PILOT_CREATE_V1' -or
        $createdReceipt.status -ne 'S05R2_J01_R1_NATIVE_1R_PILOT_CREATE_PASS' -or
        $createdReceipt.gate_pass -isnot [bool] -or $createdReceipt.gate_pass -ne $true) {
        throw 'J01 R1 Create receipt is not PASS'
    }
    ...
    $coldLockedArtifacts = @(
        [ordered]@{id='J01_R1_CREATE_GATE';path=$createGate;bytes=(Get-Item $createGate).Length;sha256=Get-Sha $createGate},
        [ordered]@{id='J01_R1_CREATE_RECEIPT';path=$createReceipt;bytes=[long]$created.receipt.bytes;sha256=[string]$created.receipt.sha256},
        [ordered]@{id='J01_R1_PILOT_ASSEMBLY';path=$pilotAssembly;bytes=[long]$created.output.bytes;sha256=[string]$created.output.sha256}
    )
}
...
try {
    ...
    $lock = [ordered]@{
        schema="B51R1_S05R2_J01_R1_$($Mode.ToUpperInvariant())_INPUT_LOCK_V1"
        generated_at=[DateTimeOffset]::Now.ToString('o')
        status="PASS_S05R2_J01_R1_$($Mode.ToUpperInvariant())_INPUTS_HASH_LOCKED"
        mode=$Mode
        ...
        claim_limit='J01_R1_NATIVE_1R_PILOT_ONLY_NO_FORMAL_S05_G4_T005_OR_FLIGHT_CREDIT'
    }
    Write-JsonCreateNew $inputLock $lock

    [void][Reflection.Assembly]::LoadFile((Resolve-Path -LiteralPath $toolDll).Path)
    if ($Mode -eq 'Create') {
        $result = [B51R1S05R2J01R1NativePilotTool]::Create(
        ...
        if ($data.status -ne 'S05R2_J01_R1_NATIVE_1R_PILOT_CREATE_PASS' -or $data.gate_pass -ne $true) {
            throw 'J01 R1 Create receipt is not PASS'
        }
        ...
        $gate = [ordered]@{
            schema='B51R1_S05R2_J01_R1_NATIVE_1R_CREATE_GATE_V1';generated_at=[DateTimeOffset]::Now.ToString('o')
            status='S05R2_J01_R1_NATIVE_1R_PILOT_CREATE_PASS';gate_pass=$true
            ...
            checks=[ordered]@{j00_three_root_plane_mates=$true;automatic_fix_removed=$true;j01_native_hinge_angle_selection_false=$true;j01_limit_angle_reference_axis_bound=$true;native_create_mate_data_readback_pass=$true;unique_native_limit_angle_driver=$true;native_limits_0_to_5p6_rad=$true;remaining_dof_exactly_one_rotation=$true;canonical_plus_minus_one_degree_pass=$true;enhanced_branch_and_endpoint_sweep_pass=$true;ten_q0_reset_cycles_pass=$true;returned_to_q0=$true;transform2_call_count=0;move_component_call_count=0;set_transform_and_solve_call_count=0}
            cold_reopen_authorized=$true;formal_s05_authorized=$false;t005_authorized=$false
            claim_limit='J01_R1_CREATE_AND_SAME_DOCUMENT_DRIVER_EVIDENCE_ONLY_PENDING_INDEPENDENT_COLD_REOPEN'
        }
        Write-JsonCreateNew $createGate $gate
        Write-Output $createGate
    }
    else {
        $result = [B51R1S05R2J01R1NativePilotTool]::ColdVerify(
        ...
        if ($data.status -ne 'S05R2_J01_R1_NATIVE_1R_PILOT_COLD_REOPEN_PASS' -or $data.gate_pass -ne $true) {
            throw 'J01 R1 ColdVerify receipt is not PASS'
        }
        if ($data.transform2_call_count -ne 0 -or $data.set_transform_and_solve_call_count -ne 0 -or
        ...
        $gate = [ordered]@{
            schema='B51R1_S05R2_J01_R1_NATIVE_1R_GATE_V1';generated_at=[DateTimeOffset]::Now.ToString('o')
            status='S05R2_J01_R1_NATIVE_1R_PILOT_PASS';gate_pass=$true
            ...
            claim_limit='J01_R1_NATIVE_1R_PILOT_GATE_ONLY_NO_COMPLETE_CHAIN_G4_T005_STRUCTURAL_MANUFACTURING_OR_FLIGHT_CREDIT'
        }
