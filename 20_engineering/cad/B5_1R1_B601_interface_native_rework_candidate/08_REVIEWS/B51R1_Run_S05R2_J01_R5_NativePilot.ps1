...
$sessionRoot = Join-Path $root '07_VERIFICATION\AUTONOMOUS\S05R2_J01_R5'
...
$toolSource = Join-Path $PSScriptRoot 'B51R1_S05R2_J01_R5_NativePilotTool.cs'
$toolDll = Join-Path $PSScriptRoot 'B51R1_S05R2_J01_R5_NativePilotTool.dll'
$toolSourceBytes = 108195
$toolSourceSha256 = '20D402C3F8D285F886FCB16166AF301027B55EFF6A6E5054DF91610879CAEEB8'
$toolDllBytes = 70144
$toolDllSha256 = '43ABBFABBC662503384C21FB08FCD6647B0F0C9EA7AE6D92818BFD1F4BE49FFA'
...
$j00Assembly = Join-Path $checkpointDir 'B51R1_CARRIER_J00_ROOT_PILOT_R5.SLDASM'
$pilotAssembly = Join-Path $pilotDir 'B51R1_CARRIER_J01_R5_NATIVE_PILOT.SLDASM'
$j00Checkpoint = Join-Path $sessionRoot 'B51R1_S05R2_J01_R5_J00_BASELINE_CHECKPOINT.json'
$createReceipt = Join-Path $sessionRoot 'B51R1_S05R2_J01_R5_CREATE_RECEIPT.json'
$createProgress = Join-Path $sessionRoot 'B51R1_S05R2_J01_R5_CREATE_PROGRESS.log'
$createGate = Join-Path $sessionRoot 'B51R1_S05R2_J01_R5_CREATE_GATE.json'
$coldReceipt = Join-Path $sessionRoot 'B51R1_S05R2_J01_R5_COLD_REOPEN_RECEIPT.json'
$coldProgress = Join-Path $sessionRoot 'B51R1_S05R2_J01_R5_COLD_REOPEN_PROGRESS.log'
$finalGate = Join-Path $sessionRoot 'B51R1_S05R2_J01_R5_NATIVE_1R_GATE.json'
...
function Get-RecoveryPrerequisites {
    Assert-File $hp01DirectReceipt $hp01DirectReceiptBytes $hp01DirectReceiptSha256 'J01 R5 direct-open health receipt'
    ...
    if ($direct.schema -ne 'B51R1_S05R2_R1_DRIVER_READY_COLD_REOPEN_V3' -or
        ...
        throw 'J01 R5 direct-open health receipt contract mismatch'
    }
    $directParts = @($direct.parts)
    if ($directParts.Count -ne 2 -or @($directParts.link | Select-Object -Unique).Count -ne 2) {
        throw 'J01 R5 direct-open health receipt does not bind two unique parts'
    }
    foreach ($link in @('base_link','link1')) {
        $part = @($directParts | Where-Object { $_.link -eq $link })
        if ($part.Count -ne 1) { throw "J01 R5 direct-open part is not unique: $link" }
        $expectedPath = if ($link -eq 'base_link') { $baseCarrier } else { $link1Carrier }
        $expectedBytes = if ($link -eq 'base_link') { $baseCarrierBytes } else { $link1CarrierBytes }
        $expectedSha = if ($link -eq 'base_link') { $baseCarrierSha256 } else { $link1CarrierSha256 }
        if ([IO.Path]::GetFullPath([string]$part[0].path) -ne [IO.Path]::GetFullPath($expectedPath) -or
            ...
            throw "J01 R5 direct-open health binding mismatch: $link"
        }
    }
...
Assert-File $toolCompiler $toolCompilerBytes $toolCompilerSha256 'J01 R5 frozen-tool compiler provenance'
if ($toolDllBytes -le 0 -or $toolDllSha256 -eq ('0' * 64)) {
    throw 'J01 R5 is source-only: deterministic compile and append-only binary static freeze are required before execution'
}
...
$readinessReceipt = Join-Path $sessionRoot "B51R1_S05R2_J01_R5_$($Mode.ToUpperInvariant())_READINESS_RECEIPT.json"
$inputLock = Join-Path $sessionRoot "B51R1_S05R2_J01_R5_$($Mode.ToUpperInvariant())_INPUT_LOCK.json"
...
if ($Mode -eq 'ColdVerify') {
    ...
    if ($created.schema -ne 'B51R1_S05R2_J01_R5_NATIVE_1R_CREATE_GATE_V1' -or
        $created.status -ne 'S05R2_J01_R5_NATIVE_1R_PILOT_CREATE_PASS' -or
        $created.gate_pass -isnot [bool] -or $created.gate_pass -ne $true) {
        throw 'J01 R5 Create Gate is not PASS'
    }
    if ($created.cold_reopen_authorized -isnot [bool] -or $created.cold_reopen_authorized -ne $true) {
        throw 'J01 R5 Create Gate does not authorize cold reopen'
    }
    ...
    if ($createdReceipt.schema -ne 'B51R1_S05R2_J01_R5_NATIVE_PILOT_CREATE_V1' -or
        $createdReceipt.status -ne 'S05R2_J01_R5_NATIVE_1R_PILOT_CREATE_PASS' -or
        $createdReceipt.gate_pass -isnot [bool] -or $createdReceipt.gate_pass -ne $true) {
        throw 'J01 R5 Create receipt is not PASS'
    }
    ...
    $coldLockedArtifacts = @(
        [ordered]@{id='J01_R5_CREATE_GATE';path=$createGate;bytes=(Get-Item $createGate).Length;sha256=Get-Sha $createGate},
        [ordered]@{id='J01_R5_CREATE_RECEIPT';path=$createReceipt;bytes=[long]$created.receipt.bytes;sha256=[string]$created.receipt.sha256},
        [ordered]@{id='J01_R5_PILOT_ASSEMBLY';path=$pilotAssembly;bytes=[long]$created.output.bytes;sha256=[string]$created.output.sha256}
    )
}
...
try {
    ...
    $lock = [ordered]@{
        schema="B51R1_S05R2_J01_R5_$($Mode.ToUpperInvariant())_INPUT_LOCK_V1"
        generated_at=[DateTimeOffset]::Now.ToString('o')
        status="PASS_S05R2_J01_R5_$($Mode.ToUpperInvariant())_INPUTS_HASH_LOCKED"
        mode=$Mode
        ...
        claim_limit='J01_R5_TYPED_MATE_ENTITY_RECOVERY_NATIVE_1R_PILOT_ONLY_NO_FORMAL_S05_G4_T005_OR_FLIGHT_CREDIT'
    }
    Write-JsonCreateNew $inputLock $lock

    [void][Reflection.Assembly]::LoadFile((Resolve-Path -LiteralPath $toolDll).Path)
    if ($Mode -eq 'Create') {
        $result = [B51R1S05R2J01R5NativePilotTool]::Create(
        ...
        if ($data.schema -ne 'B51R1_S05R2_J01_R5_NATIVE_PILOT_CREATE_V1' -or
            $data.status -ne 'S05R2_J01_R5_NATIVE_1R_PILOT_CREATE_PASS' -or
        ...
        if ($mateAttempts.Count -ne 5 -or @($data.native_mate_creation).Count -ne 5 -or
            ...
            throw 'J01 R5 Mate API call ledger count mismatch'
        }
        ...
        foreach ($index in 0..4) {
            ...
            if ([long]$attempt.create_mate_data_call_index -ne ($index + 1) -or
                ...
                [long]$attempt.selection_count_at_create_mate_return -ne $expectedMateSelectionCounts[$index] -or
                [long]$attempt.selection_count_after_post_create_clear -ne 0) {
                throw "J01 R5 Mate API call ledger mismatch at index $index"
            }
        }
        ...
        foreach ($index in 0..12) {
            ...
            if ([long]$selection.select_by_id2_call_index -ne ($index + 1) -or
                ...
                throw "J01 R5 selection identity/mark ledger mismatch at index $index"
            }
            $isAssemblyRoot = $index -in @(0,2,4)
            if ($isAssemblyRoot) {
                if ($null -ne $selection.selected_owner_component -or $null -ne $selection.selected_owner_path) {
                    throw "J01 R5 assembly-root selection unexpectedly has a component owner at index $index"
                }
            }
            else {
                $expectedOwnerPath = if ($index -in @(1,3,5,6,8,10,12)) { $baseCarrier } else { $link1Carrier }
                if ([string]::IsNullOrWhiteSpace([string]$selection.selected_owner_component) -or
                    [IO.Path]::GetFullPath([string]$selection.selected_owner_path) -ne [IO.Path]::GetFullPath($expectedOwnerPath)) {
                    throw "J01 R5 selected owner binding mismatch at index $index"
                }
            }
        }
        ...
        if ($preloads.Count -ne 2 -or [long]$data.document_count_before_new_assembly -ne 2 -or
            ...
            throw 'J01 R5 preload count or assembly document-count transition mismatch'
        }
        foreach ($index in 0..1) {
            ...
            if ([string]$preload.role -ne $expectedRole -or
                ...
                throw "J01 R5 strict preload evidence mismatch: $expectedRole"
            }
        }
        ...
        $gate = [ordered]@{
            schema='B51R1_S05R2_J01_R5_NATIVE_1R_CREATE_GATE_V1';generated_at=[DateTimeOffset]::Now.ToString('o')
            status='S05R2_J01_R5_NATIVE_1R_PILOT_CREATE_PASS';gate_pass=$true
            ...
            claim_limit='J01_R5_SELECTIONMGR_BOUND_TYPED_MATE_ENTITY_RECOVERY_CREATE_AND_SAME_DOCUMENT_DRIVER_EVIDENCE_ONLY_PENDING_INDEPENDENT_COLD_REOPEN'
        }
        Write-JsonCreateNew $createGate $gate
        Write-Output $createGate
    }
    else {
        $result = [B51R1S05R2J01R5NativePilotTool]::ColdVerify(
        ...
        if ($data.schema -ne 'B51R1_S05R2_J01_R5_NATIVE_PILOT_COLD_REOPEN_V1' -or
            $data.status -ne 'S05R2_J01_R5_NATIVE_1R_PILOT_COLD_REOPEN_PASS' -or
            $data.gate_pass -isnot [bool] -or $data.gate_pass -ne $true) {
            throw 'J01 R5 ColdVerify receipt is not PASS'
        }
        if ($data.transform2_call_count -ne 0 -or $data.set_transform_and_solve_call_count -ne 0 -or
            ...
            $data.get_components_call_count -ne 1 -or $data.open_errors -ne 0 -or $data.open_warnings -ne 0) {
        ...
        if ($resolution.Count -lt 2 -or $expectedIdentities.Count -ne 2 -or
            ...
            throw 'J01 R5 ColdVerify did not resolve parent and child uniquely in one component traversal'
        }
        foreach ($role in @('parent','child')) {
            ...
            if ($identity.Count -ne 1 -or $match.Count -ne 1 -or
                ...
                throw "J01 R5 ColdVerify component identity mismatch: $role"
            }
        }
        foreach ($item in $lockedItems) {
        ...
        $gate = [ordered]@{
            schema='B51R1_S05R2_J01_R5_NATIVE_1R_GATE_V1';generated_at=[DateTimeOffset]::Now.ToString('o')
            status='S05R2_J01_R5_NATIVE_1R_PILOT_PASS';gate_pass=$true
