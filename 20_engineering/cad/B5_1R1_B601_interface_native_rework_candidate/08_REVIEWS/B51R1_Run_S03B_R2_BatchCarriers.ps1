...
$sessionRoot = Join-Path $root '07_VERIFICATION\AUTONOMOUS\S03B_R2'
...
$readinessReceipt = Join-Path $sessionRoot 'B51R1_S03B_R2_READINESS_RECEIPT.json'
$inputLock = Join-Path $sessionRoot 'B51R1_S03B_R2_INPUT_LOCK.json'
$receipt = Join-Path $sessionRoot 'B51R1_S03B_R2_BATCH_CARRIER_CREATE_RECEIPT.json'
$progress = Join-Path $sessionRoot 'B51R1_S03B_R2_PROGRESS.log'
$gate = Join-Path $sessionRoot 'B51R1_S03B_R2_REMAINING_NINE_CARRIER_GATE.json'
$sessionReceipt = Join-Path $sessionRoot 'B51R1_S03B_R2_SESSION_RECEIPT.json'
...
$lockedItems = @(
    ...
    [ordered]@{id='PRESERVED_S03B_R1_FAILED_RECEIPT';path='07_VERIFICATION/AUTONOMOUS/S03B_R1/B51R1_S03B_R1_BATCH_CARRIER_CREATE_RECEIPT.json';bytes=896;sha256='CAF141D042B6BB85EC3B3300F0E71ECC703C7072E83DFD7CE65C2AACC380D6D8'},
    [ordered]@{id='PRESERVED_S03B_R1_FAILED_SESSION';path='07_VERIFICATION/AUTONOMOUS/S03B_R1/B51R1_S03B_R1_SESSION_RECEIPT.json';bytes=589;sha256='53C41173181AFDDE030BD6AA82CA8A4331D86492FCB8A8253924DAEED71508A7'},
    [ordered]@{id='PRESERVED_S03B_R1_INPUT_LOCK';path='07_VERIFICATION/AUTONOMOUS/S03B_R1/B51R1_S03B_R1_INPUT_LOCK.json';bytes=8389;sha256='85CD067239CA04EC28FA033D7C77CCDD17288B199F867A7D5417FA2D96178C7A'},
...
$session = [ordered]@{
    schema='B51R1_S03B_R2_SESSION_RECEIPT_V1'; session_id='S03B_R2'
    started_at=[DateTimeOffset]::Now.ToString('o'); status='S03B_R2_FAIL_CLOSED_NOT_STARTED'
...
try {
    ...
    $runtimeItems = @($lockedItems) + @([ordered]@{id='READINESS_RECEIPT';path='07_VERIFICATION/AUTONOMOUS/S03B_R2/B51R1_S03B_R2_READINESS_RECEIPT.json';bytes=(Get-Item $readinessReceipt).Length;sha256=Get-Sha $readinessReceipt})
    $lock = [ordered]@{
        schema='B51R1_S03B_R2_INPUT_LOCK_V1'; generated_at=[DateTimeOffset]::Now.ToString('o')
        status='PASS_S03B_R2_INPUTS_HASH_LOCKED'; session_id='S03B_R2'
    ...
    $result = [B51R1S03BS04BBatchCarrierToolV3]::Create($launchedPid, $template, $carrierDir, $receipt, $progress)
    ...
    $gateData = [ordered]@{
        schema='B51R1_S03B_R2_REMAINING_NINE_CARRIER_GATE_V1';generated_at=[DateTimeOffset]::Now.ToString('o')
        status='S03B_REMAINING_NINE_CARRIERS_CREATED';created_part_count=9
        creation_receipt=[ordered]@{path='07_VERIFICATION/AUTONOMOUS/S03B_R2/B51R1_S03B_R2_BATCH_CARRIER_CREATE_RECEIPT.json';bytes=(Get-Item $receipt).Length;sha256=Get-Sha $receipt}
        parts=$partEvidence
...
activation_gate: 07_VERIFICATION/AUTONOMOUS/S03B_R2/B51R1_S03B_R2_REMAINING_NINE_CARRIER_GATE.json
activated_slot:
  id: A07_S04B
  carrier_count: 10
...
"@
    ...
    $session['status']='PASS_S03B_R2_REMAINING_NINE_CARRIERS_CREATED'
    ...
    $session['status']='S03B_R2_FAIL_CLOSED'
