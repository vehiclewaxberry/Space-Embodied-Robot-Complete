...
$sessionRoot = Join-Path $root '07_VERIFICATION\AUTONOMOUS\S05R2_J01_R15'
...
$toolSource = Join-Path $PSScriptRoot 'B51R1_S05R2_J01_R15_NativePilotTool.cs'
$toolDll = Join-Path $PSScriptRoot 'B51R1_S05R2_J01_R15_NativePilotTool.dll'
$toolRunner = Join-Path $PSScriptRoot 'B51R1_Run_S05R2_J01_R15_NativePilot.ps1'
$toolSourceBytes = 136319
$toolSourceSha256 = 'B49A34BA14172EC596301DFA6683AD7DC81765B01692A0D6225B7FE3F643764C'
$toolDllBytes = 85504
$toolDllSha256 = '834EA37333873840221853FC8F98FB37FD73606C737648BC262D63A0478E7639'
...
$r15StaticRelease = Join-Path $PSScriptRoot 'B51R1_S05R2_J01_R15_NATIVE_PILOT_TOOL_STATIC_RELEASE_V1.json'
$r15CreateActivation = Join-Path $root '04_CONFIGURATION\B51R1_AUTONOMOUS_SESSION_PLAN_V22R23A_POOL_A_A08R2_J01_R15_SELECTION_LEDGER_NATIVE_1R_PILOT_CREATE_ACTIVATION.json'
$r15ColdActivation = Join-Path $root '04_CONFIGURATION\B51R1_AUTONOMOUS_SESSION_PLAN_V22R24A_POOL_A_A08R2_J01_R15_SELECTION_LEDGER_NATIVE_1R_PILOT_COLD_VERIFY_ACTIVATION.json'
...
$j00Assembly = Join-Path $checkpointDir 'B51R1_CARRIER_J00_ROOT_PILOT_R15.SLDASM'
$pilotAssembly = Join-Path $pilotDir 'B51R1_CARRIER_J01_R15_NATIVE_PILOT.SLDASM'
$j00Checkpoint = Join-Path $sessionRoot 'B51R1_S05R2_J01_R15_J00_BASELINE_CHECKPOINT.json'
$createReceipt = Join-Path $sessionRoot 'B51R1_S05R2_J01_R15_CREATE_RECEIPT.json'
$createProgress = Join-Path $sessionRoot 'B51R1_S05R2_J01_R15_CREATE_PROGRESS.log'
$createGate = Join-Path $sessionRoot 'B51R1_S05R2_J01_R15_CREATE_GATE.json'
$coldReceipt = Join-Path $sessionRoot 'B51R1_S05R2_J01_R15_COLD_REOPEN_RECEIPT.json'
$coldProgress = Join-Path $sessionRoot 'B51R1_S05R2_J01_R15_COLD_REOPEN_PROGRESS.log'
$finalGate = Join-Path $sessionRoot 'B51R1_S05R2_J01_R15_NATIVE_1R_GATE.json'
...
function Get-ModeActivation([string]$RequestedMode) {
    $activationPath = if ($RequestedMode -eq 'Create') { $r15CreateActivation } else { $r15ColdActivation }
    if (-not (Test-Path -LiteralPath $activationPath -PathType Leaf)) {
        throw "J01 R15 $RequestedMode has no independent activation: $activationPath"
    }
    ...
    $expectedSchema = if ($RequestedMode -eq 'Create') {
        'B51R1_AUTONOMOUS_SESSION_PLAN_V22R23A_POOL_A_A08R2_J01_R15_SELECTION_LEDGER_NATIVE_1R_PILOT_CREATE_ACTIVATION_V1'
    } else {
        'B51R1_AUTONOMOUS_SESSION_PLAN_V22R24A_POOL_A_A08R2_J01_R15_SELECTION_LEDGER_NATIVE_1R_PILOT_COLD_VERIFY_ACTIVATION_V1'
    }
    $expectedStatus = if ($RequestedMode -eq 'Create') {
        'POOL_A_A08R2_J01_R15_SELECTION_LEDGER_NATIVE_1R_PILOT_CREATE_ACTIVATED'
    } else {
        'POOL_A_A08R2_J01_R15_SELECTION_LEDGER_NATIVE_1R_PILOT_COLD_VERIFY_ACTIVATED'
    }
    ...
    if ($activation.schema -ne $expectedSchema -or $activation.status -ne $expectedStatus -or
        ...
        [IO.Path]::GetFullPath([string]$activation.static_release.path) -ne [IO.Path]::GetFullPath($r15StaticRelease)) {
        throw "J01 R15 $RequestedMode activation contract mismatch"
    }
    Assert-File $r15StaticRelease ([long]$activation.static_release.bytes) ([string]$activation.static_release.sha256) 'J01 R15 static release bound by activation'
    $release = Get-Content -Raw -Encoding UTF8 -LiteralPath $r15StaticRelease | ConvertFrom-Json
    if ($release.schema -ne 'B51R1_S05R2_J01_R15_NATIVE_PILOT_TOOL_STATIC_RELEASE_V1' -or
        $release.status -ne 'STATIC_RELEASE_PASS' -or $release.gate_pass -ne $true -or
        $release.runtime_executed -ne $false -or [long]$release.recovery_prerequisite_count -ne 15 -or
        ...
        [long]$release.runner.r14_transaction_closure_artifact_count -ne 10 -or
        [long]$release.runner.create_locked_item_count -ne 87 -or
        [long]$release.runner.cold_verify_locked_item_count -ne 90 -or
        ...
        throw 'J01 R15 static release semantic contract mismatch'
    }
    if ([IO.Path]::GetFullPath([string]$activation.tool.source.path) -ne [IO.Path]::GetFullPath($toolSource) -or
        ...
        throw "J01 R15 $RequestedMode activation tool binding mismatch"
    }
    if ($RequestedMode -eq 'Create') {
        if ($activation.create_authorized -ne $true -or $activation.cold_verify_authorized -ne $false -or
            ...
            [long]$activation.required_counts.recovery_prerequisites -ne 15 -or
            [long]$activation.required_counts.predecessor_transaction_closure_artifacts -ne 10 -or
            [long]$activation.required_counts.create_locked_items -ne 87 -or
            [IO.Path]::GetFullPath([string]$activation.predecessor_failure.path) -ne [IO.Path]::GetFullPath($r14FailureReceipt) -or
            [long]$activation.predecessor_failure.bytes -ne $r14FailureReceiptBytes -or
            [string]$activation.predecessor_failure.sha256 -ne $r14FailureReceiptSha256 -or
            [IO.Path]::GetFullPath([string]$activation.predecessor_failure.partial_j00_snapshot.path) -ne [IO.Path]::GetFullPath($r14PartialJ00Assembly) -or
            [long]$activation.predecessor_failure.partial_j00_snapshot.bytes -ne $r14PartialJ00AssemblyBytes -or
            [string]$activation.predecessor_failure.partial_j00_snapshot.sha256 -ne $r14PartialJ00AssemblySha256) {
            throw 'J01 R15 Create activation scope/predecessor binding mismatch'
        }
    } else {
        if ($activation.create_authorized -ne $false -or $activation.cold_verify_authorized -ne $true -or
            ...
            throw 'J01 R15 ColdVerify activation scope/path binding mismatch'
        }
        Assert-File $createGate ([long]$activation.create_gate.bytes) ([string]$activation.create_gate.sha256) 'J01 R15 Cold activation Create Gate'
        Assert-File $createReceipt ([long]$activation.create_receipt.bytes) ([string]$activation.create_receipt.sha256) 'J01 R15 Cold activation Create receipt'
        Assert-File $pilotAssembly ([long]$activation.pilot_output.bytes) ([string]$activation.pilot_output.sha256) 'J01 R15 Cold activation pilot output'
    }
    if ($activation.formal_s05_authorized -ne $false -or $activation.t005_authorized -ne $false) {
        throw "J01 R15 $RequestedMode activation exceeds Pilot scope"
    }
    return [ordered]@{
        id="J01_R15_$($RequestedMode.ToUpperInvariant())_ACTIVATION"
...
function Get-RecoveryPrerequisites {
    Assert-File $hp01DirectReceipt $hp01DirectReceiptBytes $hp01DirectReceiptSha256 'J01 R15 direct-open health receipt'
    ...
    if ($direct.schema -ne 'B51R1_S05R2_R1_DRIVER_READY_COLD_REOPEN_V3' -or
        ...
        throw 'J01 R15 direct-open health receipt contract mismatch'
    }
    $directParts = @($direct.parts)
    if ($directParts.Count -ne 2 -or @($directParts.link | Select-Object -Unique).Count -ne 2) {
        throw 'J01 R15 direct-open health receipt does not bind two unique parts'
    }
    foreach ($link in @('base_link','link1')) {
        $part = @($directParts | Where-Object { $_.link -eq $link })
        if ($part.Count -ne 1) { throw "J01 R15 direct-open part is not unique: $link" }
        $expectedPath = if ($link -eq 'base_link') { $baseCarrier } else { $link1Carrier }
        $expectedBytes = if ($link -eq 'base_link') { $baseCarrierBytes } else { $link1CarrierBytes }
        $expectedSha = if ($link -eq 'base_link') { $baseCarrierSha256 } else { $link1CarrierSha256 }
        if ([IO.Path]::GetFullPath([string]$part[0].path) -ne [IO.Path]::GetFullPath($expectedPath) -or
            ...
            throw "J01 R15 direct-open health binding mismatch: $link"
        }
    }
    ...
    # closure before permitting this R15 Create attempt.
    ...
    # Freeze its receipt plus the remaining nine transaction artifacts before R15.
    ...
    # Freeze the receipt plus its remaining nine transaction artifacts before R15.
    ...
    if (@($r14Attempts | Where-Object { $_.feature_returned -eq $true -and [long]$_.create_mate_error_status -eq 1 }).Count -ne 4 -or
...
Assert-File $toolCompiler $toolCompilerBytes $toolCompilerSha256 'J01 R15 frozen-tool compiler provenance'
if ($toolDllBytes -le 0 -or $toolDllSha256 -eq ('0' * 64)) {
    throw 'J01 R15 is source-only: deterministic compile and append-only binary static freeze are required before execution'
}
...
if ($persistentSoftwareOglBefore -ne 0) {
    throw 'Persistent Use Software OGL must remain disabled; R15 authorizes only the one-session launch argument'
}

$mr2Inputs = @(Get-Mr2PilotInputs)
$recoveryPrerequisites = @(Get-RecoveryPrerequisites)
if ($recoveryPrerequisites.Count -ne 15) {
    throw "J01 R15 recovery prerequisite count mismatch: $($recoveryPrerequisites.Count)"
}
...
if ($prelaunchSolidWorksCount -ne 0 -or $r9PredecessorPidPresent -or $r10PredecessorPidPresent -or
    ...
    throw 'J01 R15 predecessor process boundary is not clean immediately before launch'
}
$mr2LockedArtifacts = @(
...
$readinessReceipt = Join-Path $sessionRoot "B51R1_S05R2_J01_R15_$($Mode.ToUpperInvariant())_READINESS_RECEIPT.json"
$inputLock = Join-Path $sessionRoot "B51R1_S05R2_J01_R15_$($Mode.ToUpperInvariant())_INPUT_LOCK.json"
...
if ($Mode -eq 'ColdVerify') {
    ...
    if ($created.schema -ne 'B51R1_S05R2_J01_R15_NATIVE_1R_CREATE_GATE_V1' -or
        $created.status -ne 'S05R2_J01_R15_NATIVE_1R_PILOT_CREATE_PASS' -or
        $created.gate_pass -isnot [bool] -or $created.gate_pass -ne $true) {
        throw 'J01 R15 Create Gate is not PASS'
    }
    if ($created.cold_reopen_authorized -isnot [bool] -or $created.cold_reopen_authorized -ne $false) {
        throw 'J01 R15 Create Gate improperly self-authorizes cold reopen'
    }
    ...
    if ($createdReceipt.schema -ne 'B51R1_S05R2_J01_R15_NATIVE_PILOT_CREATE_V1' -or
        $createdReceipt.status -ne 'S05R2_J01_R15_NATIVE_1R_PILOT_CREATE_PASS' -or
        $createdReceipt.gate_pass -isnot [bool] -or $createdReceipt.gate_pass -ne $true) {
        throw 'J01 R15 Create receipt is not PASS'
    }
    ...
    $coldLockedArtifacts = @(
        [ordered]@{id='J01_R15_CREATE_GATE';path=$createGate;bytes=(Get-Item $createGate).Length;sha256=Get-Sha $createGate},
        [ordered]@{id='J01_R15_CREATE_RECEIPT';path=$createReceipt;bytes=[long]$created.receipt.bytes;sha256=[string]$created.receipt.sha256},
        [ordered]@{id='J01_R15_PILOT_ASSEMBLY';path=$pilotAssembly;bytes=[long]$created.output.bytes;sha256=[string]$created.output.sha256}
    )
}
...
try {
    ...
    if ($persistentSoftwareOglAfterLaunch -ne 0) {
        throw 'R15 one-session Software OpenGL launch changed the persistent registry value'
    }
    ...
    $expectedLockedItemCount = if ($Mode -eq 'Create') { 87 } else { 90 }
    if ($lockedItems.Count -ne $expectedLockedItemCount) {
        throw "J01 R15 locked item count mismatch: expected=$expectedLockedItemCount actual=$($lockedItems.Count)"
    }
    $lock = [ordered]@{
        schema="B51R1_S05R2_J01_R15_$($Mode.ToUpperInvariant())_INPUT_LOCK_V1"
        generated_at=[DateTimeOffset]::Now.ToString('o')
        status="PASS_S05R2_J01_R15_$($Mode.ToUpperInvariant())_INPUTS_HASH_LOCKED"
        mode=$Mode
        ...
        claim_limit='J01_R15_ONE_SESSION_SOFTWARE_OPENGL_TYPED_MATE_ENTITY_RECOVERY_NATIVE_1R_PILOT_ONLY_NO_FORMAL_S05_G4_T005_OR_FLIGHT_CREDIT'
    }
    Write-JsonCreateNew $inputLock $lock

    [void][Reflection.Assembly]::LoadFile((Resolve-Path -LiteralPath $toolDll).Path)
    if ($Mode -eq 'Create') {
        $result = [B51R1S05R2J01R15NativePilotTool]::Create(
        ...
        if ($data.schema -ne 'B51R1_S05R2_J01_R15_NATIVE_PILOT_CREATE_V1' -or
            $data.status -ne 'S05R2_J01_R15_NATIVE_1R_PILOT_CREATE_PASS' -or
        ...
        if ($mateAttempts.Count -ne 5 -or @($data.native_mate_creation).Count -ne 5 -or
            ...
            throw 'J01 R15 Mate API call ledger count mismatch'
        }
        ...
        foreach ($index in 0..4) {
            ...
            if ([long]$attempt.create_mate_data_call_index -ne ($index + 1) -or
                ...
                throw "J01 R15 Mate API call ledger mismatch at index $index"
            }
        }
        ...
        if ([long]$angleAttempt.selection_count_before_direct_property_contract_clear -ne 3 -or
            ...
            throw 'J01 R15 Limit Angle direct-property input-channel ledger mismatch'
        }
        if ($angleAttempt.precreate_entities_to_mate_getter_is_null) {
            if ($angleAttempt.precreate_entities_to_mate_getter_is_array -ne $false -or
                ...
                throw 'J01 R15 null EntitiesToMate getter diagnostics are not self-consistent'
            }
        }
        elseif ($angleAttempt.precreate_entities_to_mate_getter_is_array) {
            if ([string]::IsNullOrWhiteSpace([string]$angleAttempt.precreate_entities_to_mate_getter_managed_type) -or
                ...
                throw 'J01 R15 array EntitiesToMate getter diagnostics are not self-consistent'
            }
        }
        else {
            if ([string]::IsNullOrWhiteSpace([string]$angleAttempt.precreate_entities_to_mate_getter_managed_type) -or
                $null -ne $angleAttempt.precreate_entities_to_mate_getter_count -or
                $null -ne $angleAttempt.precreate_entities_to_mate) {
                throw 'J01 R15 scalar EntitiesToMate getter diagnostics are not self-consistent'
            }
        }
        ...
        foreach ($index in 0..12) {
            ...
            if ([long]$selection.select_by_id2_call_index -ne ($index + 1) -or
                ...
                throw "J01 R15 selection identity/mark ledger mismatch at index $index"
            }
            ...
            if ($isAssemblyRoot) {
                if ($selection.direct_corresponding_specific_diagnostic_only -ne $false -or
                    ...
                    throw "J01 R15 assembly-root selection unexpectedly has a component owner at index $index"
                }
            }
            else {
                ...
                if ($selection.direct_corresponding_specific_diagnostic_only -ne $true -or
                    ...
                    throw "J01 R15 selected owner binding mismatch at index $index"
                }
            }
            if ($null -ne $selection.selected_feature_matches_context_feature -and
                ...
                throw "J01 R15 selected context-Feature identity mismatch at index $index"
            }
        }
        ...
        if ($preloads.Count -ne 2 -or [long]$data.document_count_before_new_assembly -ne 2 -or
            ...
            throw 'J01 R15 preload count or assembly document-count transition mismatch'
        }
        foreach ($index in 0..1) {
            ...
            if ([string]$preload.role -ne $expectedRole -or
                ...
                throw "J01 R15 strict preload evidence mismatch: $expectedRole"
            }
        }
        ...
        $gate = [ordered]@{
            schema='B51R1_S05R2_J01_R15_NATIVE_1R_CREATE_GATE_V1';generated_at=[DateTimeOffset]::Now.ToString('o')
            status='S05R2_J01_R15_NATIVE_1R_PILOT_CREATE_PASS';gate_pass=$true
            ...
            claim_limit='J01_R15_SELECTIONMGR_BOUND_TYPED_MATE_ENTITY_RECOVERY_CREATE_AND_SAME_DOCUMENT_DRIVER_EVIDENCE_ONLY_PENDING_INDEPENDENT_COLD_REOPEN'
        }
        Write-JsonCreateNew $createGate $gate
        Write-Output $createGate
    }
    else {
        $result = [B51R1S05R2J01R15NativePilotTool]::ColdVerify(
        ...
        if ($data.schema -ne 'B51R1_S05R2_J01_R15_NATIVE_PILOT_COLD_REOPEN_V1' -or
            $data.status -ne 'S05R2_J01_R15_NATIVE_1R_PILOT_COLD_REOPEN_PASS' -or
            $data.gate_pass -isnot [bool] -or $data.gate_pass -ne $true) {
            throw 'J01 R15 ColdVerify receipt is not PASS'
        }
        if ($data.transform2_call_count -ne 0 -or $data.set_transform_and_solve_call_count -ne 0 -or
        ...
        if ($resolution.Count -lt 2 -or $expectedIdentities.Count -ne 2 -or
            ...
            throw 'J01 R15 ColdVerify did not resolve parent and child uniquely in one component traversal'
        }
        foreach ($role in @('parent','child')) {
            ...
            if ($identity.Count -ne 1 -or $match.Count -ne 1 -or
                ...
                throw "J01 R15 ColdVerify component identity mismatch: $role"
            }
        }
        foreach ($item in $lockedItems) {
        ...
        $gate = [ordered]@{
            schema='B51R1_S05R2_J01_R15_NATIVE_1R_GATE_V1';generated_at=[DateTimeOffset]::Now.ToString('o')
            status='S05R2_J01_R15_NATIVE_1R_PILOT_PASS';gate_pass=$true
            ...
            claim_limit='J01_R15_SELECTIONMGR_BOUND_TYPED_MATE_ENTITY_RECOVERY_NATIVE_1R_PILOT_GATE_ONLY_NO_COMPLETE_CHAIN_G4_T005_STRUCTURAL_MANUFACTURING_OR_FLIGHT_CREDIT'
        }
