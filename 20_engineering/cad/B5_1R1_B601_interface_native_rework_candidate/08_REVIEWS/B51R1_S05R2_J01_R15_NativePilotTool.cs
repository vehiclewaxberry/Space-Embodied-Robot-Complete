...
public static class B51R1S05R2J01R15NativePilotTool
{
        ...
        string outputAssemblyPath, string j00CheckpointPath, string receiptPath, string progressPath)
    {
        ...
        {
            { "schema", "B51R1_S05R2_J01_R15_NATIVE_PILOT_CREATE_V1" },
        ...
        try
        {
            ...
            ProgressCreateNew(progressPath, "J01_R15_CREATE_START " + DateTime.UtcNow.ToString("o"));
            ...
            {
                { "schema", "B51R1_S05R2_J01_R15_J00_ROOT_CHECKPOINT_V1" },
                { "status", "S05R2_J01_R15_J00_ROOT_BASELINE_PASS" },
            ...
            receipt["status"] = "S05R2_J01_R15_NATIVE_1R_PILOT_CREATE_PASS";
            ...
            Progress(progressPath, "J01_R15_CREATE_PASS " + outputEvidence["sha256"]);
            process.Dispose();
            return 0;
        }
        catch (Exception ex)
        {
            receipt["status"] = "S05R2_J01_R15_NATIVE_1R_PILOT_CREATE_FAIL_CLOSED";
            ...
            try { if (File.Exists(progressPath)) Progress(progressPath, "J01_R15_CREATE_FAIL " + ex.Message); } catch { }
            return 1;
        }
        finally
        {
        ...
        string receiptPath, string progressPath)
    {
        ...
        var receipt = new Dictionary<string, object>
        {
            { "schema", "B51R1_S05R2_J01_R15_NATIVE_PILOT_COLD_REOPEN_V1" },
            { "status", "FAIL_CLOSED_NOT_STARTED" }, { "generated_at_utc", DateTime.UtcNow.ToString("o") },
            { "expected_process_id", expectedProcessId }, { "save_as_call_count", 0 }, { "save3_call_count", 0 },
            { "create_mate_data_call_count", 0 }, { "create_mate_call_count", 0 }, { "add_mate5_call_count", 0 },
            { "mate_preselection_call_count", 0 }, { "transform2_call_count", 0 },
        ...
        try
        {
            ...
            app = Attach(expectedProcessId);
            process = Process.GetProcessById(expectedProcessId);
            ProgressCreateNew(progressPath, "J01_R15_COLD_REOPEN_START " + DateTime.UtcNow.ToString("o"));
            int errors = 0, warnings = 0;
            ...
            receipt["status"] = "S05R2_J01_R15_NATIVE_1R_PILOT_COLD_REOPEN_PASS";
            receipt["gate_pass"] = true;
            ...
            receipt["normal_application_exit"] = true;
            WriteJsonCreateNew(receiptPath, receipt);
            Progress(progressPath, "J01_R15_COLD_REOPEN_PASS " + before);
            process.Dispose();
            return 0;
        }
        catch (Exception ex)
        {
            receipt["status"] = "S05R2_J01_R15_NATIVE_1R_PILOT_COLD_REOPEN_FAIL_CLOSED";
            receipt["gate_pass"] = false;
            receipt["exception_type"] = ex.GetType().FullName;
            receipt["exception"] = ex.ToString();
            ReleaseCom(driver); driver = null;
            ...
            try { if (app != null && app.GetDocumentCount() == 0) app.ExitApp(); } catch { }
            try { if (!File.Exists(receiptPath)) WriteJsonCreateNew(receiptPath, receipt); } catch { }
            try { if (File.Exists(progressPath)) Progress(progressPath, "J01_R15_COLD_REOPEN_FAIL " + ex.Message); } catch { }
            return 1;
        }
        finally
        {
