...
$sessionRoot = Join-Path $root '07_VERIFICATION\AUTONOMOUS\S05R2_J01_R2'
...
$toolSource = Join-Path $PSScriptRoot 'B51R1_S05R2_J01_R2_NativePilotTool.cs'
$toolDll = Join-Path $PSScriptRoot 'B51R1_S05R2_J01_R2_NativePilotTool.dll'
$toolSourceBytes = 83272
$toolSourceSha256 = '54164EA5157B2A241F429E6092CB8325F7F10228ACF7B8CB6FE3DAF315AE6170'
$toolDllBytes = 56320
$toolDllSha256 = 'F8C2B545A7CB1C7EA6033EC1EFDE9B8FFF463DC65D518B070C350B8CFEC900CE'
...
$j00Assembly = Join-Path $checkpointDir 'B51R1_CARRIER_J00_ROOT_PILOT_R2.SLDASM'
$pilotAssembly = Join-Path $pilotDir 'B51R1_CARRIER_J01_R2_NATIVE_PILOT.SLDASM'
$j00Checkpoint = Join-Path $sessionRoot 'B51R1_S05R2_J01_R2_J00_BASELINE_CHECKPOINT.json'
$createReceipt = Join-Path $sessionRoot 'B51R1_S05R2_J01_R2_CREATE_RECEIPT.json'
$createProgress = Join-Path $sessionRoot 'B51R1_S05R2_J01_R2_CREATE_PROGRESS.log'
$createGate = Join-Path $sessionRoot 'B51R1_S05R2_J01_R2_CREATE_GATE.json'
$coldReceipt = Join-Path $sessionRoot 'B51R1_S05R2_J01_R2_COLD_REOPEN_RECEIPT.json'
$coldProgress = Join-Path $sessionRoot 'B51R1_S05R2_J01_R2_COLD_REOPEN_PROGRESS.log'
$finalGate = Join-Path $sessionRoot 'B51R1_S05R2_J01_R2_NATIVE_1R_GATE.json'
...
function Get-RecoveryPrerequisites {
    Assert-File $hp01DirectReceipt $hp01DirectReceiptBytes $hp01DirectReceiptSha256 'J01 R2 direct-open health receipt'
    ...
    if ($direct.schema -ne 'B51R1_S05R2_R1_DRIVER_READY_COLD_REOPEN_V3' -or
        ...
        throw 'J01 R2 direct-open health receipt contract mismatch'
    }
    $directParts = @($direct.parts)
    if ($directParts.Count -ne 2 -or @($directParts.link | Select-Object -Unique).Count -ne 2) {
        throw 'J01 R2 direct-open health receipt does not bind two unique parts'
    }
    foreach ($link in @('base_link','link1')) {
        $part = @($directParts | Where-Object { $_.link -eq $link })
        if ($part.Count -ne 1) { throw "J01 R2 direct-open part is not unique: $link" }
        $expectedPath = if ($link -eq 'base_link') { $baseCarrier } else { $link1Carrier }
        $expectedBytes = if ($link -eq 'base_link') { $baseCarrierBytes } else { $link1CarrierBytes }
        $expectedSha = if ($link -eq 'base_link') { $baseCarrierSha256 } else { $link1CarrierSha256 }
        if ([IO.Path]::GetFullPath([string]$part[0].path) -ne [IO.Path]::GetFullPath($expectedPath) -or
            ...
            throw "J01 R2 direct-open health binding mismatch: $link"
        }
    }
...
Assert-File $toolCompiler $toolCompilerBytes $toolCompilerSha256 'J01 R2 frozen-tool compiler provenance'
...
$readinessReceipt = Join-Path $sessionRoot "B51R1_S05R2_J01_R2_$($Mode.ToUpperInvariant())_READINESS_RECEIPT.json"
$inputLock = Join-Path $sessionRoot "B51R1_S05R2_J01_R2_$($Mode.ToUpperInvariant())_INPUT_LOCK.json"
...
if ($Mode -eq 'ColdVerify') {
    ...
    if ($created.schema -ne 'B51R1_S05R2_J01_R2_NATIVE_1R_CREATE_GATE_V1' -or
        $created.status -ne 'S05R2_J01_R2_NATIVE_1R_PILOT_CREATE_PASS' -or
        $created.gate_pass -isnot [bool] -or $created.gate_pass -ne $true) {
        throw 'J01 R2 Create Gate is not PASS'
    }
    if ($created.cold_reopen_authorized -isnot [bool] -or $created.cold_reopen_authorized -ne $true) {
        throw 'J01 R2 Create Gate does not authorize cold reopen'
    }
    ...
    if ($createdReceipt.schema -ne 'B51R1_S05R2_J01_R2_NATIVE_PILOT_CREATE_V1' -or
        $createdReceipt.status -ne 'S05R2_J01_R2_NATIVE_1R_PILOT_CREATE_PASS' -or
        $createdReceipt.gate_pass -isnot [bool] -or $createdReceipt.gate_pass -ne $true) {
        throw 'J01 R2 Create receipt is not PASS'
    }
    ...
    $coldLockedArtifacts = @(
        [ordered]@{id='J01_R2_CREATE_GATE';path=$createGate;bytes=(Get-Item $createGate).Length;sha256=Get-Sha $createGate},
        [ordered]@{id='J01_R2_CREATE_RECEIPT';path=$createReceipt;bytes=[long]$created.receipt.bytes;sha256=[string]$created.receipt.sha256},
        [ordered]@{id='J01_R2_PILOT_ASSEMBLY';path=$pilotAssembly;bytes=[long]$created.output.bytes;sha256=[string]$created.output.sha256}
    )
}
...
try {
    ...
    $lock = [ordered]@{
        schema="B51R1_S05R2_J01_R2_$($Mode.ToUpperInvariant())_INPUT_LOCK_V1"
        generated_at=[DateTimeOffset]::Now.ToString('o')
        status="PASS_S05R2_J01_R2_$($Mode.ToUpperInvariant())_INPUTS_HASH_LOCKED"
        mode=$Mode
        ...
        claim_limit='J01_R2_PRELOAD_RECOVERY_NATIVE_1R_PILOT_ONLY_NO_FORMAL_S05_G4_T005_OR_FLIGHT_CREDIT'
    }
    Write-JsonCreateNew $inputLock $lock

    [void][Reflection.Assembly]::LoadFile((Resolve-Path -LiteralPath $toolDll).Path)
    if ($Mode -eq 'Create') {
        $result = [B51R1S05R2J01R2NativePilotTool]::Create(
        ...
        if ($data.schema -ne 'B51R1_S05R2_J01_R2_NATIVE_PILOT_CREATE_V1' -or
            $data.status -ne 'S05R2_J01_R2_NATIVE_1R_PILOT_CREATE_PASS' -or
        ...
        if ($preloads.Count -ne 2 -or [long]$data.document_count_before_new_assembly -ne 2 -or
            ...
            throw 'J01 R2 preload count or assembly document-count transition mismatch'
        }
        foreach ($index in 0..1) {
            ...
            if ([string]$preload.role -ne $expectedRole -or
                ...
                throw "J01 R2 strict preload evidence mismatch: $expectedRole"
            }
        }
        ...
        $gate = [ordered]@{
            schema='B51R1_S05R2_J01_R2_NATIVE_1R_CREATE_GATE_V1';generated_at=[DateTimeOffset]::Now.ToString('o')
            status='S05R2_J01_R2_NATIVE_1R_PILOT_CREATE_PASS';gate_pass=$true
            ...
            claim_limit='J01_R2_PRELOAD_RECOVERY_CREATE_AND_SAME_DOCUMENT_DRIVER_EVIDENCE_ONLY_PENDING_INDEPENDENT_COLD_REOPEN'
        }
        Write-JsonCreateNew $createGate $gate
        Write-Output $createGate
    }
    else {
        $result = [B51R1S05R2J01R2NativePilotTool]::ColdVerify(
        ...
        if ($data.schema -ne 'B51R1_S05R2_J01_R2_NATIVE_PILOT_COLD_REOPEN_V1' -or
            $data.status -ne 'S05R2_J01_R2_NATIVE_1R_PILOT_COLD_REOPEN_PASS' -or
            $data.gate_pass -isnot [bool] -or $data.gate_pass -ne $true) {
            throw 'J01 R2 ColdVerify receipt is not PASS'
        }
        if ($data.transform2_call_count -ne 0 -or $data.set_transform_and_solve_call_count -ne 0 -or
        ...
        $gate = [ordered]@{
            schema='B51R1_S05R2_J01_R2_NATIVE_1R_GATE_V1';generated_at=[DateTimeOffset]::Now.ToString('o')
            status='S05R2_J01_R2_NATIVE_1R_PILOT_PASS';gate_pass=$true
            ...
            claim_limit='J01_R2_PRELOAD_RECOVERY_NATIVE_1R_PILOT_GATE_ONLY_NO_COMPLETE_CHAIN_G4_T005_STRUCTURAL_MANUFACTURING_OR_FLIGHT_CREDIT'
        }
