...
public static class B51R1S02R1MasterSkeletonColdReopenVerifierV2
{
        ...
        double expectedRxRadians)
    {
        ...
        MathTransform featureLocalToModelTransform = null;
        MathTransform localToModelTransform = null;
        ...
        try
        {
            ...
            // SOLIDWORKS' coordinate-system transform is model-to-local. The
            // official API example reports the origin as -ArrayData[9..11].
            // Use IMathTransform.IInverse deterministically to obtain the
            // local-coordinate-system pose in model coordinates.
            featureLocalToModelTransform = featureTransform.IInverse();
            localToModelTransform = namedTransform.IInverse();
            Require(
                featureLocalToModelTransform != null &&
                localToModelTransform != null,
                "IMathTransform.IInverse unavailable for one or both APIs for " +
                name);
            double[] featureLocalToModel =
                ToDoubleArray(featureLocalToModelTransform.ArrayData);
            double[] canonicalPose = ToDoubleArray(localToModelTransform.ArrayData);
            double[] manualLocalToModel = InvertRigidRowTransform(namedRaw);
            Require(
                featureLocalToModel.Length == 16 &&
                canonicalPose.Length == 16 &&
                manualLocalToModel.Length == 16,
                "Expected three 16-element inverse transforms for " + name);
            double dualApiInverseError =
                MaximumError(featureLocalToModel, canonicalPose, 0, 16);
            double apiVsAlgebraInverseError =
                MaximumError(canonicalPose, manualLocalToModel, 0, 16);
            Require(
                dualApiInverseError <= HomogeneousTolerance,
                name + " dual-API inverse mismatch; maximum error=" +
                dualApiInverseError);
            Require(
                apiVsAlgebraInverseError <= HomogeneousTolerance,
                name + " API inverse differs from fixed rigid algebra inverse; maximum error=" +
                apiVsAlgebraInverseError);
            ...
            return new Dictionary<string, object>
            {
                ...
                {"raw_model_to_local_feature_data_array", featureRaw},
                {"raw_model_to_local_named_api_array", namedRaw},
                {"dual_api_raw_max_abs_error", dualApiRawError},
                {"feature_data_local_to_model_inverse_array",
                    featureLocalToModel},
                {"canonical_local_to_model_pose_array", canonicalPose},
                {"fixed_algebra_local_to_model_inverse_array",
                    manualLocalToModel},
                {"dual_api_inverse_max_abs_error", dualApiInverseError},
                {"api_vs_fixed_algebra_inverse_max_abs_error",
                    apiVsAlgebraInverseError},
                {"canonical_pose_source",
                    "IMathTransform.IInverse_OF_GetCoordinateSystemTransformByName"},
                ...
                {"solidworks_transform_convention",
                    "RAW_MODEL_TO_LOCAL; CANONICAL_LOCAL_TO_MODEL_IS_FIXED_INVERSE"}
            };
        }
        finally
        {
            ...
            ReleaseCom(localToModelTransform);
            ReleaseCom(featureLocalToModelTransform);
        ...
        double expectedSignedOffsetMetres)
    {
        Feature feature = null;
        RefPlane plane = null;
        MathTransform modelToPlane = null;
        MathTransform planeToModel = null;
        try
        {
            ...
            modelToPlane = plane.Transform;
            Require(modelToPlane != null, "IRefPlane.Transform unavailable for " + name);
            double[] raw = ToDoubleArray(modelToPlane.ArrayData);
            Require(raw.Length == 16, "Expected 16 plane transform values for " + name);
            planeToModel = modelToPlane.IInverse();
            Require(planeToModel != null,
                "IMathTransform.IInverse unavailable for reference plane " + name);
            double[] pose = ToDoubleArray(planeToModel.ArrayData);
            Require(pose.Length == 16,
                "Expected 16 inverse plane transform values for " + name);
            ...
            {
                {"name", name},
                {"api_readback", "IFeature.GetSpecificFeature2_IRefPlane.Transform_IInverse"},
                {"raw_model_to_plane_array", raw},
        ...
        {
            ReleaseCom(planeToModel);
            ReleaseCom(modelToPlane);
            ReleaseCom(plane);
            ReleaseCom(feature);
        }
    }
        ...
        ModelDoc2 model)
    {
        ...
        try
        {
            ...
            Require(extension != null,
                "ModelDocExtension unavailable during property-scope verification");
            foreach (string configurationName in RequiredConfigurations)
            {
                ...
                try
                {
                    ...
                    Require(manager != null,
                        "Configuration-level CustomPropertyManager unavailable: " +
                        configurationName);
        ...
        string expectedFinalLockSha256)
    {
        ...
        Dictionary<string, object> configurationPropertyScope =
            VerifyNoConfigurationLevelRepairProperties(model);
        try
        {
            extension = model.Extension;
        ...
        string expectedFinalLockSha256)
    {
        ...
        finally
        {
            ...
            {
                try { model.ShowConfiguration2(initialConfiguration); }
                catch { }
            }
        }
        Require(
        ...
        string expectedFinalLockSha256)
    {
        ...
        {
            {"schema", "B51R1_S02_R1_MASTER_SKELETON_COLD_REOPEN_RECEIPT_V2"},
            {"session_id", "S02_R1"},
            {"generated_at_utc", DateTime.UtcNow.ToString("o")},
            {"status", "S02_R1_FAIL_CLOSED_NOT_STARTED"},
        ...
        try
        {
            ...
            receipt["status"] = "S02_R1_MASTER_SKELETON_COLD_REOPEN_PASS";
        ...
        {
            receipt["status"] = "S02_R1_FAIL_CLOSED";
