...
public static class B51R1S05R2J01R4NativePilotTool
{
    ...
    private static Dictionary<string, object> EntitySummary(object value)
    {
        ...
        if (value is RefPlane)
            return new Dictionary<string, object>
            {
                { "runtime_type", "RefPlane" }, { "name", null }, { "feature_type", "RefPlane" }
            };
        if (value is RefAxis)
            return new Dictionary<string, object>
            {
                { "runtime_type", "RefAxis" }, { "name", null }, { "feature_type", "RefAxis" }
            };
        return new Dictionary<string, object>
        {
    ...
    private static RefPlane SpecificPlane(Feature feature, string label)
    {
        Require(feature != null, "Reference-plane Feature is absent: " + label);
        RefPlane plane = feature.GetSpecificFeature2() as RefPlane;
        Require(plane != null, "Reference-plane Feature did not yield IRefPlane: " + label);
        return plane;
    }

    private static RefAxis SpecificAxis(Feature feature, string label)
    {
        Require(feature != null, "Reference-axis Feature is absent: " + label);
        RefAxis axis = feature.GetSpecificFeature2() as RefAxis;
        Require(axis != null, "Reference-axis Feature did not yield IRefAxis: " + label);
        return axis;
    }
    ...
    private static Feature CreateCoincidentMate(ModelDoc2 model, AssemblyDoc assembly, RefPlane first,
        ...
        List<Dictionary<string, object>> apiAttempts, out Dictionary<string, object> evidence)
    {
        object rawData = null;
        ...
        {
            { "name", name }, { "mate_type", MateCoincident },
            { "entity_contract", "IRefPlane/IRefPlane" }
        };
        ...
        List<Dictionary<string, object>> apiAttempts, out Dictionary<string, object> evidence)
    {
        ...
        var attempt = new Dictionary<string, object>
        {
            { "name", name }, { "mate_type", MateHinge },
            { "entity_contract", "IRefAxis/IRefAxis+IRefPlane/IRefPlane" }
        };
        apiAttempts.Add(attempt);
        try
        {
    ...
    private static Feature CreateLimitAngleMate(ModelDoc2 model, AssemblyDoc assembly,
        ...
        Dictionary<string, object> receipt, List<Dictionary<string, object>> apiAttempts,
        out Dictionary<string, object> evidence)
    {
        ...
        var attempt = new Dictionary<string, object>
        {
            { "name", name }, { "mate_type", MateAngle },
            { "entity_contract", "IRefPlane/IRefPlane+IRefAxis_REFERENCE" }
        };
        apiAttempts.Add(attempt);
        try
        {
        ...
        Component2 parent, Component2 child)
    {
        ...
        RefAxis parentAxisRef = null, childAxisRef = null;
        RefPlane parentSeatRef = null, childSeatRef = null, parentLimitRef = null, childZeroRef = null;
        ...
        try
        {
            ...
            parentAxisRef = SpecificAxis(parentAxis, "AXIS_joint1_PARENT_SIDE");
            childAxisRef = SpecificAxis(childAxis, "AXIS_joint1");
            parentSeatRef = SpecificPlane(parentSeat, "PLANE_SEAT_joint1_PARENT_SIDE");
            childSeatRef = SpecificPlane(childSeat, "PLANE_SEAT_joint1_CHILD_SIDE");
            parentLimitRef = SpecificPlane(parentLimit, "PLANE_LIMIT_REF_joint1_PARENT_SIDE");
            childZeroRef = SpecificPlane(childZero, "PLANE_ZERO_joint1");
            ...
            Require(SameUnorderedPair(hingeAxes, parentAxisRef, childAxisRef), "Native hinge axis binding identity mismatch");
            Require(SameUnorderedPair(hingeSeats, parentSeatRef, childSeatRef), "Native hinge seat binding identity mismatch");
            ...
            Require(SameUnorderedPair(angleEntities, parentLimitRef, childZeroRef), "Native limit-angle plane binding identity mismatch");
            Require(SameComIdentity(referenceEntity, parentAxisRef), "Native limit-angle parent-axis identity mismatch");
        ...
        finally
        {
            ...
            ReleaseCom(childZeroRef); ReleaseCom(parentLimitRef); ReleaseCom(childSeatRef); ReleaseCom(parentSeatRef);
            ReleaseCom(childAxisRef); ReleaseCom(parentAxisRef);
        ...
        string outputAssemblyPath, string j00CheckpointPath, string receiptPath, string progressPath)
    {
        ...
        {
            { "schema", "B51R1_S05R2_J01_R4_NATIVE_PILOT_CREATE_V1" },
        ...
        try
        {
            ...
            ProgressCreateNew(progressPath, "J01_R4_CREATE_START " + DateTime.UtcNow.ToString("o"));
            ...
            {
                Feature assemblyPlaneFeature = null, componentPlaneFeature = null, rootMate = null;
                RefPlane assemblyPlane = null, componentPlane = null;
                try
                {
                    assemblyPlaneFeature = FindFeature(model, "ASM_ROOT_PLN_" + axes[i], "RefPlane");
                    componentPlaneFeature = CorrespondingFeature(parent, "ROOT_PLN_" + axes[i], "RefPlane");
                    assemblyPlane = SpecificPlane(assemblyPlaneFeature, "ASM_ROOT_PLN_" + axes[i]);
                    componentPlane = SpecificPlane(componentPlaneFeature, "ROOT_PLN_" + axes[i]);
                    Dictionary<string, object> creation;
                    rootMate = CreateCoincidentMate(model, assembly, assemblyPlane, componentPlane,
                        "J00_ROOT_" + axes[i] + "_COINCIDENT", receipt, mateApiAttempts, out creation);
                ...
                finally
                {
                    ...
                    ReleaseCom(componentPlaneFeature); ReleaseCom(assemblyPlaneFeature);
                }
            }
            ...
            {
                { "schema", "B51R1_S05R2_J01_R4_J00_ROOT_CHECKPOINT_V1" },
                { "status", "S05R2_J01_R4_J00_ROOT_BASELINE_PASS" },
            ...
            Feature parentAxisFeature = null, childAxisFeature = null, parentSeatFeature = null, childSeatFeature = null;
            Feature parentLimitFeature = null, childZeroFeature = null, hinge = null;
            RefAxis parentAxis = null, childAxis = null;
            RefPlane parentSeat = null, childSeat = null, parentLimit = null, childZero = null;
            try
            {
                parentAxisFeature = CorrespondingFeature(parent, "AXIS_joint1_PARENT_SIDE", "RefAxis");
                childAxisFeature = CorrespondingFeature(child, "AXIS_joint1", "RefAxis");
                parentSeatFeature = CorrespondingFeature(parent, "PLANE_SEAT_joint1_PARENT_SIDE", "RefPlane");
                childSeatFeature = CorrespondingFeature(child, "PLANE_SEAT_joint1_CHILD_SIDE", "RefPlane");
                parentAxis = SpecificAxis(parentAxisFeature, "AXIS_joint1_PARENT_SIDE");
                childAxis = SpecificAxis(childAxisFeature, "AXIS_joint1");
                parentSeat = SpecificPlane(parentSeatFeature, "PLANE_SEAT_joint1_PARENT_SIDE");
                childSeat = SpecificPlane(childSeatFeature, "PLANE_SEAT_joint1_CHILD_SIDE");
                Dictionary<string, object> hingeCreation;
                hinge = CreateHingeMate(model, assembly, parentAxis, childAxis, parentSeat, childSeat,
                    "J01_HINGE_NATIVE", receipt, mateApiAttempts, out hingeCreation);
                nativeMateCreations.Add(hingeCreation);
                ReleaseCom(hinge); hinge = null;

                parentLimitFeature = CorrespondingFeature(parent, "PLANE_LIMIT_REF_joint1_PARENT_SIDE", "RefPlane");
                childZeroFeature = CorrespondingFeature(child, "PLANE_ZERO_joint1", "RefPlane");
                parentLimit = SpecificPlane(parentLimitFeature, "PLANE_LIMIT_REF_joint1_PARENT_SIDE");
                childZero = SpecificPlane(childZeroFeature, "PLANE_ZERO_joint1");
                Dictionary<string, object> driverCreation;
                driver = CreateLimitAngleMate(model, assembly, parentLimit, childZero, parentAxis,
                    "J01_LIMIT_ANGLE_DRIVER", receipt, mateApiAttempts, out driverCreation);
            ...
            finally
            {
                ...
                ReleaseCom(childZeroFeature); ReleaseCom(parentLimitFeature);
                ReleaseCom(childSeatFeature); ReleaseCom(parentSeatFeature);
                ReleaseCom(childAxisFeature); ReleaseCom(parentAxisFeature);
            }
            ...
            receipt["status"] = "S05R2_J01_R4_NATIVE_1R_PILOT_CREATE_PASS";
            ...
            Progress(progressPath, "J01_R4_CREATE_PASS " + Sha256(outputAssemblyPath));
            process.Dispose();
            return 0;
        }
        catch (Exception ex)
        {
            receipt["status"] = "S05R2_J01_R4_NATIVE_1R_PILOT_CREATE_FAIL_CLOSED";
            ...
            try { if (File.Exists(progressPath)) Progress(progressPath, "J01_R4_CREATE_FAIL " + ex.Message); } catch { }
            return 1;
        }
        finally
        {
        ...
        string baseCarrierFileName, string link1CarrierFileName, string receiptPath, string progressPath)
    {
        var receipt = new Dictionary<string, object>
        {
            { "schema", "B51R1_S05R2_J01_R4_NATIVE_PILOT_COLD_REOPEN_V1" },
            { "status", "FAIL_CLOSED_NOT_STARTED" }, { "generated_at_utc", DateTime.UtcNow.ToString("o") },
            { "expected_process_id", expectedProcessId }, { "save_as_call_count", 0 }, { "save3_call_count", 0 },
            { "create_mate_data_call_count", 0 }, { "create_mate_call_count", 0 }, { "add_mate5_call_count", 0 },
            { "mate_preselection_call_count", 0 }, { "transform2_call_count", 0 },
        ...
        try
        {
            ...
            app = Attach(expectedProcessId);
            process = Process.GetProcessById(expectedProcessId);
            ProgressCreateNew(progressPath, "J01_R4_COLD_REOPEN_START " + DateTime.UtcNow.ToString("o"));
            int errors = 0, warnings = 0;
            ...
            receipt["status"] = "S05R2_J01_R4_NATIVE_1R_PILOT_COLD_REOPEN_PASS";
            receipt["gate_pass"] = true;
            ...
            receipt["normal_application_exit"] = true;
            WriteJsonCreateNew(receiptPath, receipt);
            Progress(progressPath, "J01_R4_COLD_REOPEN_PASS " + before);
            process.Dispose();
            return 0;
        }
        catch (Exception ex)
        {
            receipt["status"] = "S05R2_J01_R4_NATIVE_1R_PILOT_COLD_REOPEN_FAIL_CLOSED";
            receipt["gate_pass"] = false;
            receipt["exception_type"] = ex.GetType().FullName;
            receipt["exception"] = ex.ToString();
            ReleaseCom(driver); driver = null;
            ...
            try { if (app != null && app.GetDocumentCount() == 0) app.ExitApp(); } catch { }
            try { if (!File.Exists(receiptPath)) WriteJsonCreateNew(receiptPath, receipt); } catch { }
            try { if (File.Exists(progressPath)) Progress(progressPath, "J01_R4_COLD_REOPEN_FAIL " + ex.Message); } catch { }
            return 1;
        }
        finally
        {
