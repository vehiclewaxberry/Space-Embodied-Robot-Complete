...
public static class B51R1S05R2J01R5NativePilotTool
{
    ...
    private static object CorrespondingSpecific(Component2 component, string name, string type,
        int expectedSelectionType)
    {
        ModelDoc2 part = component.GetModelDoc2() as ModelDoc2;
        Require(part != null, "Component model unavailable: " + component.Name2);
        Feature source = FindFeature(part, name, type);
        object sourceSpecific = source.GetSpecificFeature2();
        Require(IsExpectedSpecificType(sourceSpecific, expectedSelectionType),
            "Source Feature did not yield the required specific interface: " + name);
        object result = component.GetCorresponding(sourceSpecific);
        Require(IsExpectedSpecificType(result, expectedSelectionType),
            "Specific-object GetCorresponding failed: " + name + "@" + component.Name2);
        // The returned specific object can share COM identity with other live mate RCWs.
        // Leave these borrowed aliases to normal GC instead of FinalReleaseComObject.
        return result;
    }
        ...
        List<Dictionary<string, object>> selectionLedger)
    {
        ...
        try
        {
            ...
            Require(IsExpectedSpecificType(directSpecific, expectedSelectionType),
                "Direct corresponding specific object has the wrong runtime interface: " + role);

            record["source_feature"] = EntitySummary(sourceFeature);
            record["source_specific"] = EntitySummary(sourceSpecific);
            record["direct_corresponding_specific"] = EntitySummary(directSpecific);
            ...
            Require(SameComIdentity(selectedSpecific, directSpecific),
                "Selected specific entity differs from direct specific-object GetCorresponding: " + role);
            if (component == null)
            ...
            record["selected_specific_matches_direct_corresponding_specific"] = true;
        ...
        Dictionary<string, object> attempt, out Dictionary<string, object> evidence)
    {
        ...
        try
        {
            ...
            Require(selectionCountAtReturn == preselectionCount,
                "Coincident selection did not persist through CreateMate return: " + name);
            model.ClearSelection2(true);
            attempt["selection_count_after_post_create_clear"] = SelectedObjectCount(model);
        ...
        Dictionary<string, object> attempt, out Dictionary<string, object> evidence)
    {
        ...
        try
        {
            ...
            attempt["selection_count_at_create_mate_return"] = selectionCountAtReturn;
            Require(selectionCountAtReturn == preselectionCount,
                "Hinge selection did not persist through CreateMate return: " + name);
            model.ClearSelection2(true);
            attempt["selection_count_after_post_create_clear"] = SelectedObjectCount(model);
        ...
        out Dictionary<string, object> evidence)
    {
        ...
        {
            int preselectionCount = SelectedObjectCount(model);
            Require(preselectionCount == 3, "Limit angle mate requires exactly three marked selections: " + name);
            attempt["preselection_count_before_create_mate_data"] = preselectionCount;
            attempt["create_mate_data_call_index"] = IncrementCounter(receipt, "create_mate_data_call_count");
            ...
            attempt["selection_count_at_create_mate_return"] = selectionCountAtReturn;
            Require(selectionCountAtReturn == preselectionCount,
                "Limit-angle selection did not persist through CreateMate return: " + name);
            model.ClearSelection2(true);
            attempt["selection_count_after_post_create_clear"] = SelectedObjectCount(model);
        ...
        Component2 parent, Component2 child)
    {
        ...
        try
        {
            ...
            parentAxisRef = CorrespondingSpecific(parent, "AXIS_joint1_PARENT_SIDE", "RefAxis", SelectDatumAxis) as RefAxis;
            childAxisRef = CorrespondingSpecific(child, "AXIS_joint1", "RefAxis", SelectDatumAxis) as RefAxis;
            parentSeatRef = CorrespondingSpecific(parent, "PLANE_SEAT_joint1_PARENT_SIDE", "RefPlane", SelectDatumPlane) as RefPlane;
            childSeatRef = CorrespondingSpecific(child, "PLANE_SEAT_joint1_CHILD_SIDE", "RefPlane", SelectDatumPlane) as RefPlane;
            parentLimitRef = CorrespondingSpecific(parent, "PLANE_LIMIT_REF_joint1_PARENT_SIDE", "RefPlane", SelectDatumPlane) as RefPlane;
            childZeroRef = CorrespondingSpecific(child, "PLANE_ZERO_joint1", "RefPlane", SelectDatumPlane) as RefPlane;
        ...
        string outputAssemblyPath, string j00CheckpointPath, string receiptPath, string progressPath)
    {
        ...
        {
            { "schema", "B51R1_S05R2_J01_R5_NATIVE_PILOT_CREATE_V1" },
        ...
        try
        {
            ...
            ProgressCreateNew(progressPath, "J01_R5_CREATE_START " + DateTime.UtcNow.ToString("o"));
            ...
            {
                { "schema", "B51R1_S05R2_J01_R5_J00_ROOT_CHECKPOINT_V1" },
                { "status", "S05R2_J01_R5_J00_ROOT_BASELINE_PASS" },
            ...
            receipt["status"] = "S05R2_J01_R5_NATIVE_1R_PILOT_CREATE_PASS";
            ...
            Progress(progressPath, "J01_R5_CREATE_PASS " + Sha256(outputAssemblyPath));
            process.Dispose();
            return 0;
        }
        catch (Exception ex)
        {
            receipt["status"] = "S05R2_J01_R5_NATIVE_1R_PILOT_CREATE_FAIL_CLOSED";
            ...
            try { if (File.Exists(progressPath)) Progress(progressPath, "J01_R5_CREATE_FAIL " + ex.Message); } catch { }
            return 1;
        }
        finally
        {
        ...
        string receiptPath, string progressPath)
    {
        ...
        var receipt = new Dictionary<string, object>
        {
            { "schema", "B51R1_S05R2_J01_R5_NATIVE_PILOT_COLD_REOPEN_V1" },
            { "status", "FAIL_CLOSED_NOT_STARTED" }, { "generated_at_utc", DateTime.UtcNow.ToString("o") },
            { "expected_process_id", expectedProcessId }, { "save_as_call_count", 0 }, { "save3_call_count", 0 },
            { "create_mate_data_call_count", 0 }, { "create_mate_call_count", 0 }, { "add_mate5_call_count", 0 },
            { "mate_preselection_call_count", 0 }, { "transform2_call_count", 0 },
        ...
        try
        {
            ...
            app = Attach(expectedProcessId);
            process = Process.GetProcessById(expectedProcessId);
            ProgressCreateNew(progressPath, "J01_R5_COLD_REOPEN_START " + DateTime.UtcNow.ToString("o"));
            int errors = 0, warnings = 0;
            ...
            receipt["status"] = "S05R2_J01_R5_NATIVE_1R_PILOT_COLD_REOPEN_PASS";
            receipt["gate_pass"] = true;
            ...
            receipt["normal_application_exit"] = true;
            WriteJsonCreateNew(receiptPath, receipt);
            Progress(progressPath, "J01_R5_COLD_REOPEN_PASS " + before);
            process.Dispose();
            return 0;
        }
        catch (Exception ex)
        {
            receipt["status"] = "S05R2_J01_R5_NATIVE_1R_PILOT_COLD_REOPEN_FAIL_CLOSED";
            receipt["gate_pass"] = false;
            receipt["exception_type"] = ex.GetType().FullName;
            receipt["exception"] = ex.ToString();
            ReleaseCom(driver); driver = null;
            ...
            try { if (app != null && app.GetDocumentCount() == 0) app.ExitApp(); } catch { }
            try { if (!File.Exists(receiptPath)) WriteJsonCreateNew(receiptPath, receipt); } catch { }
            try { if (File.Exists(progressPath)) Progress(progressPath, "J01_R5_COLD_REOPEN_FAIL " + ex.Message); } catch { }
            return 1;
        }
        finally
        {
