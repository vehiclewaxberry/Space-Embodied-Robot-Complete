...
$sessionRoot = Join-Path $root '07_VERIFICATION\AUTONOMOUS\S03B'
...
$readinessReceipt = Join-Path $sessionRoot 'B51R1_S03B_READINESS_RECEIPT.json'
$inputLock = Join-Path $sessionRoot 'B51R1_S03B_INPUT_LOCK.json'
$receipt = Join-Path $sessionRoot 'B51R1_S03B_BATCH_CARRIER_CREATE_RECEIPT.json'
$progress = Join-Path $sessionRoot 'B51R1_S03B_PROGRESS.log'
$gate = Join-Path $sessionRoot 'B51R1_S03B_REMAINING_NINE_CARRIER_GATE.json'
$sessionReceipt = Join-Path $sessionRoot 'B51R1_S03B_SESSION_RECEIPT.json'
...
$session = [ordered]@{
    schema='B51R1_S03B_SESSION_RECEIPT_V1'; session_id='S03B'
    started_at=[DateTimeOffset]::Now.ToString('o'); status='S03B_FAIL_CLOSED_NOT_STARTED'
...
try {
    ...
    $runtimeItems = @($lockedItems) + @([ordered]@{id='READINESS_RECEIPT';path='07_VERIFICATION/AUTONOMOUS/S03B/B51R1_S03B_READINESS_RECEIPT.json';bytes=(Get-Item $readinessReceipt).Length;sha256=Get-Sha $readinessReceipt})
    $lock = [ordered]@{
        schema='B51R1_S03B_INPUT_LOCK_V1'; generated_at=[DateTimeOffset]::Now.ToString('o')
        status='PASS_S03B_INPUTS_HASH_LOCKED'; session_id='S03B'
    ...
    $result = [B51R1S03BS04BBatchCarrierTool]::Create($launchedPid, $template, $carrierDir, $receipt, $progress)
    ...
    $gateData = [ordered]@{
        schema='B51R1_S03B_REMAINING_NINE_CARRIER_GATE_V1';generated_at=[DateTimeOffset]::Now.ToString('o')
        status='S03B_REMAINING_NINE_CARRIERS_CREATED';created_part_count=9
        creation_receipt=[ordered]@{path='07_VERIFICATION/AUTONOMOUS/S03B/B51R1_S03B_BATCH_CARRIER_CREATE_RECEIPT.json';bytes=(Get-Item $receipt).Length;sha256=Get-Sha $receipt}
        parts=$partEvidence
...
activation_gate: 07_VERIFICATION/AUTONOMOUS/S03B/B51R1_S03B_REMAINING_NINE_CARRIER_GATE.json
activated_slot:
  id: A07_S04B
  carrier_count: 10
...
"@
    ...
    $session['status']='PASS_S03B_REMAINING_NINE_CARRIERS_CREATED'
    ...
    $session['status']='S03B_FAIL_CLOSED'
