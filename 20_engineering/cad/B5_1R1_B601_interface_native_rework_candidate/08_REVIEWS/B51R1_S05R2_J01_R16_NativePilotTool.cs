...
public static class B51R1S05R2J01R16NativePilotTool
{
        ...
        out Dictionary<string, object> evidence)
    {
        ...
        IMateFeatureData status = null;
        Feature feature = null;
        object preCreateEntitiesRaw = null;
        Array preCreateEntitiesArray = null;
        ...
        try
        {
            ...
            preCreateEntitiesRaw = data.EntitiesToMate;
            preCreateEntitiesArray = preCreateEntitiesRaw as Array;
            preCreateReferenceEntity = data.ReferenceEntity;
            int selectionCountBeforeCreate = SelectedObjectCount(model);
            bool preCreateIsAdvancedMate = data.IsAdvancedMate;
            double preCreateAngle = data.Angle;
            double preCreateMinimumAngle = data.MinimumAngle;
            double preCreateMaximumAngle = data.MaximumAngle;
            int preCreateMateAlignment = data.MateAlignment;
            bool preCreateFlipDimension = data.FlipDimension;
            attempt["precreate_entities_to_mate_getter_diagnostic_only"] = true;
            attempt["precreate_entities_to_mate_getter_raw_summary"] = EntitySummary(preCreateEntitiesRaw);
            attempt["precreate_entities_to_mate_getter_is_null"] = preCreateEntitiesRaw == null;
            attempt["precreate_entities_to_mate_getter_is_array"] = preCreateEntitiesArray != null;
            attempt["precreate_entities_to_mate_getter_managed_type"] =
                preCreateEntitiesRaw == null ? null : preCreateEntitiesRaw.GetType().FullName;
            attempt["precreate_entities_to_mate"] = preCreateEntitiesArray == null
                ? null
                : preCreateEntitiesArray.Cast<object>().Select(EntitySummary).ToArray();
            attempt["precreate_entities_to_mate_getter_count"] = preCreateEntitiesArray == null
                ? null
                : (object)preCreateEntitiesArray.Length;
            attempt["precreate_entities_to_mate_getter_count_is_diagnostic_only"] = true;
            ...
            attempt["precreate_is_advanced_mate"] = preCreateIsAdvancedMate;
            attempt["precreate_angle_rad"] = preCreateAngle;
            attempt["precreate_minimum_angle_rad"] = preCreateMinimumAngle;
            attempt["precreate_maximum_angle_rad"] = preCreateMaximumAngle;
            attempt["precreate_mate_alignment"] = preCreateMateAlignment;
            attempt["precreate_flip_dimension"] = preCreateFlipDimension;
            Require(preCreateReferenceEntity == null,
                "Limit-angle pre-CreateMate ReferenceEntity getter must remain null: " + name);
            Require(selectionCountBeforeCreate == 0 && selectionCountBeforeCreate == preselectionCount,
                "Limit-angle selection changed before CreateMate: " + name);
            Require(preCreateIsAdvancedMate,
                "Limit-angle pre-CreateMate IsAdvancedMate mismatch: " + name);
            Require(Math.Abs(preCreateAngle - NativeQ0) <= JointTolerance,
                "Limit-angle pre-CreateMate initial value mismatch: " + name);
            Require(Math.Abs(preCreateMinimumAngle - NativeMinimum) <= LimitTolerance &&
                Math.Abs(preCreateMaximumAngle - NativeMaximum) <= LimitTolerance,
                "Limit-angle pre-CreateMate bounds mismatch: " + name);
            Require(preCreateMateAlignment == MateAlignAligned,
                "Limit-angle pre-CreateMate alignment mismatch: " + name);
            Require(!preCreateFlipDimension,
                "Limit-angle pre-CreateMate flip-dimension mismatch: " + name);
            attempt["create_mate_call_index"] = IncrementCounter(receipt, "create_mate_call_count");
            feature = assembly.CreateMate(rawData) as Feature;
            int createStatus = status.ErrorStatus;
            attempt["create_mate_error_status"] = createStatus;
        ...
        finally
        {
            ...
            if (preCreateEntitiesArray != null)
            {
                foreach (object entity in preCreateEntitiesArray)
                    if (!SameComIdentity(entity, parentLimit) && !SameComIdentity(entity, childZero))
                        ReleaseCom(entity);
            }
            else if (preCreateEntitiesRaw != null &&
                !SameComIdentity(preCreateEntitiesRaw, parentLimit) &&
                !SameComIdentity(preCreateEntitiesRaw, childZero))
            {
                ReleaseCom(preCreateEntitiesRaw);
            }
            readbackReferenceEntity = null; preCreateReferenceEntity = null;
            preCreateEntitiesArray = null; preCreateEntitiesRaw = null;
            ReleaseCom(readback);
            ReleaseCom(rawData);
        }
    }
        ...
        string outputAssemblyPath, string j00CheckpointPath, string receiptPath, string progressPath)
    {
        ...
        {
            { "schema", "B51R1_S05R2_J01_R16_NATIVE_PILOT_CREATE_V1" },
        ...
        try
        {
            ...
            ProgressCreateNew(progressPath, "J01_R16_CREATE_START " + DateTime.UtcNow.ToString("o"));
            ...
            {
                { "schema", "B51R1_S05R2_J01_R16_J00_ROOT_CHECKPOINT_V1" },
                { "status", "S05R2_J01_R16_J00_ROOT_BASELINE_PASS" },
            ...
            receipt["status"] = "S05R2_J01_R16_NATIVE_1R_PILOT_CREATE_PASS";
            ...
            Progress(progressPath, "J01_R16_CREATE_PASS " + outputEvidence["sha256"]);
            process.Dispose();
            return 0;
        }
        catch (Exception ex)
        {
            receipt["status"] = "S05R2_J01_R16_NATIVE_1R_PILOT_CREATE_FAIL_CLOSED";
            ...
            try { if (File.Exists(progressPath)) Progress(progressPath, "J01_R16_CREATE_FAIL " + ex.Message); } catch { }
            return 1;
        }
        finally
        {
        ...
        string receiptPath, string progressPath)
    {
        ...
        var receipt = new Dictionary<string, object>
        {
            { "schema", "B51R1_S05R2_J01_R16_NATIVE_PILOT_COLD_REOPEN_V1" },
            { "status", "FAIL_CLOSED_NOT_STARTED" }, { "generated_at_utc", DateTime.UtcNow.ToString("o") },
            { "expected_process_id", expectedProcessId }, { "save_as_call_count", 0 }, { "save3_call_count", 0 },
            { "create_mate_data_call_count", 0 }, { "create_mate_call_count", 0 }, { "add_mate5_call_count", 0 },
            { "mate_preselection_call_count", 0 }, { "transform2_call_count", 0 },
        ...
        try
        {
            ...
            app = Attach(expectedProcessId);
            process = Process.GetProcessById(expectedProcessId);
            ProgressCreateNew(progressPath, "J01_R16_COLD_REOPEN_START " + DateTime.UtcNow.ToString("o"));
            int errors = 0, warnings = 0;
            ...
            receipt["status"] = "S05R2_J01_R16_NATIVE_1R_PILOT_COLD_REOPEN_PASS";
            receipt["gate_pass"] = true;
            ...
            receipt["normal_application_exit"] = true;
            WriteJsonCreateNew(receiptPath, receipt);
            Progress(progressPath, "J01_R16_COLD_REOPEN_PASS " + before);
            process.Dispose();
            return 0;
        }
        catch (Exception ex)
        {
            receipt["status"] = "S05R2_J01_R16_NATIVE_1R_PILOT_COLD_REOPEN_FAIL_CLOSED";
            receipt["gate_pass"] = false;
            receipt["exception_type"] = ex.GetType().FullName;
            receipt["exception"] = ex.ToString();
            ReleaseCom(driver); driver = null;
            ...
            try { if (app != null && app.GetDocumentCount() == 0) app.ExitApp(); } catch { }
            try { if (!File.Exists(receiptPath)) WriteJsonCreateNew(receiptPath, receipt); } catch { }
            try { if (File.Exists(progressPath)) Progress(progressPath, "J01_R16_COLD_REOPEN_FAIL " + ex.Message); } catch { }
            return 1;
        }
        finally
        {
