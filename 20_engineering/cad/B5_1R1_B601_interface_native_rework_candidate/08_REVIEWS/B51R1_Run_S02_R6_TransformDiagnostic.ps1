...
$sessionRoot = Join-Path $root '07_VERIFICATION\AUTONOMOUS\S02_R6'
...
$readinessReceipt = Join-Path $sessionRoot 'B51R1_S02_R6_ONE_SHOT_READINESS_PROBE_RECEIPT.json'
$inputLock = Join-Path $sessionRoot 'B51R1_S02_R6_TRANSFORM_DIAGNOSTIC_INPUT_LOCK.json'
$receipt = Join-Path $sessionRoot 'B51R1_S02_R6_TRANSFORM_DIAGNOSTIC_RECEIPT.json'
$progress = Join-Path $sessionRoot 'B51R1_S02_R6_TRANSFORM_DIAGNOSTIC_PROGRESS.log'
$sessionReceipt = Join-Path $sessionRoot 'B51R1_S02_R6_TRANSFORM_DIAGNOSTIC_SESSION_RECEIPT.json'
$toolDll = Join-Path $PSScriptRoot 'B51R1_S02_R6_TransformConventionProbe_V4.dll'
...
$lockedItems = @(
    ...
    [ordered]@{id='S02_R5_READINESS_RECEIPT';path='07_VERIFICATION/AUTONOMOUS/S02_R5/B51R1_S02_R5_WAITING_READINESS_PROBE_RECEIPT.json';bytes=823;sha256='D43F94D11BA66417D23300CD0C2880E9B27F74DEA91D34E47D552198A91D5463'},
    [ordered]@{id='S02_R5_SESSION_RECEIPT';path='07_VERIFICATION/AUTONOMOUS/S02_R5/B51R1_S02_R5_TRANSFORM_DIAGNOSTIC_SESSION_RECEIPT.json';bytes=823;sha256='120DF3CA0F6D425A21CAAF938E329794796C962CAFB675EF34275A3F7DB25CE3'},
    [ordered]@{id='S02_R5_POLLING_INCIDENT';path='07_VERIFICATION/AUTONOMOUS/S02_R5/B51R1_S02_R5_HIGH_FREQUENCY_STARTUP_POLLING_FAIL_CLOSED_INCIDENT.json';bytes=2471;sha256='B8734E6758D61D3ECCB8C9B5D3550CDBA5FC53B7605D157A9F3CEF52CF45532A'},
    [ordered]@{id='POOL_A_A17_EXTENSION';path='04_CONFIGURATION/B51R1_AUTONOMOUS_SESSION_PLAN_V8_POOL_A_S02_R6_SPARSE_DELAYED_TRANSFORM_DIAGNOSTIC_EXTENSION.yaml';bytes=1759;sha256='5FC794BC9B70F25F4C4826B70890DAB8EDDDF9CE6A6A76130072D7B90886731B'},
    [ordered]@{id='S02_R6_AUTHORIZATION';path='07_VERIFICATION/AUTONOMOUS/S02_R6/B51R1_S02_R6_TRANSFORM_DIAGNOSTIC_OWNER_AUTHORIZATION.json';bytes=1424;sha256='7A8A21F7DED8BF59F529BC08AF111B40D50C9439C18EB26300A8E80C55EA98B6'},
    [ordered]@{id='S02_R6_ADMISSION';path='07_VERIFICATION/AUTONOMOUS/S02_R6/B51R1_S02_R6_TRANSFORM_DIAGNOSTIC_ADMISSION.json';bytes=1277;sha256='BF42A55D089E6D8F36449B1CA239925F5EE90192F5E8EDB22736C77C2C18640A'},
    [ordered]@{id='TRANSFORM_PROBE_SOURCE_V4';path='08_REVIEWS/B51R1_S02_R6_TransformConventionProbe_V4.cs';bytes=25944;sha256='B376CA42213352C5846988E8EEC12BB55E2543A7EEBFCDDA3B96F169004CFE2A'},
    [ordered]@{id='TRANSFORM_PROBE_BINARY_V4';path='08_REVIEWS/B51R1_S02_R6_TransformConventionProbe_V4.dll';bytes=21504;sha256='6DF423449B875BB456A148A1BB8CCFC642D47C30121675E5705A0A94741ACF05'},
    ...
    [ordered]@{id='TOOL_STATIC_RELEASE_V4';path='08_REVIEWS/B51R1_S02_R6_TRANSFORM_DIAGNOSTIC_TOOL_STATIC_RELEASE_V4.json';bytes=2411;sha256='61058588EC8AEEA78C007A7AF6D85CBAFC40937281DAA9E83A5E3365D0FBBD41'}
)

foreach ($output in @($readinessReceipt, $inputLock, $receipt, $progress, $sessionReceipt)) {
    if (Test-Path -LiteralPath $output) {
        throw "S02_R6 append-only output already exists: $output"
    }
}
if (@(Get-Process -Name SLDWORKS -ErrorAction SilentlyContinue).Count -ne 0) {
    throw 'S02_R6 requires zero existing SLDWORKS processes before launch'
}
...
$session = [ordered]@{
    schema = 'B51R1_S02_R6_TRANSFORM_DIAGNOSTIC_SESSION_RECEIPT_V1'
    session_id = 'S02_R6'
    started_at = [DateTimeOffset]::Now.ToString('o')
    status = 'S02_R6_FAIL_CLOSED_NOT_STARTED'
...
try {
    ...
    $quietWindowStarted = [DateTimeOffset]::Now
    Start-Sleep -Seconds 60
    $quietWindowEnded = [DateTimeOffset]::Now
    $processesAfterQuietWindow = @(Get-Process -Name SLDWORKS -ErrorAction SilentlyContinue)
    if ($processesAfterQuietWindow.Count -ne 1) {
        throw 'S02_R6 lost the sole SolidWorks process during the zero-COM quiet window'
    }
    $processAfterQuietWindow = $processesAfterQuietWindow[0]
    $processAfterQuietWindow.Refresh()
    if ($processAfterQuietWindow.Id -ne $launchedPid -or
        -not $processAfterQuietWindow.Responding -or
        $processAfterQuietWindow.MainWindowHandle -eq [IntPtr]::Zero) {
        throw 'S02_R6 SolidWorks process is not the same visible responsive process after the quiet window'
    }
    $session['zero_com_quiet_window_started_at'] = $quietWindowStarted.ToString('o')
    $session['zero_com_quiet_window_ended_at'] = $quietWindowEnded.ToString('o')
    $session['zero_com_quiet_window_elapsed_seconds'] =
        ($quietWindowEnded - $quietWindowStarted).TotalSeconds
    $session['solidworks_com_call_count_before_readiness_probe'] = 0
    ...
    if ($readinessResult -ne 0) {
        throw "S02_R6 one-shot readiness probe returned $readinessResult"
    }
    ...
    if ($readiness.status -ne 'PASS_EXISTING_SINGLE_VISIBLE_RESPONSIVE_DOCUMENT_EMPTY_SESSION') {
        throw "S02_R6 one-shot readiness probe did not pass: $($readiness.status)"
    }
    $revision = [string]$readiness.solidworks_revision
    $runtimeLockedItems = @($lockedItems) + @(
        [ordered]@{
            id = 'S02_R6_ONE_SHOT_READINESS_RECEIPT'
            path = '07_VERIFICATION/AUTONOMOUS/S02_R6/B51R1_S02_R6_ONE_SHOT_READINESS_PROBE_RECEIPT.json'
    ...
    $lock = [ordered]@{
        schema = 'B51R1_S02_R6_TRANSFORM_DIAGNOSTIC_INPUT_LOCK_V1'
        generated_at = [DateTimeOffset]::Now.ToString('o')
        status = 'PASS_S02_R6_TRANSFORM_DIAGNOSTIC_INPUTS_HASH_LOCKED'
        session_id = 'S02_R6'
        runtime_boundary = [ordered]@{
            ...
            pre_readiness_solidworks_com_call_count = 0
            zero_com_quiet_window_seconds =
                ($quietWindowEnded - $quietWindowStarted).TotalSeconds
            one_shot_readiness_probe_call_count = 1
        }
        transaction = [ordered]@{
            ...
            receipt = '07_VERIFICATION/AUTONOMOUS/S02_R6/B51R1_S02_R6_TRANSFORM_DIAGNOSTIC_RECEIPT.json'
            progress_log = '07_VERIFICATION/AUTONOMOUS/S02_R6/B51R1_S02_R6_TRANSFORM_DIAGNOSTIC_PROGRESS.log'
        ...
        claim_limit = 'S02_R6_DIAGNOSTIC_INPUT_LOCK_ONLY_NO_TRANSFORM_CLASSIFICATION_S02_PASS_ACTIVE_CANDIDATE_OR_S03_CREDIT'
    }
    ...
    $result = [B51R1S02R6TransformConventionProbeV4]::Run(
        $target,
        $stageA,
        $receipt,
        $progress,
        ...
        $expectedStageA)
    if ($result -ne 0) {
        throw "S02_R6 transform probe returned $result"
    }
    ...
    if ($probe.status -ne 'PASS_READ_ONLY_TRANSFORM_CONVENTION_EVIDENCE_CAPTURED') {
        throw "S02_R6 transform probe did not pass evidence capture: $($probe.status)"
    }
    if (@(Get-Process -Name SLDWORKS -ErrorAction SilentlyContinue).Count -ne 0) {
        throw 'SolidWorks remains after transform probe requested normal ExitApp'
    }
    Assert-File $target 232433 $expectedTarget 'R1 target after S02_R6'
    Assert-File $stageA 511198 $expectedStageA 'Protected Stage A after S02_R6'
    $session['status'] = 'PASS_S02_R6_TRANSFORM_CONVENTION_EVIDENCE_CAPTURED'
    ...
    $session['status'] = 'S02_R6_FAIL_CLOSED'
