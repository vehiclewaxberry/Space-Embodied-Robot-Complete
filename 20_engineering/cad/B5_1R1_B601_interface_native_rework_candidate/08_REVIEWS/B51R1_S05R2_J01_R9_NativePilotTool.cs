...
public static class B51R1S05R2J01R9NativePilotTool
{
    ...
    private static bool SameMateEntityIdentity(object first, object second)
    {
        if (first == null || second == null) return false;
        string firstPath, firstToken, firstName, firstType;
        string secondPath, secondToken, secondName, secondType;
        return TrySelectionIdentity(first, out firstPath, out firstToken, out firstName, out firstType) &&
            TrySelectionIdentity(second, out secondPath, out secondToken, out secondName, out secondType) &&
            firstPath.Equals(secondPath, StringComparison.OrdinalIgnoreCase) &&
            firstToken.Equals(secondToken, StringComparison.OrdinalIgnoreCase) &&
            firstName.Equals(secondName, StringComparison.Ordinal) &&
            firstType.Equals(secondType, StringComparison.Ordinal);
    }
    ...
    {
        return actual.Length == 2 &&
            ((SameMateEntityIdentity(actual[0], first) && SameMateEntityIdentity(actual[1], second)) ||
             (SameMateEntityIdentity(actual[0], second) && SameMateEntityIdentity(actual[1], first)));
    }
        ...
        Dictionary<string, object> attempt, out Dictionary<string, object> evidence)
    {
        ...
        try
        {
            ...
            bool semanticPairMatch = SameUnorderedPair(entities, first, second);
            ...
            attempt["definition_identity_authority"] = "IFeature.GetNameForSelection_PATH_TOKEN_NAME_TYPE";
            ...
            evidence["definition_identity_authority"] = "IFeature.GetNameForSelection_PATH_TOKEN_NAME_TYPE";
        ...
        Dictionary<string, object> attempt, out Dictionary<string, object> evidence)
    {
        ...
        try
        {
            ...
            bool axisSemanticMatch = SameUnorderedPair(axes, parentAxis, childAxis);
            bool seatSemanticMatch = SameUnorderedPair(seats, parentSeat, childSeat);
        ...
        out Dictionary<string, object> evidence)
    {
        ...
        try
        {
            ...
            bool entitySemanticMatch = SameUnorderedPair(entities, parentLimit, childZero);
            bool referenceIunknownMatch = SameComIdentity(readback.ReferenceEntity, parentAxis);
            bool referenceSemanticMatch = SameMateEntityIdentity(readback.ReferenceEntity, parentAxis);
            attempt["definition_entities"] = entities.Select(EntitySummary).ToArray();
            attempt["definition_reference_entity"] = EntitySummary(readback.ReferenceEntity);
            ...
            attempt["definition_reference_semantic_match"] = referenceSemanticMatch;
            attempt["definition_identity_authority"] = "IFeature.GetNameForSelection_PATH_TOKEN_NAME_TYPE";
            Require(entitySemanticMatch, "Limit angle semantic entity readback mismatch: " + name);
            Require(referenceSemanticMatch, "Limit angle semantic reference-axis readback mismatch: " + name);
            ...
            evidence["definition_entity_count"] = entities.Length;
            evidence["definition_entities"] = entities.Select(EntitySummary).ToArray();
            evidence["reference_entity"] = EntitySummary(readback.ReferenceEntity);
            ...
            evidence["definition_semantic_pair_match"] = true;
            evidence["reference_semantic_match"] = true;
            evidence["definition_identity_authority"] = "IFeature.GetNameForSelection_PATH_TOKEN_NAME_TYPE";
        ...
        Component2 parent, Component2 child)
    {
        ...
        try
        {
            ...
            Require(MatchesContextSelection(referenceEntity,
                "AXIS_joint1_PARENT_SIDE", "RefAxis", "AXIS", parent, model),
                "Native limit-angle parent-axis semantic binding mismatch");
            ...
            return new Dictionary<string, object>
            {
                { "binding_authority", "Persisted MateFeatureData plus IFeature.GetNameForSelection path/token/name/type and component instance" },
        ...
        string outputAssemblyPath, string j00CheckpointPath, string receiptPath, string progressPath)
    {
        ...
        {
            { "schema", "B51R1_S05R2_J01_R9_NATIVE_PILOT_CREATE_V1" },
        ...
        try
        {
            ...
            ProgressCreateNew(progressPath, "J01_R9_CREATE_START " + DateTime.UtcNow.ToString("o"));
            ...
            {
                { "schema", "B51R1_S05R2_J01_R9_J00_ROOT_CHECKPOINT_V1" },
                { "status", "S05R2_J01_R9_J00_ROOT_BASELINE_PASS" },
            ...
            receipt["status"] = "S05R2_J01_R9_NATIVE_1R_PILOT_CREATE_PASS";
            ...
            Progress(progressPath, "J01_R9_CREATE_PASS " + Sha256(outputAssemblyPath));
            process.Dispose();
            return 0;
        }
        catch (Exception ex)
        {
            receipt["status"] = "S05R2_J01_R9_NATIVE_1R_PILOT_CREATE_FAIL_CLOSED";
            ...
            try { if (File.Exists(progressPath)) Progress(progressPath, "J01_R9_CREATE_FAIL " + ex.Message); } catch { }
            return 1;
        }
        finally
        {
        ...
        string receiptPath, string progressPath)
    {
        ...
        var receipt = new Dictionary<string, object>
        {
            { "schema", "B51R1_S05R2_J01_R9_NATIVE_PILOT_COLD_REOPEN_V1" },
            { "status", "FAIL_CLOSED_NOT_STARTED" }, { "generated_at_utc", DateTime.UtcNow.ToString("o") },
            { "expected_process_id", expectedProcessId }, { "save_as_call_count", 0 }, { "save3_call_count", 0 },
            { "create_mate_data_call_count", 0 }, { "create_mate_call_count", 0 }, { "add_mate5_call_count", 0 },
            { "mate_preselection_call_count", 0 }, { "transform2_call_count", 0 },
        ...
        try
        {
            ...
            app = Attach(expectedProcessId);
            process = Process.GetProcessById(expectedProcessId);
            ProgressCreateNew(progressPath, "J01_R9_COLD_REOPEN_START " + DateTime.UtcNow.ToString("o"));
            int errors = 0, warnings = 0;
            ...
            receipt["status"] = "S05R2_J01_R9_NATIVE_1R_PILOT_COLD_REOPEN_PASS";
            receipt["gate_pass"] = true;
            ...
            receipt["normal_application_exit"] = true;
            WriteJsonCreateNew(receiptPath, receipt);
            Progress(progressPath, "J01_R9_COLD_REOPEN_PASS " + before);
            process.Dispose();
            return 0;
        }
        catch (Exception ex)
        {
            receipt["status"] = "S05R2_J01_R9_NATIVE_1R_PILOT_COLD_REOPEN_FAIL_CLOSED";
            receipt["gate_pass"] = false;
            receipt["exception_type"] = ex.GetType().FullName;
            receipt["exception"] = ex.ToString();
            ReleaseCom(driver); driver = null;
            ...
            try { if (app != null && app.GetDocumentCount() == 0) app.ExitApp(); } catch { }
            try { if (!File.Exists(receiptPath)) WriteJsonCreateNew(receiptPath, receipt); } catch { }
            try { if (File.Exists(progressPath)) Progress(progressPath, "J01_R9_COLD_REOPEN_FAIL " + ex.Message); } catch { }
            return 1;
        }
        finally
        {
