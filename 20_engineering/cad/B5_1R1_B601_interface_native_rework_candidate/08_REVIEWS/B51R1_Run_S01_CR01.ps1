...
$inputLock = Join-Path $sessionRoot 'B51R1_S01_CR01_INPUT_LOCK_SUPPLEMENT.json'
...
function Wait-SoleVisibleSolidWorks([Diagnostics.Process]$StartedProcess) {
    $deadline = [DateTime]::UtcNow.AddSeconds(120)
    do {
        Start-Sleep -Milliseconds 500
        $all = @(Get-Process -Name SLDWORKS -ErrorAction SilentlyContinue)
        if ($all.Count -gt 1) {
            throw "More than one SLDWORKS process observed during startup: $($all.Count)"
        }
        if ($all.Count -eq 1) {
            $candidate = $all[0]
            $candidate.Refresh()
            if ($candidate.Id -ne $StartedProcess.Id) {
                throw "Started PID $($StartedProcess.Id), but sole SLDWORKS PID is $($candidate.Id)"
            }
            if ($candidate.Responding -and $candidate.MainWindowHandle -ne [IntPtr]::Zero) {
                return $candidate.Id
            }
        }
    } while ([DateTime]::UtcNow -lt $deadline)
    throw 'SolidWorks did not become a sole visible responsive process within 120 seconds'
}
...
if (@(Get-Process -Name SLDWORKS -ErrorAction SilentlyContinue).Count -ne 0) {
    throw 'S01_CR01 requires zero SLDWORKS processes before the runner launches its sole visible session'
}
...
$session = [ordered]@{
    ...
    solidworks_process_count_before_launch = 0
    launch_new_visible_process = $true
    force_termination_path_present = $false
}

$exitCode = 1
try {
    Write-Host 'S01_CR01: launching one visible SolidWorks process...'
    $started = Start-Process -FilePath $solidworksExe -PassThru
    $expectedProcessId = Wait-SoleVisibleSolidWorks $started
    $session['solidworks_process_id'] = $expectedProcessId
    $session['solidworks_visible_and_responding'] = $true
    Write-Host "S01_CR01: attached process precondition ready (PID $expectedProcessId)."
