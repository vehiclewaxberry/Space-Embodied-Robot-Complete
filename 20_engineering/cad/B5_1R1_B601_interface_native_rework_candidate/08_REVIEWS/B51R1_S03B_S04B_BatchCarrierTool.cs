...
public static class B51R1S03BS04BBatchCarrierTool
{
    ...
    private static int FeatureTypeCount(ModelDoc2 model, string type)
    {
        ...
        try
        {
            while (cursor != null)
            {
                if (cursor.GetTypeName2() == type) count++;
                Feature next = cursor.GetNextFeature() as Feature;
                ReleaseCom(cursor);
                cursor = next;
            }
    ...
    private static Feature FindUniqueFeature(ModelDoc2 model, string name, string type)
    {
        ...
        try
        {
            ...
            {
                Feature next = cursor.GetNextFeature() as Feature;
                if (cursor.Name == name && cursor.GetTypeName2() == type)
                {
                    count++;
    ...
    private static Dictionary<int, Feature> DefaultPlanes(ModelDoc2 model)
    {
        ...
        try
        {
            ...
            {
                Feature next = cursor.GetNextFeature() as Feature;
                if (cursor.GetTypeName2() == "RefPlane")
                {
                    RefPlane plane = null;
                    MathTransform transform = null;
                    try
                    {
                        plane = cursor.GetSpecificFeature2() as RefPlane;
                        Require(plane != null, "Default IRefPlane unavailable");
                        transform = plane.Transform;
                        double[] raw = ToArray(transform.ArrayData);
                        Require(raw.Length == 16, "Default plane transform length mismatch");
                        int axis = 0;
                        double maximum = Math.Abs(raw[6]);
                        if (Math.Abs(raw[7]) > maximum) { axis = 1; maximum = Math.Abs(raw[7]); }
                        if (Math.Abs(raw[8]) > maximum) { axis = 2; maximum = Math.Abs(raw[8]); }
                        Require(Math.Abs(1.0 - maximum) <= DirectionTolerance,
                            "Default plane is not axis aligned: " + cursor.Name);
    ...
    private static Feature AddAxisFromPlanes(ModelDoc2 model, string name,
        Feature planeA, Feature planeB)
    {
        ...
        try
        {
            ...
            {
                Feature next = cursor.GetNextFeature() as Feature;
                if (cursor.GetTypeName2() == "RefAxis")
                {
                    count++;
                    if (match == null) { match = cursor; cursor = null; }
                }
            ...
            Require(count == 1 && match != null, "Expected one new reference axis: " + name);
            match.Name = name;
            return match;
        }
        finally { ReleaseCom(cursor); }
    ...
    private static void AddJointReferences(ModelDoc2 model, CarrierSpec spec)
    {
        ...
        try
        {
            ...
            Feature axis = AddAxisFromPlanes(model, "AXIS_" + spec.IncomingJoint,
