...
$toolDll = Join-Path $PSScriptRoot 'B51R1_S05R2_R1_DriverReadyCarrierTool.dll'
...
function Read-J01R1Gate {
    $relativeGate = '07_VERIFICATION/AUTONOMOUS/S05R2_J01_R1/B51R1_S05R2_J01_R1_NATIVE_1R_GATE.json'
    $relativeBindings = [ordered]@{
        create_gate='07_VERIFICATION/AUTONOMOUS/S05R2_J01_R1/B51R1_S05R2_J01_R1_CREATE_GATE.json'
        receipt='07_VERIFICATION/AUTONOMOUS/S05R2_J01_R1/B51R1_S05R2_J01_R1_COLD_REOPEN_RECEIPT.json'
        output='03_CAD/20_NATIVE_ASSEMBLY/PILOT/B51R1_CARRIER_J01_R1_NATIVE_PILOT.SLDASM'
    }
    $path = Join-Path $root $relativeGate
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) { throw "J01 R1 authoritative Gate missing: $path" }
    $data = Get-Content -Raw -Encoding UTF8 -LiteralPath $path | ConvertFrom-Json
    $requiredTopLevel = @('schema','status','gate_pass','mr2_batch_create_authorized','create_gate','receipt','cold_reopen_receipt','output')
    foreach ($name in $requiredTopLevel) {
        if ($data.PSObject.Properties.Name -notcontains $name) { throw "J01 R1 Gate missing field: $name" }
    }
    if ($data.schema -ne 'B51R1_S05R2_J01_R1_NATIVE_1R_GATE_V1' -or
        $data.status -ne 'S05R2_J01_R1_NATIVE_1R_PILOT_PASS' -or
        $data.gate_pass -isnot [bool] -or $data.gate_pass -ne $true -or
        $data.mr2_batch_create_authorized -isnot [bool] -or $data.mr2_batch_create_authorized -ne $true) {
        throw "J01 R1 authoritative Gate contract mismatch: $path"
    }
    if ($data.formal_s05_authorized -isnot [bool] -or $data.formal_s05_authorized -ne $false -or
        $data.t005_authorized -isnot [bool] -or $data.t005_authorized -ne $false) {
        throw 'J01 R1 Gate exceeds its Pilot-only claim boundary'
    }

    $lockedItems = @(
        [ordered]@{id='J01_R1_AUTHORITATIVE_GATE';path=$relativeGate;bytes=(Get-Item -LiteralPath $path).Length;sha256=Get-Sha $path}
    )
    foreach ($bindingName in @('create_gate','receipt','output')) {
        $binding = $data.$bindingName
        foreach ($field in @('path','bytes','sha256')) {
            if ($binding.PSObject.Properties.Name -notcontains $field) { throw "J01 R1 $bindingName binding missing field: $field" }
        }
        $expectedPath = Join-Path $root ([string]$relativeBindings[$bindingName])
        if ([IO.Path]::GetFullPath([string]$binding.path) -ne [IO.Path]::GetFullPath($expectedPath)) {
            throw "J01 R1 $bindingName path mismatch"
        }
        Assert-File $expectedPath ([long]$binding.bytes) ([string]$binding.sha256) "J01_R1_$($bindingName.ToUpperInvariant())"
        $id = switch ($bindingName) {
            'create_gate' { 'J01_R1_CREATE_GATE' }
            'receipt' { 'J01_R1_COLD_RECEIPT' }
            'output' { 'J01_R1_PILOT_ASSEMBLY' }
        }
        $lockedItems += [ordered]@{id=$id;path=[string]$relativeBindings[$bindingName];bytes=[long]$binding.bytes;sha256=[string]$binding.sha256}
    }

    $coldBinding = $data.cold_reopen_receipt
    foreach ($field in @('path','bytes','sha256')) {
        if ($coldBinding.PSObject.Properties.Name -notcontains $field) { throw "J01 R1 cold_reopen_receipt binding missing field: $field" }
    }
    if ([IO.Path]::GetFullPath([string]$coldBinding.path) -ne [IO.Path]::GetFullPath([string]$data.receipt.path) -or
        [long]$coldBinding.bytes -ne [long]$data.receipt.bytes -or
        [string]$coldBinding.sha256 -ne [string]$data.receipt.sha256) {
        throw 'J01 R1 cold-reopen receipt aliases do not have identical bindings'
    }

    $createData = Get-Content -Raw -Encoding UTF8 -LiteralPath (Join-Path $root $relativeBindings.create_gate) | ConvertFrom-Json
    if ($createData.schema -ne 'B51R1_S05R2_J01_R1_NATIVE_1R_CREATE_GATE_V1' -or
        $createData.status -ne 'S05R2_J01_R1_NATIVE_1R_PILOT_CREATE_PASS' -or
        $createData.gate_pass -isnot [bool] -or $createData.gate_pass -ne $true -or
        $createData.cold_reopen_authorized -isnot [bool] -or $createData.cold_reopen_authorized -ne $true) {
        throw 'J01 R1 Create Gate contract mismatch'
    }
    if ($createData.PSObject.Properties.Name -notcontains 'output' -or
        [IO.Path]::GetFullPath([string]$createData.output.path) -ne [IO.Path]::GetFullPath([string]$data.output.path) -or
        [long]$createData.output.bytes -ne [long]$data.output.bytes -or
        [string]$createData.output.sha256 -ne [string]$data.output.sha256) {
        throw 'J01 R1 Create Gate output binding differs from the authoritative Gate'
    }

    $receiptData = Get-Content -Raw -Encoding UTF8 -LiteralPath (Join-Path $root $relativeBindings.receipt) | ConvertFrom-Json
    if ($receiptData.schema -ne 'B51R1_S05R2_J01_R1_NATIVE_PILOT_COLD_REOPEN_V1' -or
        $receiptData.status -ne 'S05R2_J01_R1_NATIVE_1R_PILOT_COLD_REOPEN_PASS' -or
        $receiptData.gate_pass -isnot [bool] -or $receiptData.gate_pass -ne $true -or
        $receiptData.normal_application_exit -isnot [bool] -or $receiptData.normal_application_exit -ne $true) {
        throw 'J01 R1 cold-reopen receipt contract mismatch'
    }
    if ($receiptData.PSObject.Properties.Name -notcontains 'assembly' -or
        [IO.Path]::GetFullPath([string]$receiptData.assembly.path) -ne [IO.Path]::GetFullPath([string]$data.output.path) -or
        [long]$receiptData.assembly.bytes -ne [long]$data.output.bytes -or
        [string]$receiptData.assembly.sha256_before_after -ne [string]$data.output.sha256) {
        throw 'J01 R1 cold-reopen receipt assembly binding differs from the authoritative Gate'
    }

    [ordered]@{
        path=$path;bytes=(Get-Item -LiteralPath $path).Length;sha256=Get-Sha $path
        status=$data.status;data=$data;locked_items=$lockedItems
    }
}
...
$modeData = switch ($Mode) {
    'PilotCreate' {
        [ordered]@{
            session='S05R2_P_R1';expectedStatus='S05R2_P_R1_BASE_AND_LINK1_MR2_CREATED';expectedCount=2;expectedSaveAs=2
            targetLinks=@('base_link','link1');receipt='B51R1_S05R2_P_R1_PILOT_CREATE_RECEIPT.json';progress='B51R1_S05R2_P_R1_PROGRESS.log'
            gate='B51R1_S05R2_P_R1_MR2_PILOT_CREATE_GATE.json'
            nextActivation='04_CONFIGURATION/B51R1_AUTONOMOUS_SESSION_PLAN_V22R2A_POOL_A_A08R2_PC_R1_MR2_PILOT_COLD_REOPEN_ACTIVATION.yaml'
        }
    }
    'PilotVerify' {
        [ordered]@{
            session='S05R2_PC_R1';expectedStatus='S05R2_PC_R1_BASE_AND_LINK1_MR2_COLD_REOPEN_PASS';expectedCount=2;expectedSaveAs=0
            targetLinks=@('base_link','link1');receipt='B51R1_S05R2_PC_R1_PILOT_COLD_REOPEN_RECEIPT.json';progress='B51R1_S05R2_PC_R1_PROGRESS.log'
            gate='B51R1_S05R2_PC_R1_MR2_PILOT_COLD_REOPEN_GATE.json'
            nextActivation='04_CONFIGURATION/B51R1_AUTONOMOUS_SESSION_PLAN_V22R3A_POOL_A_A08R2_J01_R1_MR2_NATIVE_1R_PILOT_ACTIVATION.yaml'
        }
    }
    'BatchCreate' {
        [ordered]@{
            session='S05R2_B_R1';expectedStatus='S05R2_B_R1_REMAINING_EIGHT_MR2_CREATED';expectedCount=8;expectedSaveAs=8
            targetLinks=@('link2','link3','link4','link5','link6','gripper_link','gripper_left','gripper_right')
            receipt='B51R1_S05R2_B_R1_BATCH_CREATE_RECEIPT.json';progress='B51R1_S05R2_B_R1_PROGRESS.log'
            gate='B51R1_S05R2_B_R1_REMAINING_EIGHT_CREATE_GATE.json'
            nextActivation='04_CONFIGURATION/B51R1_AUTONOMOUS_SESSION_PLAN_V23R1A_POOL_A_A08R2_BC_R1_ALL_MR2_COLD_REOPEN_ACTIVATION.yaml'
        }
    }
    'AllVerify' {
        [ordered]@{
            session='S05R2_BC_R1';expectedStatus='S05R2_BC_R1_ALL_TEN_MR2_COLD_REOPEN_PASS';expectedCount=10;expectedSaveAs=0
            targetLinks=$allParts;receipt='B51R1_S05R2_BC_R1_ALL_COLD_REOPEN_RECEIPT.json';progress='B51R1_S05R2_BC_R1_PROGRESS.log'
            gate='B51R1_S05R2_BC_R1_ALL_TEN_COLD_REOPEN_GATE.json'
            nextActivation='04_CONFIGURATION/B51R1_AUTONOMOUS_SESSION_PLAN_V24R1A_POOL_A_A08_S05_NATIVE_CHAIN_REACTIVATION_WITH_MR2_R1.yaml'
        }
    }
}
...
$commonLocks = @(
    ...
    [ordered]@{id='MR2_V1_TOOL_SOURCE';path='08_REVIEWS/B51R1_S05R2_DriverReadyCarrierTool.cs';bytes=53299;sha256='B357F3F6BA5D9F420CDD4AE32F96F65DD417771D17F6F82D127AAB40077ADF50'},
    [ordered]@{id='MR2_V1_TOOL_BINARY';path='08_REVIEWS/B51R1_S05R2_DriverReadyCarrierTool.dll';bytes=43008;sha256='32945766072FF821AF21B0E91512453A3232D793215D18C2888D32653AF81BD4'},
    [ordered]@{id='MR2_V1_RUNNER';path='08_REVIEWS/B51R1_Run_S05R2_DriverReadyCarriers.ps1';bytes=21532;sha256='759BE4BB8347DF93CB279822A5DA202B4F637268530E2F8C2D9365307468C86B'},
    [ordered]@{id='MR2_V1_STATIC_RELEASE';path='08_REVIEWS/B51R1_S05R2_DRIVER_READY_CARRIER_TOOL_STATIC_RELEASE_V1.json';bytes=1819;sha256='CAEB8C403EA62F710741EEA5C1E52BD339A4A6CDACE99776C27EEE8561B50F59'},
    [ordered]@{id='MR2_V1_FAILURE_INPUT_LOCK';path='07_VERIFICATION/AUTONOMOUS/S05R2_P/B51R1_S05R2_P_INPUT_LOCK.json';bytes=4915;sha256='719B88D997D1506294A913BD4E68956E19F3EFC37E11DBD0C47CE047E199928A'},
    [ordered]@{id='MR2_V1_FAILURE_RECEIPT';path='07_VERIFICATION/AUTONOMOUS/S05R2_P/B51R1_S05R2_P_PILOT_CREATE_RECEIPT.json';bytes=1898;sha256='1C20D66AC34DB78B5DFD6731F17ED4805105C5E88276CEA089FC50A2BB5E1EF8'},
    [ordered]@{id='MR2_V1_FAILURE_HOLD';path='07_VERIFICATION/AUTONOMOUS/S05R2_P/B51R1_S05R2_P_STAGING_PATH_LENGTH_HOLD_GATE.json';bytes=1389;sha256='47AD62A62FAFE3467DCEF802BBC79C855C3125BD6D143DB3D609DDE63EAE828C'},
    [ordered]@{id='MR2_R1_RECOVERY_ACTIVATION';path='04_CONFIGURATION/B51R1_AUTONOMOUS_SESSION_PLAN_V22R1A_POOL_A_A08R2_MR2_SHORT_STAGING_RECOVERY_ACTIVATION.yaml';bytes=694;sha256='850F4D94CAA0F81708F62568CB131BD52FD39C0E75BAFFE21F4DC57C2701B470'},
    [ordered]@{id='MR2_R1_TOOL_SOURCE';path='08_REVIEWS/B51R1_S05R2_R1_DriverReadyCarrierTool.cs';bytes=53583;sha256='3101116E7813D26FDC6E1B222806C3D9F4000204499C4247EE7C38B8ECE0721B'},
    [ordered]@{id='MR2_R1_TOOL_BINARY';path='08_REVIEWS/B51R1_S05R2_R1_DriverReadyCarrierTool.dll';bytes=43008;sha256='18A9C2B1068F812F4B98ED534E2542C26F46412FB9F496F122553918FB9855C4'},
...
switch ($Mode) {
    'PilotVerify' {
        $record = Read-PassGate '07_VERIFICATION/AUTONOMOUS/S05R2_P_R1/B51R1_S05R2_P_R1_MR2_PILOT_CREATE_GATE.json' 'S05R2_P_R1_BASE_AND_LINK1_MR2_CREATED'
        Assert-GateParts $record @('base_link','link1') 'MR2_PILOT_CREATE'; $prerequisites += $record
    }
    'BatchCreate' {
        $j01 = Read-J01R1Gate
        $j01LockedItems = @($j01.locked_items)
    ...
    'AllVerify' {
        $j01 = Read-J01R1Gate
        $j01LockedItems = @($j01.locked_items)
        $pilot = Read-PassGate '07_VERIFICATION/AUTONOMOUS/S05R2_PC_R1/B51R1_S05R2_PC_R1_MR2_PILOT_COLD_REOPEN_GATE.json' 'S05R2_PC_R1_BASE_AND_LINK1_MR2_COLD_REOPEN_PASS'
        $batch = Read-PassGate '07_VERIFICATION/AUTONOMOUS/S05R2_B_R1/B51R1_S05R2_B_R1_REMAINING_EIGHT_CREATE_GATE.json' 'S05R2_B_R1_REMAINING_EIGHT_MR2_CREATED'
        ...
        $prerequisites += @($j01,$pilot,$batch)
    }
}
$runLocks = @($commonLocks) + @($j01LockedItems)
...
try {
    ...
    foreach ($item in $runLocks) {
        Assert-File (Join-Path $root ([string]$item.path)) ([long]$item.bytes) ([string]$item.sha256) "RUN_LOCK_$($item.id)"
    }
    ...
    $lock = [ordered]@{
        ...
        locked_items=$runLocks;v2_carrier_hashes=$v2Locks;mr1_inputs_before=$mr1Before;mr2_inputs_before=$mr2Before;prerequisite_gates=$prerequisites
    ...
    $result = switch ($Mode) {
        'PilotCreate' { [B51R1S05R2R1DriverReadyCarrierTool]::CreatePilot($launchedPid,$inputDir,$outputDir,$receipt,$progress) }
        'PilotVerify' { [B51R1S05R2R1DriverReadyCarrierTool]::VerifyPilot($launchedPid,$outputDir,$receipt,$progress) }
        'BatchCreate' { [B51R1S05R2R1DriverReadyCarrierTool]::CreateRemaining($launchedPid,$inputDir,$outputDir,$receipt,$progress) }
        'AllVerify' { [B51R1S05R2R1DriverReadyCarrierTool]::VerifyAll($launchedPid,$outputDir,$receipt,$progress) }
    }
    ...
    foreach ($item in $runLocks) { Assert-File (Join-Path $root ([string]$item.path)) ([long]$item.bytes) ([string]$item.sha256) "POST_LOCK_$($item.id)" }
    ...
    $activation = switch ($Mode) {
        'PilotCreate' { @"
schema: B51R1_AUTONOMOUS_SESSION_PLAN_V22R2A_POOL_A_A08R2_PC_R1_MR2_PILOT_COLD_REOPEN_ACTIVATION
generated_at: '$([DateTimeOffset]::Now.ToString('o'))'
status: S05R2_PC_R1_MR2_PILOT_COLD_REOPEN_ACTIVATED
activation_gate: 07_VERIFICATION/AUTONOMOUS/S05R2_P_R1/B51R1_S05R2_P_R1_MR2_PILOT_CREATE_GATE.json
...
"@ }
        'PilotVerify' { @"
schema: B51R1_AUTONOMOUS_SESSION_PLAN_V22R3A_POOL_A_A08R2_J01_R1_MR2_NATIVE_1R_PILOT_ACTIVATION
generated_at: '$([DateTimeOffset]::Now.ToString('o'))'
status: S05R2_J01_R1_NATIVE_1R_PILOT_ACTIVATED
activation_gate: 07_VERIFICATION/AUTONOMOUS/S05R2_PC_R1/B51R1_S05R2_PC_R1_MR2_PILOT_COLD_REOPEN_GATE.json
output: 03_CAD/20_NATIVE_ASSEMBLY/PILOT/B51R1_CARRIER_J01_R1_NATIVE_PILOT.SLDASM
...
"@ }
        'BatchCreate' { @"
schema: B51R1_AUTONOMOUS_SESSION_PLAN_V23R1A_POOL_A_A08R2_BC_R1_ALL_MR2_COLD_REOPEN_ACTIVATION
generated_at: '$([DateTimeOffset]::Now.ToString('o'))'
status: S05R2_BC_R1_ALL_TEN_MR2_COLD_REOPEN_ACTIVATED
activation_gate: 07_VERIFICATION/AUTONOMOUS/S05R2_B_R1/B51R1_S05R2_B_R1_REMAINING_EIGHT_CREATE_GATE.json
...
"@ }
        'AllVerify' { @"
schema: B51R1_AUTONOMOUS_SESSION_PLAN_V24R1A_POOL_A_A08_S05_NATIVE_CHAIN_REACTIVATION_WITH_MR2_R1
generated_at: '$([DateTimeOffset]::Now.ToString('o'))'
status: POOL_A_A08_S05_NATIVE_CARRIER_CHAIN_REACTIVATED_WITH_MR2_R1
activation_gate: 07_VERIFICATION/AUTONOMOUS/S05R2_BC_R1/B51R1_S05R2_BC_R1_ALL_TEN_COLD_REOPEN_GATE.json
