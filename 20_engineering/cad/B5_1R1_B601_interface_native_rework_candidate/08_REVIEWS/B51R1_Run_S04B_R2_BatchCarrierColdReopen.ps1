...
$sessionRoot = Join-Path $root '07_VERIFICATION\AUTONOMOUS\S04B_R2'
$s03bRoot = Join-Path $root '07_VERIFICATION\AUTONOMOUS\S03B_R2'
...
$toolDll = Join-Path $PSScriptRoot 'B51R1_S03B_S04B_BatchCarrierTool_V3.dll'
$s03bReceipt = Join-Path $s03bRoot 'B51R1_S03B_R2_BATCH_CARRIER_CREATE_RECEIPT.json'
$s03bGate = Join-Path $s03bRoot 'B51R1_S03B_R2_REMAINING_NINE_CARRIER_GATE.json'
$activation = Join-Path $root '04_CONFIGURATION\B51R1_AUTONOMOUS_SESSION_PLAN_V18_POOL_A_A07_S04B_ACTIVATION.yaml'
$manifest = Join-Path $sessionRoot 'B51R1_S04B_R2_CARRIER_MANIFEST.json'
$readinessReceipt = Join-Path $sessionRoot 'B51R1_S04B_R2_READINESS_RECEIPT.json'
$inputLock = Join-Path $sessionRoot 'B51R1_S04B_R2_INPUT_LOCK.json'
$receipt = Join-Path $sessionRoot 'B51R1_S04B_R2_ALL_CARRIER_COLD_REOPEN_RECEIPT.json'
$progress = Join-Path $sessionRoot 'B51R1_S04B_R2_PROGRESS.log'
$gate = Join-Path $sessionRoot 'B51R1_S04B_R2_ALL_TEN_CARRIER_GATE.json'
$sessionReceipt = Join-Path $sessionRoot 'B51R1_S04B_R2_SESSION_RECEIPT.json'
...
$lockedItems = @(
    ...
    [ordered]@{id='BATCH_TOOL_SOURCE_V3';path='08_REVIEWS/B51R1_S03B_S04B_BatchCarrierTool_V3.cs';bytes=34590;sha256='AF291529345D6178575705AB7DEA0F4238FDE11505E9702CB8BB66C516489324'},
    [ordered]@{id='BATCH_TOOL_BINARY_V3';path='08_REVIEWS/B51R1_S03B_S04B_BatchCarrierTool_V3.dll';bytes=30208;sha256='4D21F90B2936E0FBE4146C2DBBA2BDCD0BAD7737EDC1653561D1DAC880D447DC'},
    [ordered]@{id='S03B_R2_RECEIPT';path='07_VERIFICATION/AUTONOMOUS/S03B_R2/B51R1_S03B_R2_BATCH_CARRIER_CREATE_RECEIPT.json';bytes=(Get-Item $s03bReceipt).Length;sha256=Get-Sha $s03bReceipt},
    [ordered]@{id='S03B_R2_GATE';path='07_VERIFICATION/AUTONOMOUS/S03B_R2/B51R1_S03B_R2_REMAINING_NINE_CARRIER_GATE.json';bytes=(Get-Item $s03bGate).Length;sha256=Get-Sha $s03bGate},
...
Write-JsonCreateNew $manifest ([ordered]@{schema='B51R1_S04B_R2_CARRIER_MANIFEST_V1';generated_at=[DateTimeOffset]::Now.ToString('o');status='PASS_TEN_CARRIER_HASHES_LOCKED_FOR_READ_ONLY_COLD_REOPEN';parts=$parts})

$session = [ordered]@{
    schema='B51R1_S04B_R2_SESSION_RECEIPT_V1';session_id='S04B_R2'
    started_at=[DateTimeOffset]::Now.ToString('o');status='S04B_R2_FAIL_CLOSED_NOT_STARTED'
...
try {
    ...
    $runtimeItems=@($lockedItems)+@([ordered]@{id='CARRIER_MANIFEST';path='07_VERIFICATION/AUTONOMOUS/S04B_R2/B51R1_S04B_R2_CARRIER_MANIFEST.json';bytes=(Get-Item $manifest).Length;sha256=Get-Sha $manifest},[ordered]@{id='READINESS_RECEIPT';path='07_VERIFICATION/AUTONOMOUS/S04B_R2/B51R1_S04B_R2_READINESS_RECEIPT.json';bytes=(Get-Item $readinessReceipt).Length;sha256=Get-Sha $readinessReceipt})
    $lock=[ordered]@{
        schema='B51R1_S04B_R2_INPUT_LOCK_V1';generated_at=[DateTimeOffset]::Now.ToString('o');status='PASS_S04B_R2_INPUTS_HASH_LOCKED';session_id='S04B_R2'
    ...
    $result=[B51R1S03BS04BBatchCarrierToolV3]::Verify($launchedPid,$manifest,$receipt,$progress)
    ...
    foreach ($item in $lockedItems) { Assert-File (Join-Path $root ([string]$item.path)) ([long]$item.bytes) ([string]$item.sha256) ([string]$item.id) }
    $gateData=[ordered]@{
        schema='B51R1_S04B_R2_ALL_TEN_CARRIER_GATE_V1';generated_at=[DateTimeOffset]::Now.ToString('o')
        status='S04B_10_OF_10_NATIVE_CARRIERS_COLD_REOPEN_PASS';verified_part_count=10
        manifest=[ordered]@{path='07_VERIFICATION/AUTONOMOUS/S04B_R2/B51R1_S04B_R2_CARRIER_MANIFEST.json';bytes=(Get-Item $manifest).Length;sha256=Get-Sha $manifest}
        cold_reopen_receipt=[ordered]@{path='07_VERIFICATION/AUTONOMOUS/S04B_R2/B51R1_S04B_R2_ALL_CARRIER_COLD_REOPEN_RECEIPT.json';bytes=(Get-Item $receipt).Length;sha256=Get-Sha $receipt}
        parts=$parts
...
activation_gate: 07_VERIFICATION/AUTONOMOUS/S04B_R2/B51R1_S04B_R2_ALL_TEN_CARRIER_GATE.json
activated_slot:
  id: A08_S05
...
"@
    ...
    $session['status']='PASS_S04B_R2_10_OF_10_NATIVE_CARRIERS_COLD_REOPEN'
    ...
    $session['status']='S04B_R2_FAIL_CLOSED';$session['error_type']=$_.Exception.GetType().FullName
