...
def inspect(sw,t,m,state,expected,imports,asm_path):
 a=wrap(m,'IAssemblyDoc',t);components=a.GetComponents(True) or []
 ...
 return {'state':state,'target':str(asm_path),'components':actual,'component_count':len(actual),'dependencies':[{'path':str(p),'sha256':sha(p)} for p in paths],'status':'NATIVE_FIXED_POSE_ASSEMBLY_COLD_REOPENED','storage_mode':'LIGHTWEIGHT_WITH_EACH_COMPONENT_TEMPORARILY_RESOLVED_FOR_INSPECTION','physical_assembly_completed':False,'manufacturing_release':False,'continuous_motion_verified':False,'mate_based_motion_model':False}
def main(sw,t,job):
