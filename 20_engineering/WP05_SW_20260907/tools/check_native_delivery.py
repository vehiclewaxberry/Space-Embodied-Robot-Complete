...
def check_state_rows(expected_rows, actual_rows, *, field_map=None,
    ...
    a.add(prefix + ":instance_count", len(expected_rows) == len(actual_rows) == expected_count,
          expected_source=len(expected_rows), actual=len(actual_rows), required_instance_count=expected_count)
...
def check_assemblies(audit, manifest, records, native_targets, package_root, field_map=None,
    ...
    part_by_key = {p['part_key']:p for p in manifest.get('parts', [])}
    for state in STATES:
        ...
        bad_bodies, expected_total, actual_total = [], 0, 0
        for source in expected:
            row = actual.get(source.get('id'), {})
            count = part_by_key.get(source.get('part_key'), {}).get('expected_solid_count')
            recorded = row.get('resolved_solid_count')
            sheets = row.get('resolved_sheet_count')
            if type(count) is not int or type(recorded) is not int or recorded != count or type(sheets) is not int or sheets != 0:
                bad_bodies.append(source.get('id'))
            if type(count) is int: expected_total += count
            if type(recorded) is int: actual_total += recorded
        audit.add('assembly:'+state+':per_instance_body_reconciliation', not bad_bodies,
                  mismatched_ids=bad_bodies)
        audit.add('assembly:'+state+':total_solid_reconciliation',
                  actual_total == expected_total == rec.get('resolved_solid_total'),
                  expected_source_total=expected_total, actual_component_total=actual_total,
                  producer_reported_total=rec.get('resolved_solid_total'))
