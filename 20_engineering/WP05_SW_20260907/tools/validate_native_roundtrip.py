...
class Kernel:
    ...
    def compare(self, source, returned, expected_solids):
        ...
        # Validity is a required prerequisite, so reject invalid BReps before
        # spending adaptive quadrature on the preceding valid bodies. Keep the
        # actual body indices and both sides of the transfer for diagnosis.
        validity = {label:dict(solid_count=len(bodies),
                    invalid_solid_indices=[i for i,body in enumerate(bodies) if not bool(body.is_valid)])
                    for label,bodies in [('source',source_bodies),('roundtrip',returned_bodies)]}
        if any(v['invalid_solid_indices'] for v in validity.values()):
            return dict(status='FAIL', reason='Invalid OCC solid(s) in source and/or actual native roundtrip; see zero-based indices',
                        validity_preflight=validity, body_index_base=0,
                        source_body_count=len(source_bodies), returned_body_count=len(returned_bodies),
                        complete_body_coverage=False, volume_difference_mm3=None,
                        symmetric_difference_mm3=None, bbox_max_difference_mm=None,
                        adaptive_integration_skipped_for_invalid_input=True)
