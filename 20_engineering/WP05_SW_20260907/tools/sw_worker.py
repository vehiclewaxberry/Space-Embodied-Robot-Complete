...
while True:
 for p in sorted(Q.glob('*.json')):
  ...
  if job.get('worker_exit_after_completion') and result.get('ok') is True:
   # A visible UserControl session belongs to the user; do not call ExitApp.
   sw=None;raw=None
   pythoncom.CoUninitialize()
   print(json.dumps({'status':'WORKER_DETACHED_LEAVING_USER_APPLICATION_OPEN'}),flush=True)
   sys.exit(0)
