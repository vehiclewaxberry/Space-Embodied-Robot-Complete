"""User-requested fresh, read-only cold audit of the final R4 assembly."""
...
rp = D/'results/NATIVE_ASSEMBLY_RECHECK.json'
assert not rp.exists(), 'Preserve prior audit receipts'
report = {'status':'RUNNING', 'progress':[], 'save_attempts':[],
          'request':'按既定规划布置 R4 并冷复验', 'scope':'cold native fixed-pose assembly, identities, total transforms, suppression, body counts, rebuild and preservation',
...
] + [g['saved'] for g in delivery['groups']]+[delivery['service']['saved']]
...
try:
    ...
    assert len(leaves)==plan['expected_leaf_count'] and set(leaves)==set(expected)
    assert total==plan['expected_solid_instances']==1540
    ...
    try:
        ...
        bmp=D/'views/SERVICE_STAR_SERVICE_R4_RESOLVED.bmp'
