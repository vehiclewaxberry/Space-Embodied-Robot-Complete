...
LOCAL_MAPPING_AUTHORITY = "SOURCE_SLDPRT_COLD_LINK6_LOCAL_EXACT_LABEL_PLUS_BREP_FINGERPRINT"
...
def audit_expected_mapping_uniqueness(groups: Dict[str, List[Dict[str, Any]]]) -> Dict[str, Any]:
    ...
    return {
        ...
        "runtime_local_fingerprint_uniqueness_required": True,
        "mapping_authority": LOCAL_MAPPING_AUTHORITY,
        "mapping_fields": ["exact_source_body_label", "faces", "volume_mm3", "link6_local_bbox_mm", "link6_local_bbox_centroid_mm", "link6_local_mass_centroid_mm"],
...
def canonical_source_label(reported_name: str) -> str:
    """Map only the immutable B50/B51 namespace alias; never infer by order."""
    name = str(reported_name).strip()
    for prefix in (BODY_LABEL_PREFIX, B50_BODY_LABEL_PREFIX):
        if name.startswith(prefix):
            suffix = name[len(prefix):]
            if suffix == "" or (len(suffix) == 3 and suffix.isdigit() and 0 <= int(suffix) <= 57):
                return BODY_LABEL_PREFIX + suffix
    raise GateError("SOURCE_LOCAL_LABEL_FAIL", "source SLDPRT body does not carry an exact B50/B51 LINKLOCAL physical label", {"reported_name": reported_name})
...
def map_source_bodies(model: Any, groups: Dict[str, List[Dict[str, Any]]], types: Any, pythoncom: Any) -> Tuple[Dict[str, Any], List[Dict[str, Any]]]:
    ...
    # Pass 1 establishes a cold link-6-local fingerprint manifest directly
    # from the exact source SLDPRT.  Exact labels bind semantic identity;
    # bbox/centroid/volume/face fingerprints bind geometry.  No full-top world
    # coordinate enters this manifest.
    observed: List[Tuple[Any, Dict[str, Any], Dict[str, Any], Dict[str, Any]]] = []
    seen_labels: set[str] = set()
    seen_fingerprints: Dict[str, str] = {}
    for raw in raw_bodies:
        body = base.wrap(raw, "IBody2", types, pythoncom)
        facts = body_evidence(body)
        canonical = canonical_source_label(facts["name"])
        normalized = normalize_body_name(canonical)
        if normalized == normalize_body_name(AGGREGATE_LABEL):
        ...
        if contract is None or canonical not in PHYSICAL_LABELS:
            raise GateError("SOURCE_LOCAL_LABEL_UNAUTHORIZED", "source SLDPRT body label is outside physical rows 000..056", {"reported_name": facts["name"], "canonical_name": canonical})
        if normalized in seen_labels:
            raise GateError("SOURCE_LOCAL_LABEL_DUPLICATE", "source SLDPRT repeats one physical body label", {"canonical_name": canonical})
        if not mapping_volume_matches(float(facts["volume_mm3"]), float(contract["geometry"]["volume_mm3"])):
            raise GateError("SOURCE_LOCAL_VOLUME_INVARIANT_FAIL", "source-local SLDPRT volume does not corroborate the named full-top object invariant", {"canonical_name": canonical, "actual_volume_mm3": facts["volume_mm3"], "expected_volume_mm3": contract["geometry"]["volume_mm3"], "native_face_count_recorded_in_local_fingerprint": facts["faces"], "historical_step_face_count_not_used_for_mapping": contract["geometry"]["step_brep_faces"]})
        fingerprint = local_fingerprint(facts)
        digest = fingerprint["sha256"]
        if digest in seen_fingerprints:
            raise GateError("SOURCE_LOCAL_FINGERPRINT_NOT_UNIQUE", "two source SLDPRT bodies share the complete local B-rep fingerprint", {"first": seen_fingerprints[digest], "second": canonical, "fingerprint": fingerprint})
        seen_labels.add(normalized)
        seen_fingerprints[digest] = canonical
        observed.append((body, facts, fingerprint, contract))

    # Pass 2 maps by the unique local fingerprint established above.  This
    # makes body enumeration order irrelevant while retaining the exact label
    # as a fail-closed identity witness.
    contract_by_fingerprint = {fingerprint["sha256"]: contract for _body, _facts, fingerprint, contract in observed}
    mapped: Dict[str, Any] = {}
    evidence: List[Dict[str, Any]] = []
    for body, facts, fingerprint, label_contract in observed:
        contract = contract_by_fingerprint.get(fingerprint["sha256"])
        if contract is None or contract is not label_contract:
            raise GateError("SOURCE_LOCAL_FINGERPRINT_MAPPING_FAIL", "local B-rep fingerprint did not map back to its exact source identity", {"fingerprint": fingerprint, "label_contract": label_contract["source_name"]})
        normalized = contract["normalized_name"]
        runtime = {**facts, "mapped_source_name": contract["source_name"], "local_brep_fingerprint": fingerprint}
        contract["runtime"] = runtime
        mapped[normalized] = body
        evidence.append({**runtime, "assigned_body": contract["assigned_body"], "historical_csv_volume_mm3": contract["volume_mm3"], "historical_step_faces": contract["faces"], "mapping_authority": LOCAL_MAPPING_AUTHORITY, "full_top_world_coordinate_fields_used": False})
