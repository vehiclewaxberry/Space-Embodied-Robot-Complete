...
def main():
        ...
        for string in range(2):
            for cell in range(7):
                ...
                item={'id':f'PV{channel}_S{string+1}_C{cell+1:02}', 'parent_leaf':panel_id,
                      ...
                      'nominal_glass_footprint_layer_size_mm':[40.15,80.15,.29],
                      ...
                      'solid_geometry_role':'GLASS_FOOTPRINT_CIC_LAYER_PROXY',
                      'entire_package_bounds_known':False,
                      'entire_package_maximum_size_mm':None,
                      'geometry_scope':'Glass-footprint device layer only; interconnects/diode interconnect can protrude beyond outer dimensions. No full CIC package bound.',
    ...
    layout={
        'identity':'WP09F_SOLAR_84_CIC_LAYOUT_V2_GLASS_LAYER_SCOPE','status':'GLASS_LAYOUT_AND_4_5MM_HINGE_CANDIDATE_DEFINED__ENTIRE_CIC_PACKAGE_INTERFACE_OPEN',
        ...
        'source_azur':{'path':str(source),'sha256':h.sha(source),'revision':'DB00010891-01 2025-04-11','mpn':'81442',
                        'page2':'Glass footprint 40.15 +/-0.05 x80.15 +/-0.05 mm; device layer 290 +/-50 um; average CIC weight <=3.6 g; out-of-plane interconnect 6.5 x7.53 x0.025 mm',
                        'outer_dimension_scope_confirmed_by':'AZUR General Information DB00010890-01 2025-04-08 p7; tab/diode interconnect may protrude; full package maximum unknown',
                        'tab_intake_review_path':str(C/'solar/TAB_INTERFACE_REVIEW.md')},
    ...
    checks=[
        ...
        {'id':'NONZERO_GLASS_FOOTPRINT_AND_ADHESIVE_LAYERS','pass':all(c['nominal_glass_footprint_layer_size_mm'][2]>0 and c['adhesive_nominal_mm']>0 and not c['entire_package_bounds_known'] for c in cells)},
    ...
    with (C/'solar/SOLAR_BOM.csv').open('w',encoding='utf-8-sig',newline='') as f:
        ...
        w.writeheader();w.writerows([
          {'item':'AZUR81442_GLASS_FOOTPRINT_CIC_LAYER_PROXY','quantity':84,'unit':'layer proxy','source_or_requirement':'DB00010891-01 p2; whole CIC mean weight reference <=3.6g assigned for bookkeeping, not measured layer mass','unit_mass_kg':.0036,'total_mass_kg':.3024,'mass_owner':'whole solar_CIC_reference; parent budget coverage UNBOUND','status':'GLASS_FOOTPRINT_CIC_LAYER_PROXY; entire CIC package/interconnect max bounds UNKNOWN'},
    ...
    svg=['<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1100 900"><style>text{font-family:Arial,Microsoft YaHei,sans-serif}.t{font-size:16px;fill:#163047}.s{font-size:11px;fill:#17324a}</style><rect width="1100" height="900" fill="#f4f7fb"/><text x="25" y="30" class="t">84 CIC GLASS-FOOTPRINT LAYERS / 6 NATIVE LEAVES — 300 × 200 mm</text><text x="25" y="53" class="s">7S2P target · world +Z active faces · stack step 4.5 mm · glass-footprint layer 0.29 + bond 0.10 mm; complete package UNKNOWN</text>']
    for b in boards:
