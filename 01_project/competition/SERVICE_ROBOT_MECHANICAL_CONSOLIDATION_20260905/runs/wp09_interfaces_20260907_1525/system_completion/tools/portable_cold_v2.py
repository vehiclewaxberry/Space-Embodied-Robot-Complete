...
import sys,json,importlib.util,traceback,argparse,gc,psutil
...
ap=argparse.ArgumentParser();ap.add_argument('location',choices=['primary','relocated']);ap.add_argument('state',choices=['service','parking','released']);ap.add_argument('--receipt-suffix',default='');a=ap.parse_args()
...
base=C/'mechanical/portable';P=base/('package' if a.location=='primary' else 'moved')
if a.location=='relocated':assert not (base/'package').exists(),'Old package must be absent during the moved-root test'
out=C/'results'/('PORTABLE_COLD_'+a.location.upper()+'_'+a.state.upper()+a.receipt_suffix+'.json');assert not out.exists()
...
try:
 ...
 r['preflight']={'available_mib':psutil.virtual_memory().available/1048576,'minimum_start_available_mib':2048,
   'actual_open_documents':[],'process_is_fresh_single_task_owned_SW':True,
   'cleanup':'Prior own saved documents closed and prior process exited; fresh ownership and PID bound by external guard; no unknown processes stopped'}
 assert r['preflight']['available_mib']>=2048,'Conservative 2 GiB startup gate'
 r['session_start_empty']=True;r['solidworks_revision']=str(m.val(sw,'RevisionNumber'))
 r['reference_search']={'prior_working_directory':str(m.val(sw,'GetCurrentWorkingDirectory')),
   'working_directory_for_open':str(P),'method':'SetCurrentWorkingDirectory(parent assembly folder) before OpenDoc6, matching File Open dialog behavior',
   'official_api':'https://help.solidworks.com/2024/english/api/sldworksapi/SolidWorks.Interop.sldworks~SolidWorks.Interop.sldworks.ISldWorks~SetCurrentWorkingDirectory.html'}
 assert sw.SetCurrentWorkingDirectory(str(P))
 for state,s in [(a.state,j['states'][a.state])]:
 ...
 r.update(status='PASS_SINGLE_STATE_FRESH_PROCESS_COLD_PORTABLE',all_parts_byte_identical_to_parent=True,all_external_dependencies_zero=True,
          source_inputs_unchanged=True,open_documents_after=[],physical_path_relocation_test=(a.location=='relocated'))
 sw.SetCurrentWorkingDirectory(r['reference_search']['prior_working_directory'])
