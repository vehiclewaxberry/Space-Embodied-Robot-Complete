...
ck('TTL_LEVEL_BUFFER_INSTALLED',expected['U119.2']=='RUN_LATCH' and expected['U119.4']=='RUN_LEVEL_5V' and expected['R111.1']=='RUN_LEVEL_5V')
ck('LVC_RUN_OUTPUT_LOAD_LT100UA',3.366/(47000*.98)+6e-6<100e-6)
ck('LVC_TO_AHCT_HIGH_MARGIN',3.234-.2>2.0)
ck('AHCT_TO_UCC_HIGH_MARGIN',3.8/(1+1020/9800)>2.4)
ck('AHCT_TO_UCC_LOW_MARGIN',.44<1.0)
for ref,net in [('U116','RUN_STATUS_MCU'),('U117','SAFE_STATUS_MCU'),('U118','AUX_STATUS_MCU')]:
 ck('STATUS_OPEN_DRAIN_'+ref,any(a['net']==net and a['electrical_type']=='open_collector' for a in parts[ref]['pins']))
ck('STATUS_PULLS_RECEIVER_SUPPLY_ONLY',all(expected[r+'.1']=='MCU_VIO' for r in ['R120','R121','R122']))
...
calc=dict(kind='DATASHEET_BASED_DESIGN_CALCULATION_NOT_MEASUREMENT',cwd_mpn='C0603C121F5GACTU',cwd_tolerance=.01,temperature_delta_max_C=100,tempco_ppm_per_C=30,capacitance_pf=[capmin,capmax],board_stray_cap_pf_max_requirement=5,watchdog_ms=[wdmin,wdmax],watchdog_formula_scope='TI application equations 1,3,4,9.5% bound; layout leakage/parasitics must be validated',watchdog_reset_hold_ms=[170,230],heartbeat_max_interval_ms=30,heartbeat_min_high_low_us=100,manual_reset_start_min_press_release_ms=100,full_contact_open_100ms_proven=False,remaining_100ms_after_watchdog_ms=100-wdmax,gx11_release_table_max_ms_25C=12,full_chain_ns_timing_not_qualified=True,coil_VDS_nominal_screen=32+55,mosfet_VDS_abs_V=200,mosfet_VGS_abs_V=10,gate_drive_input_min_output_required_V=drive_high_requirement,AHCT_min_output_V=3.8,AHCT_after_divider_min_V=3.8/(1+1020/9800),AHCT_input_poweroff_leakage_max_A=1e-6,LVC_RUN_load_max_A=3.366/(47000*.98)+6e-6,gate_supply_required_V=[5.049,5.151],RDS_max_25C_VGS4V_ohm=.5,coil_current25C_nom_A=.28,mosfet_conduction_nominal25C_W=.28**2*.5,mosfet_conduction_06A_screen_W=.6**2*.5,coil_current_06A_is_design_screen_not_supplier_bound=True,FAULT_pullup_sink_A=low_current,FAULT_high_min_V=highmin,aux_wetting_current_A=[aux_min,aux_max],rail_24V_UV_threshold_V=[uvmin,uvmax],rail_24V_UV_restore_screen_V=uvrestore,coil_negative_voltage_and_cable_transients='NOT_BOUNDED; no added coil diode; body diode is not a verified reverse-polarity protector',power_on_off_ramps='Only component logic basis; actual sequencing and backfeed need bench observation',manufacturing_release=False,hardware_io_count=0)
calc.update(UCC_INplus_sink_max_allowed_A=(3.8-2.4)/1020-2.4/9800,UCC_INplus_sink_actual_max_A=None,AHCT_after_divider_min_is_zero_UCC_input_current_screen=True,full_worst_case_gate_logic_margin_closed=False,source_of_open_condition='UCC27517 internal input pulldown exists; input current upper bound not published in inspected table; confirm <= allowed current',board_leakage_not_included_in_CWD_bound=True)
calc.update(aux_resistor_only_current_A=calc['aux_wetting_current_A'],aux_wetting_current_with_input_leakage_A=[aux_min-5e-6,aux_max+5e-6],aux_open_input_max_V=5e-6*(10200+1020),aux_closed_input_min_V=5.049-5e-6*1020)
dump(E/'STOP_CIRCUIT_CALCULATIONS.json',calc)
dump(E/'STOP_LOGIC_SCENARIOS.json',logic)
ports=[]
port_direction={('J101','1'):'input',('J101','2'):'input',('J101','3'):'input',('J101','4'):'bidirectional',('J102','1'):'input',('J102','2'):'input',('J102','3'):'bidirectional',('J102','4'):'output',('J102','5'):'output',('J102','6'):'output',('J102','7'):'input',('J102','8'):'input',('J103','1'):'output',('J103','2'):'input',('J103','3'):'output',('J103','4'):'input',('J103','5'):'output',('J103','6'):'input',('J104','1'):'output',('J104','2'):'bidirectional',('J105','1'):'output',('J105','2'):'input'}
...
for p in j['parts']:
 if p['ref'][0]=='J':
  for a in p['pins']:ports.append(dict(connector=p['ref'],pin=a['number'],label=a['name'],net=a['net'],hierarchical_direction=port_direction[(p['ref'],a['number'])],status=p['scope']))
