...
try:
 ...
 if a.mode=='import':
  ...
  for row in j['parts'][a.start:a.end]:
  ...
  if a.mode=='integrate':
   ...
   try:
    ...
    model.EditRebuild3();model.ShowNamedView2('',7);model.ViewZoomtofit2();r['native_save']=b.save_new(model,target)
   finally:
    for o,k,v in reversed(before):setattr(o,k,v)
   before=settings=[];view=feature=props=lookup=ext=asm=None;gc.collect()
   model.GraphicsRedraw2();bmp=folder/f'WP09D_{a.state.upper()}.bmp';r['native_view_capture']=dict(path=str(bmp),api_ok=bool(model.SaveBMP(str(bmp),1600,1200)))
   assert r['native_view_capture']['api_ok'] and bmp.exists()
   b.close_own_saved(model,target,r['native_save']['sha256']);model=None;gc.collect()
   ...
   lookup=observed=None;av=af=None;settings=old=[];gc.collect();b.close_own_saved(model,target,digest);model=None
 if prefs:
