...
Run only after the three integrations, four cold reads and physical relocation
...
def validate_cold(path, state, expected_rows, target, input_hash, relocated=False):
    ...
    require(all(Path(p).resolve().parent == root.resolve()
                and Path(p).suffix.upper() == ".SLDPRT" for p in receipt["dependencies"]),
            f"Nonlocal or non-part dependency: {state}")
...
def validate_parents(inputs):
    ...
    require(len(portable["package_files"]) == 511, "Parent portable file inventory differs")
    ...
    return {
        ...
        "parent_portable_files_unchanged": 511, "parent_source_parts_unchanged": len(pi["parts"]),
...
def memory_receipts():
    ...
    required = {("integrate", state) for state in STATES} | {
        ("cold", state) for state in STATES} | {("cold", "relocated_service")}
    accepted = {}
    for path in sorted((C / "logs").glob("native_delta_*.run.json")):
        ...
        for i, arg in enumerate(command):
            if Path(arg).name == "native_delta_driver.py" and len(command) > i + 2:
...
def main():
    ...
    for state in STATES:
        ...
        capture = cold["native_view_capture"]
    ...
    relocated_path = RESULTS / "NATIVE_DELTA_COLD_relocated_service.json"
    _, relocated_deps, relocated_proof = validate_cold(
        relocated_path, "service", inputs["states"]["service"]["rows"],
        Path(assembly_paths["service"]), input_hash, relocated=True)
    require(relocated_deps == {
        Path(row["native_path"]).name for row in inputs["states"]["service"]["rows"]
    }, "Relocation dependency membership changed")
    memory = memory_receipts()
    for name in sorted(all_dependencies):
        path = D / name
        require(path.suffix.upper() == ".SLDPRT" and not name.startswith("D000_probe"),
                "Invalid native part in ZIP whitelist")
        archives[name] = path
    ...
    readme_text = (
        ...
        "每态 873 个固定组件，1254 个实体为当前装配成员结合零件证据的 SHA 绑定计数。"
        "138 个本轮原生零件逐一冷读实体/包围盒/体积；其中 8 个边框另通过同内核往返几何等价校核，"
        "保留 SolidWorks 标量体积差异。其他零件继承母版 SHA 证据，未重测全部 1254 个实体。\n\n"
        "三态已冷启动读取组件 ID、变换和本地依赖；SERVICE 另在原目录缺席时移至另一短目录冷读，"
        "并将文件原样返回。PNG 来自实际 SolidWorks 视口 BMP 的无损转换。\n\n"
        "保存使用固定组件图和 AvoidRebuildOnSave；未执行强制全量特征重建。"
    ...
    require({name for name in archives if name.upper().endswith(".SLDASM")} ==
            {f"WP09D_{s.upper()}.SLDASM" for s in STATES}, "Archive includes history assembly")
    require(len(archives) == len(all_dependencies) + 7, "Unexpected archive inventory")
    ...
    delivery = {
        ...
        "unique_native_part_files_in_archive": len(all_dependencies),
        ...
        "relocation": relocated_proof, "parent_hash_preservation": parent_proof,
