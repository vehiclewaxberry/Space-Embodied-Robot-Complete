...
h=d.h;m=d.m
out=N/'results/NATIVE_SERVICE_RECOVERY_V2.json';assert not out.exists()
prior=F/'results/NATIVE_SERVICE.json';r=json.loads(prior.read_text());rows=r['rows'];
for row in rows:row['T_S_local']=row.get('native_T_local_to_S',row['T_S_local'])
r['transform_contract_correction']='Use native_T_local_to_S for canonical native parts, preserve source STEP T only in source manifest; no geometry edited'
target=N/'mechanical/native/WP09R_SERVICE.SLDASM';th=m.sha(target)
