...
class Builder(m.ModuleBuilder):
    def identity_inventory(self,asm,expected_rows):
        raw=asm.GetComponents(True) or [];lookup={}
        for i,rc in enumerate(raw):
            comp=self.wrap(rc,'IComponent2');k=comp.ComponentReference;assert k not in lookup;lookup[k]=comp
            if i%64==63:self.checkpoint('identity_inventory',components=i+1)
        assert set(lookup)=={r['id'] for r in expected_rows} and len(raw)==len(expected_rows)
        return lookup
...
def main():
    ...
    try:
        ...
        lookup=b.identity_inventory(asm,base);before=None
        report['parent_preedit_evidence']='EXACT_PARENT_ASSEMBLY_HASH_AND_HASH_BOUND_COLD_WP08_RECEIPTS_PLUS_CURRENT_IDENTITY_SET';b.checkpoint('parent_identity_verified')
        ...
        lookup=b.identity_inventory(asm,[r for r in base if r['id'] not in mi['removed_parent_ids']]);remaining=None;report['retained_verified']=len(lookup)
        ...
        try:
            ...
            b.identity_inventory(asm,rows);props=b.wrap(ext.CustomPropertyManager(''),'ICustomPropertyManager')
