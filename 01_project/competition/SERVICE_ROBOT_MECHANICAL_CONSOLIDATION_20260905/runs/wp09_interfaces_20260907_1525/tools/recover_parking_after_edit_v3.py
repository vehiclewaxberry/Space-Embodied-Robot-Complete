...
out=R/'results/NATIVE_PARKING_RECOVERY_V3.json';assert not out.exists()
...
try:
    ...
    b.sw.CommandInProgress=old['ui_before']['CommandInProgress']
    docs=b.documents();report['documents_before']=[d for _,d in docs]
    new_paths={m.m.normalized(r['native_path']):r['native_sha256'] for r in rows[619:]}
    for doc,d in docs:
        p=m.m.normalized(d['path'])
        assert p in new_paths or p in {m.m.normalized(target),m.m.normalized(work)},'Unknown document preserved'
        if p in new_paths:
            assert not d['dirty'] and m.m.sha(d['path'])==new_paths[p]
    docs=[(doc,d) for doc,d in docs if m.m.normalized(d['path']) not in new_paths];assert len(docs)<=1
    ...
    for doc,d in b.documents():
        p=m.m.normalized(d['path']);assert p in new_paths and not d['dirty']
        b.close_own_saved(doc,Path(d['path']),new_paths[p])
