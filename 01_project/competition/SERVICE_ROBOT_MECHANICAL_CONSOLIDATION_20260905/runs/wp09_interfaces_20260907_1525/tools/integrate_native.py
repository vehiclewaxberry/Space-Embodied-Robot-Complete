    ...
    def metadata(self,model,rows,measure=False):
        ...
        for n,rc in enumerate(raw):
            ...
            if measure:
                ...
                o.update(actual_solids=len(bodies),actual_sheets=len(sheets),basis_max_error_mm=max(basis));bodies=sheets=point=q=None
...
def main():
    ...
    parent_receipt=W8/'results'/('NATIVE_'+a.state.upper()+'.json');pr=json.loads(parent_receipt.read_text());assert pr['status']=='PASS_NATIVE_FIXED_POSE_DELTA_COLD_REOPEN' and pr['native_save']['sha256']==parenthash
    ...
    try:
        ...
        opened=sw.OpenDoc6(str(target),2,193,'',0,0);assert opened[0] is not None and opened[1]==0;cold=b.wrap(opened[0],'IModelDoc2');b.activate(cold,target)
        ...
        report.update(status='PASS_FIXED_POSE_NATIVE_DELTA_COLD_BODY_AND_TRANSFORM_CHECK',component_count=len(observed),solid_count=sum(x['actual_solids'] for x in observed),body_counts_are_actual=True,continuous_motion_verified=False)
