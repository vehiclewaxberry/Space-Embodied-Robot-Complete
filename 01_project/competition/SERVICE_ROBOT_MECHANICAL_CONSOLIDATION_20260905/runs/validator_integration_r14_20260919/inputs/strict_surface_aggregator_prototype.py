"""Isolated protocol prototype, not a deployed verifier or geometry algorithm.

Only aggregates supplied records. A successful synthetic case covers exactly the
declared historical nonadjacent pair set minus the explicitly unknown finger pair.
...
def aggregate(contract, evidence):
    """Both arguments are synthetic protocol records, not original WP03 receipts."""
    result = {'evidence_type': 'ISOLATED_CODE_CONTROL_NOT_CAD',
              'geometry_executed': False, 'status': 'INCOMPLETE_OR_INVALID_EVIDENCE',
...
def require_physical_view(receipt):
    """Proposed input guard only; no claim that current consumers already enforce it."""
