...
def main():
    output=R/'results/NATIVE_PARKING_RECOVERY.json';require(not output.exists(),'Recovery receipt protected')
    ...
    try:
        ...
        allowed={norm(r['native_path']):r['native_sha256'] for s in m['states'].values() for r in s['instances']}
        allowed[norm(target)]=digest
        for p in (R/'results').glob('NATIVE_*.json'):
            d=read(p)
            if d.get('status')=='PASS_NATIVE_FIXED_POSE_DELTA_COLD_REOPEN': allowed[norm(d['native_save']['path'])]=d['native_save']['sha256']
