...
class Builder(reuse.ComponentwiseIntegrator):
    ...
    def integrate(self,state,info,parent_rows):
        ...
        view=self.wrap(model.ActiveView,'IModelView');feature=self.wrap(model.FeatureManager,'IFeatureManager')
        require(view is not None and feature is not None,'No mutation UI handles')
        settings=[(self.sw,'CommandInProgress',True),(view,'EnableGraphicsUpdate',False),(feature,'EnableFeatureTree',False),(feature,'EnableFeatureTreeWindow',False)]
        before_ui=[(obj,key,bool(getattr(obj,key))) for obj,key,_ in settings]
        try:
            for obj,key,want in settings:setattr(obj,key,want)
            self.report['insertion_batches']=[]
            for start in range(0,len(new),8):
                chunk=new[start:start+8];self.ram_floor()
                transforms=[v for row in chunk for v in t16(row['T_S_local'])]
                raw=a.AddComponents3(self.VARIANT(self.pythoncom.VT_ARRAY|self.pythoncom.VT_BSTR,[r['native_path'] for r in chunk]),self.VARIANT(self.pythoncom.VT_ARRAY|self.pythoncom.VT_R8,transforms),self.VARIANT(self.pythoncom.VT_ARRAY|self.pythoncom.VT_BSTR,['']*len(chunk))) or []
                require(len(raw)==len(chunk),'Insertion batch incomplete');model.ClearSelection2(True)
                for item,row in zip(raw,chunk):
                ...
                a.FixComponent();model.ClearSelection2(True);raw=item=comp=None;gc.collect()
                unload=a.LightweightAllResolved()
                self.report['insertion_batches'].append(dict(ids=[r['id'] for r in chunk],unload_api_return=unload))
                self.checkpoint('insertion_batch_completed_and_unloaded',completed=min(start+8,len(new)),memory=self.memory_snapshot())
        finally:
            restores=[]
            for obj,key,want in reversed(before_ui):
                setattr(obj,key,want);restores.append(dict(property=key,expected=want,actual=bool(getattr(obj,key))))
            self.report['mutation_ui_restore']=restores
            require(all(x['expected']==x['actual'] for x in restores),'Mutation UI restore failed')
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('state',choices=['released']);args=ap.parse_args()
    ...
    builder=None
    try:
        reuse.require_outer_guard(report);pins=dict(m['input_sha256_after']);pins[str(R/'tools/integrate_native_delta.py')]=sha(R/'tools/integrate_native_delta.py')
