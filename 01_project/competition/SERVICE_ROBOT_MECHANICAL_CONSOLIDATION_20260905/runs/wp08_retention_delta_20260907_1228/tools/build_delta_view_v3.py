...
TARGET = OUT / 'WP08_SERVICE_RETENTION_DELTA_V3.glb'
REPORT = OUT / 'DISPLAY_DELTA_CHECK_V3.json'
...
def main():
    ...
    report = dict(schema='WP08_DISPLAY_DELTA_V3', status='RUNNING', source_sha256={},
    ...
    try:
        bind(__file__)
        bind(RUN/'tools/build_delta_view_v2.py')
        bind(OUT/'DISPLAY_DELTA_CHECK_V2.json')
        report['prior_transform_assumption']={'status':'REJECTED_BY_PREFLIGHT','reason':'Catalogue hardware cache prototypes use occurrence transforms; V3 composes T_S_mast @ T_mast_cache_prototype explicitly.'}
        ...
        for name in sorted(changed):
            ...
            local_cache_transform=np.asarray(occ['transform'],dtype=float).reshape(4,4)
            require(np.isfinite(local_cache_transform).all() and np.allclose(local_cache_transform[3],[0,0,0,1],rtol=0,atol=1e-12), 'Invalid local cache transform')
            world_cache_transform=np.asarray(row['T_S_local'],dtype=float) @ local_cache_transform
            ...
            node=dict(name=name,mesh=len(model['meshes']),matrix=h.gltf_matrix(world_cache_transform),
            ...
            mappings.append(dict(id=name,cache=str(cache_path),cache_occurrence_id=occ['id'],
                component_glb=str(partfile),T_mast_cache_prototype=local_cache_transform.tolist(),T_S_cache_prototype=world_cache_transform.tolist(),sha256=digest,bin_output_offset=base,bytes=length))
