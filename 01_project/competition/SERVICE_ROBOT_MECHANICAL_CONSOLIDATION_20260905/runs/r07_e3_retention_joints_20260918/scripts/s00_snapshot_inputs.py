# -*- coding: utf-8 -*-
"""R07-E3 run: 输入快照与 sha256 清单（LF 二进制写）。
快照改件前输入（E2 后状态，整装基线 459 实例）；备份烟测将覆盖的 results 回执。"""
...
def main():
    ...
    if cur.exists():
        shutil.copy2(cur, RUN / 'logs' / 'service_structure_instances.json.pre_e3_smoke_bak')
    manifest = {
        'run': 'r07_e3_retention_joints_20260918',
        'created_by': 'WP03_R07_E3_builder (single CAD builder)',
        'role': 'pre_change_input_snapshot',
        'note': '改件前输入快照（E2 已闭环后状态，整装基线 459 实例）；sha256 对原文件字节计算。WP02 文件只读不改，快照仅为来源绑定。',
        'files': man,
    }
