...
import json,sys,hashlib,importlib.util,math
sys.dont_write_bytecode=True
R=Path(__file__).resolve().parents[1];CP=R/'inputs/BATTERY_MOUNT_CONTRACT_V2.json'
def sha(p):
...
def build():
    ...
    for x in p['axis_x_mm']:
        for y in p['axis_y_mm']:
            # Match the source STEP analytic main cone, preserving nominal90deg callout.
...
def emit():
    out=R/'results/battery_mount_v2';out.mkdir(exist_ok=True);receipt=out/'EMISSION.json'
    ...
    primary=R/'candidate/battery_mount_v2.step';assert not primary.exists();export_step(a,primary)
