...
assert len(bom)==44
bp=R/'docs/LOCAL_INSTANCE_BOM.csv'
if bp.exists():
    with bp.open(encoding='utf-8-sig',newline='') as f:existing=list(csv.DictReader(f))
    assert existing==[{k:str(v) for k,v in row.items()} for row in bom],'Existing BOM differs'
else:
