...
class Audit:
    def __init__(self, output):
        self.output = output
        self.data = dict(
            schema='WP09_SOURCE_STEP_IGES_TRANSFER_V1', status='IN_PROGRESS',
            ...
            scope='Four existing catalogue proxy screw solids; STEP-to-IGES '
                  'material transfer only. Not SolidWorks import equivalence, '
...
def execute(audit, emission_path, contract_path, outdir):
    ...
    def directed_cut(a, b):
        ...
        require(fuzzy == 0.0, 'Nonzero Boolean fuzzy value')
        ...
        return dict(done=True, valid=valid, fuzzy_mm=fuzzy, **volume(result))
...
def main():
    ...
    parser.add_argument('--output-dir', type=Path, default=R / 'results/battery_iges_transfer')
