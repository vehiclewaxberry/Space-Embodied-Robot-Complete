...
extra_inputs=[Path(__file__),R/'results/COMPONENT_ENVELOPE_SCREEN.json',R/'docs/DIMENSIONS_AND_ASSEMBLY_SEQUENCE_ZH.md',
 R/'docs/ELECTRO_PROPULSION_MECHANICAL_WORK_PACKAGE_ZH.md',R/'docs/INDEPENDENT_DELIVERY_REVIEW_ZH.md',R/'research/ELECTRICAL_HARDWARE_RESEARCH.md',
...
status=dict(schema='WP09_DELIVERY_STATUS',status='THREE_LOCAL_CAD_NATIVE_PACKAGES_VERIFIED__ELECTRO_PROPULSION_INTEGRATION_OPEN',
 date=datetime.datetime.now().astimezone().isoformat(),modules=mods,total_local_instances=44,total_native_parts=44,total_native_assemblies=3,
 local_nominal_geometry_verified=True,native_cold_reopen_verified=True,native_source_material_transfer_verified=True,
...
lines=['# WP09 电气与推进部件机械设计候选','',
 '本轮完成三套可查看的局部 STEP / SolidWorks 装配，共44个局部实体、44个原生零件文件和3套固定定位装配。独立检查覆盖各组件的源几何、孔位和材料交叠，并专项检查电池连接的实际接触面积；三套均完成原生冷重开与 STEP 回导材料等价。当前可进入硬件型号绑定和受控整星布置调整；整星电气、推进、实物装配与制造放行仍未完成。','',
 '## 直接查看','',
 '| 组件 | 原生装配 | 局部检查 | 回导材料检查 |','|---|---|---|---|']
for k,d in mods.items():
    lines.append('|'+names[k]+'（'+str(d['instances'])+'实体）|'+link(d['native_assembly']['path'],'打开 SLDASM')+'|'+link(d['local_check'],str(d['local_check_count']['PASS'])+'项通过')+'|'+link(d['native_material_check'],str(d['native_material_check_count']['PASS'])+'项通过')+'|')
lines+=['','SolidWorks 零件为源 STEP 导入实体，装配采用固定定位，没有可动配合。尺寸合同和构建源位于 inputs/ 与 candidate/，可按合同改版重建；当前原生实体没有完整的特征历史。保留 native/<模块>/parts 全部依赖；尚未进行异机路径迁移测试。','',
 '## 本轮实际变化','',
 ...
 '- '+link(bp,'44项局部实例 BOM'),
 '- '+link(R/'docs/ELECTRO_PROPULSION_MECHANICAL_WORK_PACKAGE_ZH.md','12项后续机械工程工作包'),
 ...
 '直接STEP导入电池沉头螺钉的四次尝试因COM极值检测与源相差约5.639µm而失败。补充高精度体积、极值点与源距离、原生STEP回导诊断未建立等价证明，故原失败保留。最终改用源STEP→IGES BRep的传递路径；先独立验证四颗螺钉的IGES回读，再按原源STEP验收原生冷重开和最终装配回导，全程不放宽阈值。','',
