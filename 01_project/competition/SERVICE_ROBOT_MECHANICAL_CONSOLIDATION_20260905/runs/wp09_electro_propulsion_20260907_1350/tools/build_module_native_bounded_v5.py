...
def main():
    ...
    try:
        ...
        prefs=dict(toggles={k:sw.GetUserPreferenceToggle(k) for k in (111,290,291,691,690,769)},
          strings={k:sw.GetUserPreferenceStringValue(k) for k in (8,9,10)},integers={k:sw.GetUserPreferenceIntegerValue(k) for k in (140,577,578,579,580)})
        report['import_toggle_before']={str(k):v for k,v in prefs['toggles'].items()}
        for k in prefs['toggles']:
            sw.SetUserPreferenceToggle(k,k in (111,290))
            m.require(bool(sw.GetUserPreferenceToggle(k))==(k in (111,290)),'Import toggle readback differs')
        ...
        report['import_integer_during']={'140':sw.GetUserPreferenceIntegerValue(140)}
