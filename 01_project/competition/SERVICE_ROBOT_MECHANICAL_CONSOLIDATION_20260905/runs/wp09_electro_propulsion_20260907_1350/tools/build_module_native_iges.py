...
class ModuleBuilder(frozen.Builder):
    ...
    def import_part(self, row):
            ...
            transfer_source=Path(row.get('iges_path',source))
            if 'iges_path' in row:
                require(sha(transfer_source)==row['iges_sha256'],'IGES transport changed')
                importer=self.wrap(self.sw.GetImportFileData(str(transfer_source)),'IImportIgesData')
                importer.IncludeSurfaces=True
                importer.IncludeCurves=False
                importer.CurvesAsSketches=False
                importer.ProcessByLevel=False
                require(importer.SetLevels(True,None),'IGES all-level selection failed')
            else:
            ...
            receipt['transport']=dict(path=str(transfer_source),sha256=sha(transfer_source),kind='IGES_BREP_EQUIVALENCE_CHECKED' if 'iges_path' in row else 'STEP_DIRECT')
            result = self.sw.LoadFile4(str(transfer_source), 'r', importer, 0)
