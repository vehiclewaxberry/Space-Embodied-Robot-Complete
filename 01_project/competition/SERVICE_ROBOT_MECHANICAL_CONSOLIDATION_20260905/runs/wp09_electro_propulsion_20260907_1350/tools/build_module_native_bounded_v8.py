...
s=importlib.util.spec_from_file_location('wp09_native_parent',R/'tools/build_module_native_iges.py')
m=importlib.util.module_from_spec(s);s.loader.exec_module(m)
def main():
    ap=argparse.ArgumentParser();ap.add_argument('module');ap.add_argument('--batch',type=int,required=True);ap.add_argument('--local-check',type=Path);ap.add_argument('--emission',type=Path);ap.add_argument('--native-folder');a=ap.parse_args();kind=a.module
    ep=a.emission or R/'results'/kind/'EMISSION.json';cp=a.local_check or R/'results'/kind/'CHECK.json'
    ...
    folder=R/'native'/(a.native_folder or kind);m.require(folder.resolve().is_relative_to((R/'native').resolve()),'Native output outside run');target=folder/('WP09_'+kind.upper()+'.SLDASM');neutral=folder/'native_roundtrip.step'
    ...
    rows.sort(key=lambda x:(x['id']!='WP09_BAT_-152_-78_screw',x['id']))
    ...
    transfer_receipt=R/'results/battery_iges_transfer_v3/CHECK.json'
    transfer=json.loads(transfer_receipt.read_text())
    m.require(transfer['status']=='PASS_SOURCE_STEP_IGES_TRANSFER','IGES source material check not passed')
    pins[str(transfer_receipt)]=m.sha(transfer_receipt)
    pins.update(transfer['input_sha256_before'])
    for row in rows:
        if row['id'].endswith('_screw'):
            t=transfer['parts'][row['id']]
            m.require(m.normalized(t['source_step'])==m.normalized(row['step_path']) and t['source_sha256']==row['source_sha256'],'IGES original source mismatch')
            row.update(iges_path=t['iges_path'],iges_sha256=t['iges_sha256'])
            pins[t['iges_path']]=t['iges_sha256']
    prior={};allowed={}
    ...
    try:
        ...
        prefs=dict(toggles={k:sw.GetUserPreferenceToggle(k) for k in (111,161,290,291,691,690,769)},
          strings={k:sw.GetUserPreferenceStringValue(k) for k in (8,9,10)},integers={k:sw.GetUserPreferenceIntegerValue(k) for k in (12,140,207,577,578,579,580)})
        report['import_toggle_before']={str(k):v for k,v in prefs['toggles'].items()}
        for k in prefs['toggles']:
            sw.SetUserPreferenceToggle(k,k in (111,161,290))
            m.require(bool(sw.GetUserPreferenceToggle(k))==(k in (111,161,290)),'Import toggle readback differs')
        report['import_toggle_during']={str(k):sw.GetUserPreferenceToggle(k) for k in prefs['toggles']}
        report['neutral_import_basis']='Disable import repairs and diagnostics: toggle290 true,291/690/691/769 false; integer140=0. Original tolerances unchanged.'
        report['import_integer_before']={str(k):v for k,v in prefs['integers'].items()}
        sw.SetUserPreferenceIntegerValue(140,0)
        m.require(sw.GetUserPreferenceIntegerValue(140)==0,'Repair setting readback differs')
        sw.SetUserPreferenceIntegerValue(12,0)
        sw.SetUserPreferenceIntegerValue(207,0)
        m.require(sw.GetUserPreferenceIntegerValue(12)==0 and sw.GetUserPreferenceIntegerValue(207)==0,'Try forming solids readback differs')
        report['import_integer_during']={str(k):sw.GetUserPreferenceIntegerValue(k) for k in (12,140,207)}
        report['direct_brep_mapping']=False
        report['iges_surface_solid_entity_option']='swGeneralImportTryFormingSolids' 
        ...
        if b:
            try:
                ...
                report['import_toggle_after']={str(k):b.sw.GetUserPreferenceToggle(k) for k in prefs['toggles']} if prefs else {}
                report['import_integer_after']={str(k):b.sw.GetUserPreferenceIntegerValue(k) for k in prefs['integers']} if prefs else {}
                m.require(all(b.sw.GetUserPreferenceToggle(k)==v for k,v in prefs['toggles'].items()),'Toggle restore differs')
                m.require(all(b.sw.GetUserPreferenceIntegerValue(k)==v for k,v in prefs['integers'].items()),'Integer restore differs')
