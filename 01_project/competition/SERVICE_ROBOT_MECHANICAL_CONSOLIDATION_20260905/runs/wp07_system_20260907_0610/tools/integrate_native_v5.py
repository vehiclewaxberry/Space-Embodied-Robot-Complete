"""WP07 V5: componentwise cold inspection and explicit native-only delivery.
...
import resume_service_fast as fast
import check_native_delivery_v3 as independent
from integrate_native import R, require, sha, normalized, val, t16, IDENTITY16


class IntegratorV5(fast.FastIntegrator, v2.IntegratorV2):
    skip_neutral_export = True
    batch_size = 10

    def inspect(self, model, rows, expected_path):
        return fast.FastIntegrator.inspect(self, model, rows, expected_path)
    ...
    def integrate(self, state, info, removed, replaced):
        ...
        self.report['implementation'] = dict(path=str(Path(__file__).resolve()), sha256=sha(__file__),
            superclass='IntegratorV2', inspector='FastIntegrator_BatchedUnload_UIUpdatesPaused',
        ...
        self.report['explicit_graphics_and_screenshot_skipped'] = True
...
def main():
    ...
    recovery.RECOVERY = independent.preferred_receipt('service')
    ...
    try:
        ...
        for module in (Path(__file__).resolve(), Path(v2.__file__), Path(recovery.__file__), Path(fast.__file__), Path(fast.check.__file__), Path(independent.__file__), Path(v2.base.__file__)):
        ...
        builder = IntegratorV5(output, report)
        ...
        require(report['inputs_unchanged'], 'Pinned input changed during V5 native integration')
