...
def main():
    ...
    parser.add_argument('--no-screenshot', action='store_true', help='No explicit native view, zoom, redraw, or bitmap calls')
    args = parser.parse_args()
    recovery.RECOVERY = independent.preferred_receipt('service')
    output = R/'results/FINAL_REOPEN_SERVICE_V3.json'
    ...
    report = dict(schema='WP07_FINAL_NATIVE_REOPEN_V3', state='service', status='RUNNING', progress=[],
    ...
    try:
        ...
        report['explicit_graphics_and_screenshot_skipped'] = bool(args.no_screenshot)
        if not args.no_screenshot:
            model.ShowNamedView2('', 7)
            model.ViewZoomtofit2()
            model.GraphicsRedraw2()
            bmp = R/'results/WP07_SERVICE_FINAL_V3.bmp'
