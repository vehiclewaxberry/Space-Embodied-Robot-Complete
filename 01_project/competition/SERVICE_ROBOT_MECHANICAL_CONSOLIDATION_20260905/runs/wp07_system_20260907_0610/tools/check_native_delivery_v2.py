...
def preferred_receipt(state):
    fast = R/'results/NATIVE_SERVICE_RECOVERY_FAST.json'
    if state == 'service' and fast.exists():
        return fast
    recovery = R/'results/NATIVE_SERVICE_RECOVERY.json'
    if state == 'service' and recovery.exists():
        return recovery
...
def check_state(state, manifest, manifest_sha, pins, cache):
    ...
    expected_last = 'recovery_completed' if state == 'service' and receipt_path.name in ('NATIVE_SERVICE_RECOVERY.json', 'NATIVE_SERVICE_RECOVERY_FAST.json') else 'completed'
