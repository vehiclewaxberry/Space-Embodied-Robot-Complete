...
def read_contract(path=None):
    ...
    if c["schema"] != "WP07_RETENTION_GUIDE_END_DESIGN_V2":
...
def build_station(station_index, contract_path=None):
    ...
    # Through-open side grooves accept detachable collar keys from +/-Y.
    # Remaining original X wall is 1.5 mm nominal; no strength credit.
    for side in (-1, 1):
        xa, xb = ((p["key_groove_inner_x_abs_mm"], 6.01) if side > 0 else (-6.01, -p["key_groove_inner_x_abs_mm"]))
        mast = mast - box(xa, xb, -11, 11, lo+p["key_groove_z0_above_lo_mm"], lo+p["key_groove_z1_above_lo_mm"])
    parts = {mast_id: mast}
    provenance = {mast_id: {"kind": "REPLACEMENT_UPPER_EARS_AND_LOWER_KEY_GROOVES", "source_id": mast_id}}
    for side, tag in ((1, "front"), (-1, "rear")):
        def ybox(xa, xb, ya, yb, za=lo, zb=lo+ear_h):
            return box(xa, xb, side*ya if side > 0 else -yb, side*yb if side > 0 else -ya, za, zb)
        xi=p["lower_collar_inner_x_half_mm"]; xo=p["lower_collar_outer_x_half_mm"]
        yi=p["lower_collar_inner_y_half_mm"]; yo=p["lower_collar_crossbar_outer_y_half_mm"]
        sx=p["lower_collar_side_web_outer_x_half_mm"]; fy=p["lower_collar_flange_y_half_mm"]
        collar=ybox(-sx,sx,yi,yo)
        for sign in (-1,1):
            xa,xb=(xi,xo) if sign>0 else (-xo,-xi)
            collar=collar+ybox(xa,xb,0,fy)
            xa,xb=(xi,sx) if sign>0 else (-sx,-xi)
            collar=collar+ybox(xa,xb,0,yo)
            ...
            collar=collar+ybox(xa,xb,0,yi,lo+p["key_tongue_z0_above_lo_mm"],lo+p["key_tongue_z1_above_lo_mm"])
        for x in p["guide_axis_x_mm"]:
            collar=collar-cylinder(p["guide_clearance_hole_d_mm"],lo-1,lo+ear_h+1,x)
        for x in p["lower_collar_bolt_x_mm"]:
            bore=Solid.make_cylinder(p["lower_collar_bolt_hole_d_mm"]/2,2*fy+2,Plane(origin=(x,-fy-1,lo+ear_h/2),z_dir=(0,1,0)))
            collar=collar-bore
        key=f"WP07_lower_collar_{k}_{tag}"
        parts[key]=collar
        provenance[key]={"kind":"NEW_CUSTOM_KEYED_SPLIT_COLLAR","source_id":None}
    catshapes={}
    for role,row in c["catalogue_parts"].items():
        if sha(row["path"])!=row["sha256"]:
            raise ValueError("Catalogue STEP changed: "+role)
        tree=import_step(row["path"])
        catshapes[role]=type(tree)(tree.wrapped).located(tree.global_location)
    for x in p["lower_collar_bolt_x_mm"]:
        for role, source_role, base in (("screw","screw",4.5),("washer_outer","washer",4.0),("washer_inner","washer",-4.5),("nut","nut",-6.9)):
            key=f"WP07_lower_{role}_{k}_{int(x)}"
            pose=Plane(origin=(x,base,lo+ear_h/2),x_dir=(1,0,0),z_dir=(0,1,0)).location
            parts[key]=catshapes[source_role].moved(pose)
            provenance[key]={"kind":"NEW_FROZEN_CATALOGUE_GEOMETRY","source_id":None,
                "catalogue_source":c["catalogue_parts"][source_role],"catalogue_local_T":{"origin_mm":[x,base,lo+ear_h/2],"x_dir":[1,0,0],"z_dir":[0,1,0]}}
    for x in p["guide_axis_x_mm"]:
        key = str(int(x))
    ...
    metadata = dict(schema="WP07_RETENTION_DETAIL_EMISSION_V2", status="CAD_CANDIDATE_NOT_VALIDATED",
                    ...
                    parameters=p, shaft_cylinder_interval_local_mm=[lo, hi],
                    shaft_cylinder_length_mm=p["effective_guide_length_mm"],
                    ear_inner_free_span_mm=p["effective_guide_length_mm"]-2*ear_h-play,
                    length_definitions=c["length_definitions"],
...
def export_station(station_index, contract_path=None, output_dir=None):
    ...
    *_, export_step = _cad()
    ...
    for instance, shape in built["parts"].items():
        ...
        kind = built["metadata"]["provenance"][instance]["kind"]
        physical = "NOMINAL" not in kind
        rows[instance] = dict(path=str(path), sha256=sha(path),
                              representation_role="PHYSICAL_GEOMETRY" if physical else "SIMPLIFIED_PROXY",
                              product_role=kind if physical else "CUSTOM_NOMINAL_GEOMETRY",
