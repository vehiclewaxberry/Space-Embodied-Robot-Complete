...
def self_worker():
    ...
    for pose in POSES[2:]:
        ...
        for a,b in pairs:
            ...
            rows.append(rec);print(json.dumps({'event':'pair','pose':pose['id'],'record':rec}),flush=True)
            if rec['surface_intersection']:break
        print(json.dumps({'event':'pose_complete','pose':pose['id'],'expected_pairs':len(pairs),'rows':rows,
...
def main():
    ...
    inputs=[WP01/'kinematics.py',WP01/'design_parameters.json',LAYOUT,WP01/'results/stowed_build_receipt.json',WP01/'results/FINGER_INTERFACE_CHECK.json',WP02/'motion_analysis.py',kin.URDF,HERE/'wing_kinematics.py']
    data={'schema':'WP03_BOUNDED_EXISTING_POSE_SCREEN_V1','status':'GEOMETRY_COMPLETE_SELF_SCREEN_PENDING','source_hashes':{str(p):sha(p) for p in inputs},'mesh_sources':sources,
    ...
    for p in data['poses'][2:]:
        ...
        status='EXPLICIT_SELF_SURFACE_INTERSECTION_REJECTED' if hits else '35_BODY_PAIRS_SURFACE_DISJOINT_FINGER_UNKNOWN' if checked==35 else 'INCOMPLETE_UNKNOWN'
        p['self_surface_check']={'status':status,'expected_nonadjacent_pairs':36,'evaluated_pairs':checked,'records':rows,'surface_intersection_pairs':hits,
                                 'worker_completed':p['id'] in completed,'contains_status':'UNKNOWN'}
    data.update(status='BOUNDED_SCREEN_COMPLETE_NO_LAUNCH_STOW_CLAIM',self_worker_error=error,elapsed_seconds=time.monotonic()-start)
    save(data)
