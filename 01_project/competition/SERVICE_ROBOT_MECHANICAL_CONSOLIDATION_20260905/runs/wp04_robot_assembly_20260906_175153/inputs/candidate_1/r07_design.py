...
def write_contract(P,receipt):
    ...
    d=dict(schema='WP04_R07_CONNECTIONS_V1',run_id=HERE.parent.name,configuration=P['configuration_id'],state=receipt['state'],units='mm',frame='S',
        connections=cs,affected_instance_ids=affected_ids(P,receipt),source_sha256=hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),
        ...
        sleeve_installation='INSERT_EIGHT_SLEEVES_ALONG_OPEN_LONGERON_CAVITY_BEFORE_END_PLUGS; ACCESS_AND_RETENTION_NOT_YET_VERIFIED',
        remaining=['CONNECTION_STRENGTH_PRELOAD_TOLERANCE','ROOT_B601_AS_BUILT_INTERFACE','SLEEVE_INSTALLATION_AND_RETENTION'])
...
def affected_ids(P,receipt):
    return [r['id'] for r in receipt['instances'] if r['id'].startswith(('R07_','RB_longeron_','RB_lower_spacer_','RB_pillar_tierod_','RB_pillar_top_washer_','RB_end_screw_','shear_web_','wing_root_fork_','hold_roof_lug_','hold_pivot_clevis_'))]
