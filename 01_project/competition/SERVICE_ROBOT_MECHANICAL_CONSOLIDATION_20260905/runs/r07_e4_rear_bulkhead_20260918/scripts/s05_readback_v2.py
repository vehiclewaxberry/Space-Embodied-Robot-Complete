# -*- coding: utf-8 -*-
"""R07-E4 STEP 读回机器验证 v2（build123d.import_step 自读回）：
...
D) 端框 Ø4.5 孔 4（既有未改计数核对）+ 端塞 Ø3.3 导孔 1 轴/塞（既有）；
E) 同轴：每角 窗口轴 vs 套 ID 轴 vs 端框孔轴 vs 栓杆轴（|d1×d2| 与轴线距机器数值）。
v2 修正（v1 FAIL 已归档 evidence/acc_readback_holes_coaxial_v1_fail.json，内容未改）：
 1) C 尖端判据 v1 误用 bb.min.X；尖端在 max 侧（头 x[-195,-191]，杆伸 +x 至尖 -161），
    v2 改为 bb.max.X == tip_x_S_mm；
 2) D 端塞导孔 v1 按面计数=1 过严：Ø3.3 导孔被 x=-164 Z 向 E2 分段销孔（Ø4.5）横穿，
    STEP 读回同一孔劈为 2 个同轴圆柱面（半径/轴位/轴向逐位相同，几何事实已探针核实），
    v2 改按唯一轴（y,z）计数=1/塞。
所有几何 S 系世界坐标，单位 mm。"""
...
def main():
    ...
    for r in spec:
        ...
        if not cf:
            ...
            bb = sc.bounding_box()
            tip_ok = abs(bb.max.X - RS['tip_x_S_mm']) < 1e-6
    ...
    for r in spec:
        pn = r['plug']
        faces = [c for c in cyl_faces(plugs[pn]) if abs(c[0] - R_PILOT) < 1e-6 and abs(abs(c[2][0]) - 1) < 1e-6]
        axes_yz = {(round(c[1][1], 6), round(c[1][2], 6)) for c in faces}
        counts[f'{pn}_d3p3_pilot_faces'] = len(faces)
        counts[f'{pn}_d3p3_pilot_unique_axes'] = len(axes_yz)
    counts['expected_plugs'] = '端塞 Ø3.3 X 导孔 1 轴/塞（既有未改；同孔被 x=-164 Z 向分段销孔横穿劈为 2 面，按唯一轴计数）'
    ...
    for r in spec:
        if counts[f"{r['plug']}_d3p3_pilot_unique_axes"] != 1:
            result['failures'].append(f"{r['plug']} pilot unique-axis count fail")
