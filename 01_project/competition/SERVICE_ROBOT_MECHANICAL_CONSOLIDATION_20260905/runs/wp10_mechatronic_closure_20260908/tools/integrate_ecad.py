...
def uid(s):return str(uuid.uuid5(uuid.NAMESPACE_URL,'wp10-aux-supply-system:'+s))
def parse(text):
...
parent_rows=list(csv.DictReader((C/'ecad/MASTER_FROM_TO.csv').open(encoding='utf-8-sig')))
...
for r in rows:
    ...
    if r['wire_id'] in changes:
        ...
        r['status']='WP10_ACTUAL_STOP_SUPPLY_DRIVER_REVISION; connector_MPN_and_harness_unbound'
        r['source']+='; WP10 electrical/STOP_PIN_NET_MAP.csv'
...
mapping=scope['schematic']('wp09_system',modules,nets,'WP10 actual auxiliary supplies and driver; GSE candidate; full system open','A2')
...
dump(E/'STOP_INTEGRATION_BINDING.json',{'parent_master':{'path':str(C/'ecad/MASTER_FROM_TO.csv'),'sha256':sha(C/'ecad/MASTER_FROM_TO.csv')},'child_master_sha256':sha(E/'MASTER_FROM_TO.csv'),'child_subpage_source_sha256':sha(S/'wp09_stop_circuit.kicad_sch'),'child_pinmap_sha256':sha(S/'STOP_PIN_NET_MAP.csv'),'root_generator_source_sha256':sha(N/'tools/build_ecad.py'),'source_board_net_to_system_global_net':net_global,'old_wire_ids_preserved':len(parent_rows),'wiring_rows':len(rows),'board_internal_edges_separate_but_source_bound':len(internal),'no_wire_length_or_manufacturing_credit':True,'auxiliary_supply_derivative_source':'WP10 electrical circuit; source guarantees and open conditions are separately verified','whole_flight_power_closed':False})
