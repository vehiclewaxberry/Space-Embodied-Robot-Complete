...
def main():
    ...
    rp_tol=(1+T['R207']['initial_tolerance_fraction'])*(1+T['R207']['TCR_per_K']*100)-1
    ck('R207_selected_native_code_and_bounds',nc['R207']==T['R207']['MPN'] and rp_nom==30100 and abs(rp_tol-.0035025)<1e-12)
    profiles=[]
    ...
    definition=dict(schema='WP10_HOTSWAP_REDUCED_TRANSIENT_V24',source_bindings=hashes,
        ...
        TIMER_threshold_scope='0.9..1.1uF effective-C requirement, zero leakage and zero initial timer voltage; not guaranteed maximum hardware shutdown time.',
    ...
    for idx,(vp,rsrc,rmain,raux,eta,pr,uvidx) in enumerate(product):
        ...
        if c.source(0)[2]>=c.uv_rise:
            ...
            faults.append(dict(case_id=cid,regulated_prefix_only=True,UVLO_continuous_regulation=valid,
                ...
                timer_threshold_zero_leak_requirement_s=tmax,
    ...
    with (A/'thermal/HOTSWAP_SHORT_BLOCK_LOADS_V24.csv').open('w',newline='',encoding='utf-8-sig') as g:
        heats=[dict(case_id=f['case_id'],block_W=f['fault_block_W'],timer_threshold_s=f['timer_threshold_zero_leak_requirement_s'],
