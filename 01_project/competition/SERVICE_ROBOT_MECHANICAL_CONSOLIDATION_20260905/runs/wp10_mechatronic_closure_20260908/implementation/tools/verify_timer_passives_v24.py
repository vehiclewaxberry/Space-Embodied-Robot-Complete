...
tolR=(1+.001)*(1+25e-6*100)-1;resistor=[]
...
def footprint_fit(pads):
 try:
  def nums(p,key):return [float(x) for x in children(p,key)[0][1:]]
  p1,p2=pads['1'],pads['2'];a1,a2=nums(p1,'at'),nums(p2,'at')
  d1=nums(p1,'drill');d2=children(p2,'drill')[0]
  actual=(len(pads)==2 and p1[2:4]==['thru_hole','circle'] and p2[2:4]==['thru_hole','oval']
   and a1[:2]==[0.,0.] and a2[:2]==[5.,0.]
   and (len(a1)==2 or a1[2]%180==0) and (len(a2)==2 or a2[2]%180==0)
   and d1==[.9] and d2[1]=='oval' and [float(x) for x in d2[2:]]==[2.,.9]
   and nums(p1,'size')==[1.7,1.7] and nums(p2,'size')==[2.8,1.7])
  margin=(float(d2[2])-.05-.55)/2-(.5+.05+.05)
  return actual and margin>0,margin
 except (KeyError,IndexError,ValueError):return False,None
fit,slot_margin=footprint_fit(pads)
ck('full_actual_footprint_pin_geometry',fit,slot_margin_mm=slot_margin)
import copy
for key in ['rotated_slot','shifted_slot','small_first_hole']:
 bad=copy.deepcopy(pads)
 if key=='rotated_slot':children(bad['2'],'at')[0].append('90')
 elif key=='shifted_slot':children(bad['2'],'at')[0][1]='6'
 else:children(bad['1'],'drill')[0][1]='0.4'
 ck('reject_'+key,not footprint_fit(bad)[0])
