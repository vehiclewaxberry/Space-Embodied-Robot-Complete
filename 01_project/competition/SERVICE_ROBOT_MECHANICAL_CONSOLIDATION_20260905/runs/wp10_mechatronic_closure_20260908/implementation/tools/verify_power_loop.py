...
from erc_source_contract import audit_contract, native_erc_summary
...
for args in [['sch','export','netlist','--format','kicadxml','-o',str(E/'wp10_system.xml')],['sch','export','pdf','-o',str(E/'wp10_system.pdf')],['sch','erc','--format','json','-o',str(A/'results/POWER_LOOP_ERC.json')]]:
 Path(args[args.index('-o')+1]).unlink(missing_ok=True) # generated artifact already archived; never reuse stale success
 p=subprocess.run([str(exe),*args,str(E/'wp10_system.kicad_sch')],capture_output=True,text=True,encoding='utf-8',errors='replace');ck('native_erc_command' if args[1]=='erc' else 'native_'+args[1]+'_'+args[2],p.returncode==0,stdout=p.stdout,stderr=p.stderr,returncode=p.returncode)
 if p.returncode!=0:raise RuntimeError('Native command failed; do not reuse an older export: '+repr(args))
...
source_audit=audit_contract(json.loads((A/'power/ERC_SOURCE_DECLARATIONS.json').read_text(encoding='utf-8')))
dump(A/'results/ERC_SOURCE_VALIDATION.json',source_audit)
ck('source_audited_conditional_power_declarations',source_audit['passed'],report='results/ERC_SOURCE_VALIDATION.json')
summary=native_erc_summary(erc)
ck('native_erc_report_covers_current_hierarchy_and_rules',summary['report_valid'])
out=dict(all_connectivity_checks_passed=all(c['passed'] for c in checks),count=len(checks),checks=checks,native_components=len(comps),ERC_count=len(v),ERC_types=types,ERC_clean=summary['ERC_clean'],ERC_errors=summary['errors'],ERC_warnings=summary['warnings'],ERC_command_succeeded=summary['command_succeeded'],ERC_ignored_checks=summary['ignored_checks'],whole_design_verified=False,energization_authorized=False)
...
assert out['ERC_clean'],'Native ERC command completed, but the current report is not clean'
