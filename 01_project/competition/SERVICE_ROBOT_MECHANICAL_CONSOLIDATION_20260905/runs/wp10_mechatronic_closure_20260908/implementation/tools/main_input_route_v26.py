...
# Prepare source text before native load; avoids KiCad10 SWIG removal ownership bug.
tree=parse((H/source.name).read_text(encoding='utf-8-sig'))
tree=[v for v in tree if not isinstance(v,list) or v[0] not in ['segment','via']]
for f in children(tree,'footprint'):
 ref=properties(f).get('Reference')
 if ref=='Q201':children(f,'at')[0][1:3]=['54','17']
 if ref=='C201':
  for g in children(f,'fp_rect'):
   if val(children(g,'layer')[0][1])=='F.SilkS':children(g,'end')[0][1]='6.8'
base=A/'ecad/wp10_main_input_v26_base.kicad_pcb';base.write_text(enc(tree)+'\n',encoding='utf-8')
b=k.LoadBoard(str(base));fps={f.GetReference():f for f in b.GetFootprints()}
for fp in fps.values():fp.SetLocked(True)
...
def route(r1,p1,r2,p2,w,mid=(),small=False,layer=k.F_Cu,role='SIGNAL_FIXED'):
 ...
 if r2=='U205' and p2=='5' and role in ['STARTUP_LOCAL_GROUND','TEST_RETURN']:
  path(a.GetNet(),[at(a),*mid,(63.3,42.85)],w,layer,role)
  seg(a.GetNet(),(63.3,42.85),at(c),.25,layer,role+'_PIN_NECK')
 elif r2=='U205' and p2=='11' and w>.25:
  path(a.GetNet(),[at(a),*mid],w,layer,role);seg(a.GetNet(),mid[-1],at(c),.25,layer,role+'_PIN_NECK')
 elif r2=='U201' and p2=='5' and w>.25:
  path(a.GetNet(),[at(a),*mid,(45.8,30.8)],w,layer,role);seg(a.GetNet(),(45.8,30.8),at(c),.25,layer,role+'_PIN_NECK')
 else:path(a.GetNet(),[at(a),*mid,at(c)],w,layer,role)
...
path(G,[(7,28),(17,28),(17,10),(83,10),(83,28),(93,28)],8,k.B_Cu,'MAIN_RETURN')
...
path(G,[at(pad('C202','2')),(33,10)],.8,k.B_Cu,'INPUT_BYPASS_RETURN')
...
path(G,[at(pad('U201','5')),(45.8,30.8)],.25,role='HS_SINGLE_POINT_GROUND_PIN_NECK')
path(G,[(45.8,30.8),(46.5,31),(46.5,32)],.4,role='HS_SINGLE_POINT_GROUND');via(G,46.5,32)
path(G,[(46.5,32),(46.5,10)],.5,k.B_Cu,'HS_SINGLE_POINT_GROUND_BACK')
...
path(G,[at(pad('U205','5')),(63.3,42.85)],.25,role='STARTUP_LOCAL_GROUND_PIN_NECK')
path(G,[(63.3,42.85),(63.5,43.5),(63.5,61)],.5,role='STARTUP_LOCAL_GROUND')
...
path(G,[(63.5,43.5),(62,43.5),(62,33.5),(63.5,33.5)],.5,role='STARTUP_SINGLE_POINT_GROUND');via(G,63.5,33.5)
path(G,[(63.5,33.5),(63.5,20),(68,20),(68,10)],.5,k.B_Cu,'STARTUP_SINGLE_POINT_GROUND_BACK')
...
out=dict(schema='WP10_V26_FIXED_COPPER',input_sha256=sha(H/source.name),PCB_sha256=sha(dest),DSN_sha256=sha(dsn),
 Q201_position_mm=[54,17],functional_refs_preserved=31,board_pads_refcounts_preserved=len(fps)==43,
