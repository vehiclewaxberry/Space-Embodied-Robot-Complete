...
ck('four_electrical_PTH_and_four_mount_NPTH',len(holes)==8 and sum(p['type']==g['pad_type_values']['NPTH'] for p in holes)==4 and sum(p['type']==g['pad_type_values']['PTH'] for p in holes)==4 and all(p['type']==g['pad_type_values']['PTH'] and p['number'] in ['1','2'] for p in front_pads))
...
out=dict(schema='WP10_C203_PHYSICAL_COPPER_AUDIT_V20_CAP_PTH',passed=all(c['passed'] for c in checks),checks=checks,check_count=len(checks),baseline=baseline,fault_injections=regression,
