"""V35 scenario consumer (additional downstream current allocation) for the two CPL branches and three distinct IQ/leak paths.
...
def operating_point(c,pack,shared,eta,hot,copper_C=100.,contact_extra_ohm=0.):
    ...
    apre=m['before_efuse_IQ_ohm']+contact_extra_ohm;iq=m['quiescent_current_A'];ic=.003;leak=.003;ctl=e['sequence_input_allocation_A']
    ...
    h=dict(Q201=im*im*r['Q201_25C'],shunts=im*im*r['shunts'],fuse=heat['main_before_sense'],
      ...
      aux_sequence_allocation=vt*ctl,STOP_output_allocation=e['aux_output_W'],brake_bias_output_allocation=1.)
    ...
    out=dict(equilibrium_found=True,pack_V=pack,junction_V=v,main_fused_V=vf,main_input_V=vm,aux_input_V=vt,
      sequence_input_A=ctl,THN_input_A=ia-ctl,efuse_input_V=va,main_A=im,aux_A=ia,battery_A=ib,controller_A=ic,main_leak_A=leak,efuse_quiescent_A=iq,
      input_power_W=pack*ib,heat_W=heat,heat_breakdown_W=h,accounted_non_arm_heat_W=sum(h.values()),
      power_balance_residual_W=pack*ib-mw-aw-vt*ctl-sum(heat.values()),
