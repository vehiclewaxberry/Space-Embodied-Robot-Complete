...
def add_startup(part, passive, parts, groups, nc):
    ...
    from main_board_selection_v25 import PARTS as selected
    for ref in set(parts)-before:
        if ref in selected:
            d=selected[ref]
            parts[ref].update(mpn=d['MPN'],footprint=d['footprint'],status=d['status'],source=d['source_url']+'; '+d['source_revision'])
