...
out=dict(schema='WP10_BACKGROUND_CLEANUP_V22',time_local=datetime.datetime.now().astimezone().isoformat(),before_available_MiB=before,after_samples_MiB=samples,start_threshold_MiB=2048,headroom_target_MiB=2304,threshold_met=min(samples)>=2048,rows=rows,retained=['Codex including its ChatGPT.exe hosts','Claude desktop sessions and Claude Code workers','native CAD and KiCad sessions','MCP services','network, sync, drivers and OS services'],scope='Only verified idle Widgets and SolidWorks preload processes terminated. Claude desktop resident pages trimmed only; pages can return on demand. No settings changed.')
p=A/'results/BACKGROUND_MEMORY_CLEANUP_V22.json';assert not p.exists(),'Do not overwrite cleanup receipt'
