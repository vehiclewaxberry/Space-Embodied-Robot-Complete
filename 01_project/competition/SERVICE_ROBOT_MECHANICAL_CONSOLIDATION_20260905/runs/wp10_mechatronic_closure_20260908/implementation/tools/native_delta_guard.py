...
import psutil
from native_delta_win_job import OwnedJob
...
assert a.name.startswith('native_delta_')
...
receipt['minimum_start_available_mib']=2048
receipt['SW_processes_at_start']=[p.info for p in psutil.process_iter(['pid','name','create_time']) if (p.info.get('name') or '').lower()=='sldworks.exe']
if not a.sw_owner:assert not receipt['SW_processes_at_start'],'Geometry tasks must run only with CAD COM server absent'
if a.sw_owner:
...
try:
    if v.available/2**20<2048:receipt.update(status="RESOURCE_BLOCKED_BEFORE_START",returncode=None)
    else:
