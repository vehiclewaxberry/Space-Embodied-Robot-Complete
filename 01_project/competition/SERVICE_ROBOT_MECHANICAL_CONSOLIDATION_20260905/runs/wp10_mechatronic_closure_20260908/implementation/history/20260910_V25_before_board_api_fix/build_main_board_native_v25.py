...
positions={
 ...
 'U206':[82,51,0],'C213':[88,57,90],'Q204':[91,43,0],
...
for ref,(x,y,ang) in positions.items():
 ...
 fp.Reference().SetTextSize(xy(.8,.8));fp.Reference().SetTextThickness(120000)
...
for ref,rr,pin,x,y,power in ports:
 ...
 pad.SetSize(xy(7 if power else 2.4,7 if power else 2.4));pad.SetDrillSize(xy(3.2 if power else 1.2,3.2 if power else 1.2));pad.SetLayerSet(k.LSET.AllCuMask()|k.LSET(k.F_Mask)|k.LSET(k.B_Mask))
 pad.SetPosition(xy(x,y));net=native[(rr,pin)];pad.SetNet(nets[net]);fp.Add(pad);b.Add(fp);fps[ref]=fp
 fp.Reference().SetPosition(xy(x,y+5 if power else y+2));fp.Reference().SetTextSize(xy(.7,.7));fp.Value().SetVisible(False)
...
for i,(x,y) in enumerate([(4,4),(96,4),(4,76),(96,76)],1):
 ...
 pad=k.PAD(fp);pad.SetNumber('');pad.SetAttribute(k.PAD_ATTRIB_NPTH);pad.SetShape(k.PAD_SHAPE_CIRCLE);pad.SetSize(xy(3.2,3.2));pad.SetDrillSize(xy(3.2,3.2));pad.SetLayerSet(k.LSET.AllCuMask()|k.LSET(k.F_Mask)|k.LSET(k.B_Mask));pad.SetPosition(xy(x,y));fp.Add(pad);b.Add(fp)
