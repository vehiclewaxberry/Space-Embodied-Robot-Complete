...
Run directly after native_delta_shared_battery_publish finishes. Do not wrap
...
guard=json.loads((A/'logs/native_delta_shared_battery_publish.run.json').read_text())
assert guard['status']=='COMPLETED' and guard['returncode']==0
review=json.loads((A/'results/SHARED_BATTERY_READONLY_REVIEW.json').read_text())
def sha(p):return hashlib.sha256(p.read_bytes()).hexdigest()
assert sha(A/'power/SHARED_BATTERY_PATH_CALCULATIONS.json')==review['reviewed_calculations_sha256']
assert sha(A/'thermal/SHARED_BATTERY_HEAT_LOADS.csv')==review['reviewed_thermal_csv_sha256']
...
temporary=(A.parent/'logs/wp10_shared_battery_final_package.tmp.zip').resolve()
