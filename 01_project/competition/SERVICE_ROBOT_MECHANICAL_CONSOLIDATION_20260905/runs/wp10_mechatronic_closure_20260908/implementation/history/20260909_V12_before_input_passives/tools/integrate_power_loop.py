...
for pi in range(math.ceil(len(items)/12)):
 ...
 sch.extend(objs);sch.append(textobj('Source-bound pins; fuse/capacitor MPNs, startup supervisor, clamp energy and thermal qualification remain OPEN.',10,270,pname+'note'))
...
(E/'sym-lib-table').write_text('(sym_lib_table (version 7) '+''.join(f'(lib (name "{l}") (type "KiCad") (uri "'+chr(36)+f'{{KIPRJMOD}}/{l}.kicad_sym") (options "") (descr "Same WP10 candidate"))' for l in ['WP09','WP09STOP','WP10POWER'])+')',encoding='utf-8')
...
Rs=.0022;rs_tol=.01+75e-6*100;kelvin_error=.00001
rsmin=Rs*(1-rs_tol)-kelvin_error;rsmax=Rs*(1+rs_tol)+kelvin_error
imin=.0485/rsmax;imax=.0615/rsmin
uv=[];uvrise=[];ov=[]
for th,rh,rl,bias in itertools.product([2.45,2.55],[47500*(1-.0035),47500*(1+.0035)],[5900*(1-.0035),5900*(1+.0035)],[-1e-6,1e-6]):
 threshold=th*(1+rh/rl)+bias*rh
 uv.append(threshold)
 for ihys in [12e-6,30e-6]:uvrise.append(threshold+ihys*rh)
for th,rh,rl,bias in itertools.product([2.4,2.6],[120000*(1-.0035),120000*(1+.0035)],[10000*(1-.0035),10000*(1+.0035)],[-1e-6,1e-6]):ov.append(th*(1+rh/rl)+bias*rh)
scenarios=[]
...
for ref,v in parts.items():
 ...
 bom.append(dict(role=ref+': '+v['role'],manufacturer='see source',MPN=v['mpn'],quantity=1,status=v['status'],source=v['source']))
...
print(json.dumps(dict(parent_refs=99,new_refs=len(parts),native_pages=2+len(pages),wire_ids_preserved=len(rows),conditional_current_limit_A=[imin,imax],whole_design_closed=False)))
