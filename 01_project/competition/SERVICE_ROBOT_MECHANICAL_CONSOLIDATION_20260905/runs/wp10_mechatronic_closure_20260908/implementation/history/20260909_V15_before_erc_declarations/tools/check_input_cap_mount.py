...
checks=[]
fp=(A/'ecad/WP10_PASSIVES.pretty/CP_ChemiCon_VS_D30_P10_2mm_Candidate.kicad_mod').read_text()
pads={m[0]:[float(m[1]),float(m[2])] for m in re.findall(r'\(pad "([12])" thru_hole \w+ \(at ([-\d.]+) ([-\d.]+)\)',fp)}
checks.append(dict(id='ACTUAL_FOOTPRINT_PAD_LOCATIONS',pads=pads,passed=pads=={'1':[-5.,0.],'2':[5.,0.]}))
checks.append(dict(id='ACTUAL_FOOTPRINT_DRILL',passed=len(re.findall(r'\(drill 2\)',fp))==2))
