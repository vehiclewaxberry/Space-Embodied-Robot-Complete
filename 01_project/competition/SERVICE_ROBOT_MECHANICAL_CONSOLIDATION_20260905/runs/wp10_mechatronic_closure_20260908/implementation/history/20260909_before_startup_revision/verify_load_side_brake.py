...
ck('PG_enables_driver_INplus',same('U301.6','U303.6','R303.2'))
...
bias=dict(MPN=parts['U301']['mpn'],nominal_V=vnom,voltage_sensitivity_V=[vmin,vmax],
          ...
          driver_VIH_max_V=2.4,driver_VIL_limit_V=1.0,
          ...
          output_effective_min_cap_F=3.3e-6,selected_output_nominal_cap_F=10e-6,installed_cap_derating_verified=False)
ck('PG_low_current_screen_under_50uA',bias['PG_low_current_upper_sensitivity_A']<50e-6)
ck('PG_high_nominal_logic_margin',bias['PG_high_V_typical_200k_input']>2.4)
...
for old in power['scenarios']:
    ...
    updated.append(dict(battery_loaded_V=vb,main_eta_assumed=eta,path_R_ohm=r,main_input_A=im,
       aux_input_A=old['aux_input_A'],battery_total_A=im+old['aux_input_A'] if im else None,
...
out=dict(schema='WP10_LOAD_SIDE_BRAKE_ANALYTICAL_V1',status='ACTUAL_CIRCUIT_STATIC_SCREEN__DYNAMIC_ENERGY_THERMAL_OPEN',
    native_netlist_sha256=sha(A/'ecad/wp10_system.xml'),definition_sha256=sha(A/'power/LOAD_SIDE_BRAKE_DEFINITION.json'),
    selected_parts_sha256=sha(A/'power/POWER_LOOP_PARTS.json'),bias=bias,absolute_OV=ov,thermal_monitor=thermal,
...
power['regen'].update(absolute_sink_circuit_integrated=True,selected_circuit='LT3013+TLV6700+UCC27511+3xLPS0300H1R00JB+3xCSD19536KTT',
