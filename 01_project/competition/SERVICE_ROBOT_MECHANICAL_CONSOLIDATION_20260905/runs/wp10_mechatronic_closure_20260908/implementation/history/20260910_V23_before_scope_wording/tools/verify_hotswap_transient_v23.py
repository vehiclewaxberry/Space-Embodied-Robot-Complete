...
def main():
    ...
    paths=['tools/hotswap_transient_model_v23.py','tools/verify_hotswap_transient_v23.py',
        ...
        'results/BRAKE_ENABLE_NATIVE_V22.json','SYSTEM_CLOSURE_MATRIX.csv',
    ...
    ck('C201_value_unchanged_not_new_physical_part',nc['C201']=='1uF_10pct_16V')
    ...
    match=re.fullmatch(r'(\d+(?:\.\d+)?)k_(\d+(?:\.\d+)?)pct',nc['R207'])
    assert match,'R207 engineering value format changed; rebind instead of guessing'
    rp_nom=float(match[1])*1000;rp_tol=float(match[2])/100
    ck('R207_current_native_value_parsed',rp_nom==30100 and rp_tol==.01)
    profiles=[]
    ...
    definition=dict(schema='WP10_HOTSWAP_REDUCED_TRANSIENT_V23',source_bindings=hashes,
        active_electrical_native_revision='V22_UNCHANGED',active_mechanical_revision='V21_UNCHANGED',
        C201_selected_nominal_F=1e-6,C201_nominal_tolerance=.1,C201_MPN_bound=False,
    ...
    dump('power/HOTSWAP_TRANSIENT_DEFINITION_V23.json',definition)
    ...
    for cv in [.68e-6,.82e-6,1e-6]:
        ...
        candidates.append(dict(nominal_F=cv,initial_tolerance=.1,
            ...
            installed=cv==1e-6,chosen_new_part=False,SOA_verified=False,C_effective_and_leakage_verified=False))
    ...
    result=dict(schema='WP10_HOTSWAP_TRANSIENT_SCREEN_V23',source_bindings=hashes,
        definition_sha256=sha('power/HOTSWAP_TRANSIENT_DEFINITION_V23.json'),checks=checks,
        ...
        selection_decision='Retain C201 1uF allocation. 680nF fails this sampled1.5 allowance;820nF remains unselected, effective C/leakage/gate dynamics and SOA open.',
        ...
        native_rerun=False,native_revision_unchanged='V22',whole_design_complete=False,
        SOA_full_trajectory_verified=False,physical_tests_executed=False,power_on_release=False)
    with (A/'power/HOTSWAP_WORST_PREFIX_TRACE_V23.csv').open('w',newline='',encoding='utf-8-sig') as g:
        w=csv.DictWriter(g,fieldnames=list(trace[0]));w.writeheader();w.writerows(trace)
    with (A/'thermal/HOTSWAP_SHORT_BLOCK_LOADS_V23.csv').open('w',newline='',encoding='utf-8-sig') as g:
    ...
    dump('power/HOTSWAP_TRANSIENT_CALCULATIONS_V23.json',result)
