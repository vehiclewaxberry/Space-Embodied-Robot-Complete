...
def shape(key):
    if key not in cache:
        r=rows[key];rr=STEPControl_Reader();assert int(rr.ReadFile(r['step_path']))==1;rr.TransferRoots();s=rr.OneShape();M=matrices.get(key,np.eye(4))@np.array(r['T_S_step']);t=gp_Trsf();t.SetValues(*[float(M[i,j]) for i in range(3) for j in range(4)]);cache[key]=BRepBuilderAPI_Transform(s,t,True).Shape()
    return cache[key]
