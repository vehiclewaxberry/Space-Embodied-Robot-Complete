...
import json,hashlib,math,itertools,gc,re,xml.etree.ElementTree as ET
import numpy as np
...
assert set(plan['states'])=={'service','parking','released'} and plan['component_count']==965
assert all(sha(A/p)==h for p,h in c['source_files'].items())
...
out['inputs'].update({p:sha(A/p) for p in c['source_files']})
out['inputs']['ecad/wp10_system.xml']=sha(A/'ecad/wp10_system.xml')
...
for band in ['A','B']:
 ...
 # Shared edges at the split plane are intended; positive common volume still fails.
 allow('C203_CARRIER','C203_LINER_'+band+'_BOTTOM');allow('C203_LOWER_'+band,'C203_LINER_'+band+'_TOP')
...
terminal=read('power/CAP_TERMINAL_DEFINITION.json')
fp=parse((A/('ecad/WP10_PASSIVES.pretty/'+c['native_footprint'].split(':')[1]+'.kicad_mod')).read_text())
electrical=[p for p in children(fp,'pad') if val(p[1]) in ['1','2']]
holes=[p for p in children(fp,'pad') if p[2]=='np_thru_hole']
got=sorted((val(p[1]),tuple(map(float,children(p,'at')[0][1:3]))) for p in electrical)
want=sorted((p['logical_pin'],tuple(p['local_xy_mm'])) for p in terminal['terminal_features'])
checks.append(dict(id='ACTUAL_FOUR_PHYSICAL_PAD_LOCATIONS',pads=got,passed=got==want))
got_h=sorted((tuple(map(float,children(p,'at')[0][1:3])),float(children(p,'drill')[0][1])) for p in holes)
want_h=sorted((tuple(p['local_xy_mm']),p['drill_mm']) for p in terminal['terminal_features'])
checks.append(dict(id='ACTUAL_FOUR_NONPLATED_TERMINAL_HOLES',passed=got_h==want_h))
xml=ET.parse(A/'ecad/wp10_system.xml');nets={n.get('name'):{v.get('ref')+'.'+v.get('pin') for v in n.findall('node')} for n in xml.findall('.//net')}
def samenet(*pins):return any(set(pins)<=p for p in nets.values())
checks.append(dict(id='ACTUAL_NATIVE_PLUS_NET',passed=samenet('C203.1','U203.1','Q201.3')))
checks.append(dict(id='ACTUAL_NATIVE_ISOLATED_INPUT_MINUS',passed=samenet('C203.2','J200.2') and not samenet('C203.2','J203.2')))
comp=next(n for n in xml.findall('.//comp') if n.get('ref')=='C203')
checks.append(dict(id='ACTUAL_NATIVE_MPN_AND_FOOTPRINT',passed=comp.findtext('value')==c['MPN'] and comp.findtext('footprint')==c['native_footprint']))
...
for state,sp in plan['states'].items():
 ...
 assert len(rows)==len(sp['rows'])==965 and len(parents)==936 and set(parents)<=set(rows)
 assert sp['changed_ids']==['upper_equipment_deck_B'] and len(sp['added_ids'])==len(set(sp['added_ids']))==29 and set(sp['added_ids'])==set(rows)-set(parents)
 ...
 result['passed']=result['unchanged_935_rows_identical'] and all(r['valid'] and r['volume_mm3']>0 for r in valid) and not any(result[q] for q in ['collisions','unexpected_contacts','missing_contacts','keepout_collisions','tool_collisions']) and all(abs(result['deck_cut_locality'][q])<1e-6 for q in ['added_mm3','outside_cut_mm3','missing_cut_mm3'])
