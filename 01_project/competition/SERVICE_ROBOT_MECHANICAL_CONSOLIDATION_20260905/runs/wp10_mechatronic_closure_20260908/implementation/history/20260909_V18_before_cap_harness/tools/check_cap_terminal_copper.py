...
ck('single_printed_copper_face',all(p['copper_layers']==['B.Cu'] for p in g['pads'] if p['number']) and all(p['size_mm']==p['drill_mm'] for p in holes) and all(t['layer']=='B.Cu' for t in g['tracks']) and g['zone_count']==0)
...
ck('all_holes_NPTH',len(holes)==8 and all(p['type']==3 for p in holes))
ck('native_stackup_70um_readback',s['layers_mm']['B.Cu']==.07 and s['layers_mm']['F.Cu']==0 and abs(sum(s['layers_mm'].values())-1.6)<1e-12)
...
for t in d['terminal_features']:
    ...
    if t['id'].startswith('WIRE'):
        ...
        ck(t['id']+'_pad_only_side_spacing',side>=2,nominal_mm=side,tolerances_included=False)
...
old_p='history/20260909_V16_before_cap_terminal/mechanical/input_cap_pcb.step';new_p='mechanical/input_cap_pcb.step'
def loadstep(p):
    rd=STEPControl_Reader();assert int(rd.ReadFile(str(A/p)))==1;rd.TransferRoots();return rd.OneShape()
old_shape=loadstep(old_p);new_shape=loadstep(new_p)
cutters=[]
for t in d['terminal_features']:
    if t['id'].startswith('WIRE_'):
        _,y,z=t['S_face_mm'];cutters.append(BRepPrimAPI_MakeCylinder(gp_Ax2(gp_Pnt(-8,y,z),gp_Dir(1,0,0)),t['drill_mm']/2,4).Shape())
tool=union(cutters);removed=op(BRepAlgoAPI_Cut,old_shape,new_shape);expected=op(BRepAlgoAPI_Common,old_shape,tool)
locality=dict(removed_mm3=vol(removed),expected_mm3=vol(expected),added_mm3=vol(op(BRepAlgoAPI_Cut,new_shape,old_shape)),outside_cut_mm3=vol(op(BRepAlgoAPI_Cut,removed,expected)),missing_cut_mm3=vol(op(BRepAlgoAPI_Cut,expected,removed)))
ck('STEP_change_exactly_two_1p8mm_wire_holes',abs(locality['removed_mm3']-2*math.pi*.9**2*1.6)<1e-6 and all(abs(locality[k])<1e-6 for k in ['added_mm3','outside_cut_mm3','missing_cut_mm3']),**locality)
...
inputs += [old_p,new_p]
out=dict(schema='WP10_C203_PHYSICAL_COPPER_AUDIT_V17',passed=all(c['passed'] for c in checks),checks=checks,check_count=len(checks),baseline=baseline,fault_injections=regression,
    ...
    installed_wire_length_mm=None,installed_loop_inductance_H=None,PCB_manufacturing_DRC_clean=False,whole_design_complete=False,inputs={p:sha(p) for p in inputs})
