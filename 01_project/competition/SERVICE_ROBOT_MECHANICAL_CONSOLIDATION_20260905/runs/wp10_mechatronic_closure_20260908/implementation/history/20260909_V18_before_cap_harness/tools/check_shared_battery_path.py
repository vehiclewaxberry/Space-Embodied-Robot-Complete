...
q=solve(25,.1,.04,.005,0,0,0,controller_a=.002,main_leak_a=.003)
...
passives=read('power/INPUT_PASSIVE_SELECTION.json')
ck('selected_F202_bound_with_nonnegative_other_aux_R',parts['F202']['mpn']==passives['F202']['MPN'] and all(r>=passives['F202']['typical_cold_R_ohm'] for r in c['cases']['aux_branch_R_ohm']) and c['cases']['aux_fuse_typical_cold_R_ohm']==passives['F202']['typical_cold_R_ohm'])
...
for vb, rc, mr, ar, eta, aeta in itertools.product(cases['pack_V'], cases['shared_contact_loop_R_ohm'], cases['main_branch_allocations_ohm'], cases['aux_branch_R_ohm'], cases['main_efficiency'], cases['aux_efficiency']):
    mw=361/eta+.25; aw=16.8/aeta; ctrl=cases['controller_input_A']; leak=cases['main_leak_A']
    args=dict(pack_v=vb,shared_r=rc,main_r=mr['total'],main_pre_r=mr['before_MAIN_FUSED'],aux_r=ar,main_w=mw,aux_w=aw,controller_a=ctrl,main_leak_a=leak)
    run=solve(**args); cold=solve(**dict(args,main_w=0,main_leak_a=0))
    if cold['equilibrium_found']:
        # The equivalent branch has zero load, but its physical series switch
        # is open. PRECHARGED voltage depends on stored charge/leakage, not KVL
        # through a conducting resistor at zero current.
        cold['algebraic_zero_load_extension_main_input_V']=cold['main_input_V']
        cold['main_input_V']=None
        cold['main_switch_model_state']='OPEN; downstream capacitor voltage unbound'
        cold['conversion_and_bias_heat_W']=dict(THN_conversion=16.8*(1/aeta-1),startup_bias_allocation=0)
        cold['delivered_output_allocation_W']=16.8
        cold['source_to_output_balance_residual_W']=cold['input_power_W']-16.8-sum(cold['heat_W'].values())-sum(cold['conversion_and_bias_heat_W'].values())
...
for mr, ar, eta, aeta in itertools.product(cases['main_branch_allocations_ohm'],cases['aux_branch_R_ohm'],cases['main_efficiency'],cases['aux_efficiency']):
    q=hold_boundary(25.2,max(uvfall),.1,mr['total'],mr['before_MAIN_FUSED'],ar,361/eta+.25,16.8/aeta,cases['controller_input_A'],cases['main_leak_A'])
    if q:
        verify=solve(q['required_pack_V_for_given_shared_R'],.1,mr['total'],mr['before_MAIN_FUSED'],ar,361/eta+.25,16.8/aeta,cases['controller_input_A'],cases['main_leak_A'])
...
ck('cold_output_thermal_ledger_includes_THN_loss',all(abs(r['cold']['source_to_output_balance_residual_W'])<1e-7 and r['cold']['conversion_and_bias_heat_W']['THN_conversion']>0 for r in rows if r['cold']['equilibrium_found']))
ck('cold_open_main_switch_does_not_claim_live_PRECHARGED_voltage',all(r['cold']['main_input_V'] is None and r['cold']['main_A']==0 for r in rows if r['cold']['equilibrium_found']))
...
inputs=['power/SHARED_BATTERY_PATH_DEFINITION.json','power/POWER_LOOP_CALCULATIONS.json','power/STARTUP_CIRCUIT_CALCULATIONS.json','power/POWER_LOOP_PARTS.json','ecad/wp10_system.xml','power/INPUT_PASSIVE_SELECTION.json',c['active_geometry_plan'],c['connector']['document'],'sources/pmm35.pdf','power/BATTERY_INSTALLATION_INTERFACE.json']
out=dict(schema='WP10_COUPLED_SHARED_BATTERY_DC_V3',status='SHARED_CONTACT_AND_SELECTED_FUSE_COLD_LOSS_BOUND__WHOLE_POWER_OPEN',
         ...
         same_source_native_topology_bound=True,native_schematic_modified=True,
         ...
         continuous_motion_or_current965_geometry_requalified=False,
...
for r in rows:
    for state in ['run','cold']:
        if not r[state]['equilibrium_found']:continue
        for node, watts in (r[state]['heat_W']|r[state]['conversion_and_bias_heat_W']).items():
            thermal.append(dict(case_id=r['id'],state=state,node=node,heat_W=watts,
                                condition='ASSUMING_ON_DC_ALLOCATION_NOT_MEASURED' if state=='run' else 'MAIN_SWITCH_OPEN_AUX_ON_ALLOCATION_NOT_MEASURED',
...
anchor=next(r for r in rows if r['inputs']==dict(pack_v=25.2,shared_r=.1,main_r=.04,main_pre_r=.005,aux_r=.04,main_w=361/.85+.25,aux_w=16.8/.8,controller_a=.003,main_leak_a=.003))
