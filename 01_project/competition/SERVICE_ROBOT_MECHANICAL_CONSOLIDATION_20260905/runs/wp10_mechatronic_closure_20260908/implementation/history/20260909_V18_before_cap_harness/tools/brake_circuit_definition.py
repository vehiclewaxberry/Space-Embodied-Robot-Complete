...
def add_brake(part,passive,parts,groups,nc):
 ...
 part('U303','UCC27511DBVR',{'1':'VDD','2':'OUTH','3':'OUTL','4':'GND','5':'IN-','6':'IN+'},'ucc27511.pdf RevF pp6,9,10','One gate driver; ON only with PG high and OV low')
 for ref,value,role in [
  ('R301','86.6k_0.1pct_25ppm','12V bias upper'),('R302','10k_0.1pct_25ppm','12V bias lower'),
  ('R303','270k_0.1pct_25ppm','PG pullup; never exceed50uA sink'),
  ...
  ('C302','10uF_50V_EFFECTIVE_MIN3.3uF','Bias output stability capacitor; derating unbound'),
  ('C303','4.7nF_10pct_16V_C0G','PG delay; max startup duration unbound'),
  ('C304','100nF_25V','OV comparator local bypass'),('C305','1uF_25V','Gate driver local bypass'),
 ...
 g('BRAKE_BIAS_PG','U301.6','R303.2','U303.6')
