...
def ohm(ref):
    m=re.match(r'([0-9.]+)(M|k|ohm)',parts[ref]['mpn']); assert m,ref
    return float(m[1])*{'M':1e6,'k':1e3,'ohm':1}[m[2]]
...
ck('new_not_EOL_MPN',parts['Q204']['mpn']=='2N7002K-7' and 'DiodesInc' in parts['Q204']['source'])
...
制动侧继续保持现有LT3013/TLV6700/UCC27511。只读比较发现TPS3760的最大延迟要求特定过驱与测试负载；在27.44V名义阈值下，20%过驱为32.928V，不能据此宣称低于DM公开32V值完成钳位，故未为获得“更短数字”替换原OV电路。
