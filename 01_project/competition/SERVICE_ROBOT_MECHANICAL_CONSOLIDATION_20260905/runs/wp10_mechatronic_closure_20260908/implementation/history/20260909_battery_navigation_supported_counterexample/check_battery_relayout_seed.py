...
for key,q in overrides.items():
    assert q['coordinate_basis']=='S_WORLD_MM' and key not in matrices
...
for key,q in overrides.items():
    ...
    override_bbox_checks.append(dict(id=key,actual_world_bbox_mm=actual.tolist(),contained_in_parent_world_bbox=True,only_material_removed=q['only_material_removed']))
...
out.update(status='CANDIDATE_HAS_GEOMETRY_COUNTEREXAMPLES' if bad or collisions or unexpected else 'BODY_AND_MOVED_GROUPS_SCREEN_CLEAR__MOUNT_AND_REROUTE_PENDING',
 ...
 unexpected_zero_contacts=unexpected,nominal_support_contacts_allowed=[list(x) for x in sorted(allowed)],
