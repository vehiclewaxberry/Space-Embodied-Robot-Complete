...
def main():
    ...
    q=361*(1/.85-1);Rc=.75*25.4**2/g['CHB_TIM_contact_area_mm2']+.017771/(130*g['CHB_TIM_contact_area_mm2']*1e-6)
    load=dict(sheet=0,center=[-25,0],size=[55.9,59],power_W=q)
    endpoints=[dict(sheet=0,center=[-25,0],size=[58,62]),dict(sheet=1,center=[-25,-76],size=[58,24]),dict(sheet=2,center=[-25,-76],size=[58,24])]
    rows=[]
    for state in ['service','parking','released']:
        for sun in [False,True]:
            ...
            for i,k in enumerate(keys):
                ...
                envs.append(dict(grid=row['model_surface_refinement'][-1]['spatial_grid'],occluder_K=330.,space_K=3.,solar_W_m2=1361. if sun and i==1 else 0.))
            for R in [None,.25,.5,1.,2.]:
                links=[] if R is None else [dict(a=endpoints[0],b=endpoints[i],R_K_W=R) for i in [1,2]]
                r=solve(sheets,[load],links,envs);r.update(state=state,sun_on_plus_Y=sun,per_link_R_K_W=R,case_C=r['source_peak_C'][0]+q*Rc,geometry_built=False,CHB_only=True)
                rows.append(r)
    tests=[]
    ...
    out=dict(schema='WP10_SPATIAL_RADIATOR_NETWORK_SCREEN_V1',inputs={p:sha(p) for p in ['thermal/RADIATOR_MESH_VIEW_SCREEN.json','thermal/BOTTOM_RADIATOR_MOUNT.json','results/FIXED_HEAT_GEOMETRY.json','mechanical/FIXED_HEAT_INSTANCE_PLAN.json']},source_sha256=sha('tools/spatial_radiator_network.py'),rows=rows,checks=tests,checks_passed=all(t['passed'] for t in tests),checks_are_numerical_not_thermal_release=True,source_load_W=q,case_interface_R_K_W=Rc,links_proposed_endpoints=endpoints,links_are_unbuilt=True,environment_is_illustrative_not_orbit_bound=True,other_module_heat_not_included=True,continuous_full_vehicle_thermal_verified=False,omissions=['uncertain local Monte Carlo view fractions and interpolation','gray multi-bounce exchange between components','EOL coating/contact/pressure variation','3D spreading and clamp stresses','PCB/THN/brake/battery/PMM heat','orbital attitude and external flux histories','temperature dependence of k','harness changes and as-built properties'])
