...
if phase=='label_polarity':
    ...
    stack=['stackup']
    for layer,kind,t in [('F.Mask','Top Solder Mask',.01),('F.Cu','copper',0),('dielectric 1','core',1.5),('B.Cu','copper',.07),('B.Mask','Bottom Solder Mask',.02)]:
    ...
    assert layers=={'F.Mask':.01,'F.Cu':0.0,'dielectric 1':1.5,'B.Cu':.07,'B.Mask':.02},layers
    ...
    dump(A/'results/CAP_TERMINAL_STACKUP.json',dict(native_board_sha256=sha(P),layers_mm=layers,finished_thickness_mm=1.6,
        single_printed_copper_face=True,material='FR4_CANDIDATE; laminate supplier/Tg/thermal/outgassing not qualified',
    ...
    b=k.LoadBoard(str(P));mm=k.ToMM
    def xy(v):return [mm(v.x),mm(v.y)]
    pads=[]
    for f in b.GetFootprints():
        for p in f.Pads():pads.append(dict(ref=f.GetReference(),number=p.GetNumber(),uuid=p.m_Uuid.AsString(),xy_mm=xy(p.GetPosition()),size_mm=xy(p.GetSize()),drill_mm=xy(p.GetDrillSize()),circular=p.GetShape()==k.PAD_SHAPE_CIRCLE,type=int(p.GetAttribute()),net=p.GetNetname(),copper_layers=[b.GetLayerName(n) for n in [k.F_Cu,k.B_Cu] if p.IsOnLayer(n)]))
    ...
    binding=json.loads((A/'results/CAP_TERMINAL_NATIVE_BINDING.json').read_text());binding.update(board_sha256=sha(P),source_xml_sha256=sha(E/'wp10_system.xml'),physical_holes_nonplated=8)
    dump(A/'results/CAP_TERMINAL_NATIVE_BINDING.json',binding)
    out=dict(board_sha256=sha(P),native_C203_binding_sha256=sha(A/'results/CAP_TERMINAL_NATIVE_BINDING.json'),board_thickness_mm=mm(b.GetDesignSettings().GetBoardThickness()),pads=pads,tracks=tracks,zone_count=b.GetAreaCount(),edges=[dict(start_mm=xy(d.GetStart()),end_mm=xy(d.GetEnd())) for d in b.GetDrawings() if d.GetLayer()==k.Edge_Cuts])
