...
def solve(pack_v, shared_r, main_r, main_pre_r, aux_r, main_w, aux_w,
          controller_a=0.003, main_leak_a=0.0):
    assert pack_v > 0 and min(shared_r, main_r, main_pre_r, aux_r,
                              main_w, aux_w, controller_a, main_leak_a) >= 0
    assert main_r >= main_pre_r
    shift = main_pre_r * controller_a + main_r * main_leak_a
    lower = max(shift + 2*math.sqrt(main_r*main_w),
                2*math.sqrt(aux_r*aux_w), 1e-10)

    def branches(v):
        icp = current(v-shift, main_r, main_w)
        return (None if icp is None else icp + main_leak_a), current(v, aux_r, aux_w)
    ...
    heat = dict(shared_contact_loop=total*total*shared_r,
                ...
                input_capacitor_leakage=main_in*main_leak_a)
    ...
    return dict(equilibrium_found=True, junction_V=v, main_fused_V=fused,
                ...
                controller_A=controller_a, main_leak_A=main_leak_a, battery_A=total,
...
def hold_boundary(pack_v, required_sense_v, shared_r, main_r, main_pre_r,
                  aux_r, main_w, aux_w, controller_a=0.003, main_leak_a=0.0):
    """Invert a fixed MAIN_FUSED threshold; not a selected battery cutoff."""
    post_r = main_r-main_pre_r
    im = current(required_sense_v-post_r*main_leak_a, post_r, main_w)
    if im is None:
        return None
    im += main_leak_a
