...
terminal=read('power/CAP_TERMINAL_DEFINITION.json');copper=read('results/CAP_TERMINAL_COPPER_AUDIT.json')
assert copper['passed'] and all(sha(q)==h for q,h in copper['inputs'].items())
interface['schema']='WP10_C203_CAD_ECAD_INTERFACE_V17'
interface['bindings'].update({q:sha(q) for q in ['power/CAP_TERMINAL_DEFINITION.json','results/CAP_TERMINAL_COPPER_AUDIT.json','results/CAP_TERMINAL_STACKUP.json',terminal['board'],'power/C203_WIRE_SCHEDULE.csv','ecad/WP10_PASSIVES.pretty/'+c['native_footprint'].split(':')[1]+'.kicad_mod']})
interface.update(board_routed=True,board_path=terminal['board'],PCB_manufacturing_DRC_clean=False,
    PCB_DRC_open_types={'hole_clearance':8,'solder_mask_bridge':4},PCB_DRC_scope='12 local NPTH solder-land process/model findings retained, not ignored or accepted',
    actual_copper_connectivity_passed=True,physical_terminal_features=[dict(**t,hole_entry_plane='component face',nominal_solder_land_S_mm=[c['board_x_mm'][1]-terminal['native_stackup_mm']['B.Mask'],t['S_face_mm'][1],t['S_face_mm'][2]]) for t in terminal['terminal_features']],
    wire_material=terminal['wire']['MPN'],wire_cut_lengths_bound=False,CHB_connection_complete=False)
dump('ecad/C203_MECHANICAL_INTERFACE.json',interface)
