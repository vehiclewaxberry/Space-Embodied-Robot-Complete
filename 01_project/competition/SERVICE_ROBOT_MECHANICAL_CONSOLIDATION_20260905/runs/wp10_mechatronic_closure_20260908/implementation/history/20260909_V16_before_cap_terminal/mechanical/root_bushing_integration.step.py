...
def gen_step():
    p=json.loads((A/'mechanical/ROOT_BUSHING_PREVIEW_INPUTS.json').read_text());assert all(hashlib.sha256((A/k).read_bytes()).hexdigest()==h for k,h in p['inputs'].items())
    a=AssemblyHelper('WP10_ROOT_BUSHING_LOCAL__WHOLE_DESIGN_OPEN');root=None
    for r in p['rows']:
