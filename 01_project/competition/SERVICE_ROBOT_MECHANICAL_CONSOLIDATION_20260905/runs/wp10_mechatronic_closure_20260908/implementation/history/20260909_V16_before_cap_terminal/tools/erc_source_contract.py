...
def audit_contract(contract, root_path=None, xml_path=None):
    ...
    ck('exact_201_entity_properties_preserved',newc==oldc and len(newc)==201)
    ...
    return dict(schema='WP10_ERC_SOURCE_AUDIT_V16',passed=all(c['passed'] for c in checks),check_count=len(checks),checks=checks,
                ...
                inputs={p.relative_to(A).as_posix():sha(p) for p in sheets+[xml_path,A/'power/ERC_SOURCE_DECLARATIONS.json',A/'tools/erc_source_contract.py']},
