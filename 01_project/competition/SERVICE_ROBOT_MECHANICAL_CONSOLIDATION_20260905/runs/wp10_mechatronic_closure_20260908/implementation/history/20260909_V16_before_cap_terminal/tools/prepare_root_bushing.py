...
names=['BUSH_LEFT','BUSH_RIGHT','KEEPER','SCREW_8','BRIDGE']
if a.phase=='design':
    ...
    c=dict(schema='WP10_ROOT_PASSAGE_SPLIT_BUSHING_V1',parent_source_plan=source,parent_source_sha256=sha(A/source),
        ...
        keeper_throat_ID_mm=7,flange_axial_float_mm=.1,screw_xy_S_mm=[[53,-80],[53,-70]],screw_xy_local_mm=[[-12,0],[-12,10]],keeper_outline_xy_local_mm=[-16,16,-8,14],keeper_boss_x_local_mm=[-16,-9],clearance_hole_D_mm=3.4,head_D_mm=6,head_height_mm=1.5,screw_D_mm=3,thread_envelope_depth_mm=4.5,entry_chamfer_mm=.5,screw_nominal_total_length_mm=8,
    ...
    assert abs(c['flange_z_local_mm'][0]-c['keeper_z_local_mm'][1]-c['flange_axial_float_mm'])<1e-12
    assert [[x+c['datum_S_mm'][0],y+c['datum_S_mm'][1]] for x,y in c['screw_xy_local_mm']]==c['screw_xy_S_mm']
    for state,sp in parent['states'].items():
        ...
        for name in names[:3]:put('ROOT_'+name,'root_'+name.lower(),translation(*c['datum_S_mm']))
        for i,(x,y) in enumerate(c['screw_xy_S_mm']):put(f'ROOT_SCREW_{i}','root_screw_8',translation(x,y,c['datum_S_mm'][2]+c['keeper_z_local_mm'][0]))
        assert all(rows[k]==r for k,r in ((q['id'],q) for q in sp['rows']) if k.startswith('release_power_data_route_'))
        assert len(rows)==936 and len(changed)==1 and len(added)==5
    ...
    out=dict(schema='WP10_ROOT_BUSHING_SOURCE_VARIANT_V1',source_script_sha256=sha(__file__),inputs={q:sha(A/q) for q in [c['parent_source_plan'],'mechanical/ROOT_BUSHING_DESIGN.json','mechanical/root_bushing_common.py']},states=states,component_count=936,component_count_identity='931 +2 split sleeve halves +1 side-entry metal keeper +2 nominal screws; original bridge ID replaced',whole_fit_verified=False,thermal_mass_native_requalified=False,whole_design_complete=False)
    (A/'mechanical/ROOT_BUSHING_INSTANCE_PLAN.json').write_text(json.dumps(out,indent=2),encoding='utf-8');print(json.dumps(dict(source_instances=936)))
