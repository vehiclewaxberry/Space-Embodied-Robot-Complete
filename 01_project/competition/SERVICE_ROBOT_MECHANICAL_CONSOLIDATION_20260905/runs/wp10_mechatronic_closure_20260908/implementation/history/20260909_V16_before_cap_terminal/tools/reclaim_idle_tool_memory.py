...
out=dict(schema='WP10_IDLE_CODEX_TOOL_WORKING_SET_RECLAIM_V1',time_local=datetime.datetime.now().astimezone().isoformat(),before_available_MiB=before,after_available_MiB=after,rows=rows,processes_terminated=[],method='Microsoft psapi EmptyWorkingSet; pages may be faulted back on demand; not process termination or permanent memory guarantee',api_source='https://learn.microsoft.com/en-us/windows/win32/api/psapi/nf-psapi-emptyworkingset',permission_basis='User requested background memory cleanup; only identified idle Codex tool services have reloadable resident pages trimmed')
(A/'results/COLD_PATH_MEMORY_RECOVERY.json').write_text(json.dumps(out,indent=2),encoding='utf-8');print(json.dumps(dict(before_MiB=before,after_MiB=after,trimmed=sum(r['success'] for r in rows),terminated=0)))
