...
for n,side in enumerate(['LEFT','RIGHT']):
    require('ROOT_BUSH_'+side,bridge);require('ROOT_KEEPER_'+side,bridge);require('ROOT_KEEPER_'+side,f'ROOT_SCREW_{n}');allow(f'ROOT_SCREW_{n}',bridge)
...
src=c['bridge_source'];oldp=read(c['parent_source_plan']);old=next(r for r in oldp['states'][a.state]['rows'] if r['id']==bridge);assert old==src and sha(src['step_path'])==src['source_sha256'];hashes[src['step_path']]=src['source_sha256'];previous=load(src)
...
for side in ['LEFT','RIGHT']:
    k='ROOT_BUSH_'+side;keeper='ROOT_KEEPER_'+side
...
out['failures']=[r for group in ['contacts','capture_stops','tool_allocation_tests'] for r in out[group] if not r['passed']]
