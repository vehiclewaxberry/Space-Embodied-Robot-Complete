...
def make(name):
 ...
 if name=='BODY':s=cx(nomR,-7-c['nominal_D_L_mm'][1],-7);color=(.16,.19,.23)
  ...
  from c203_pcb_surface_profile import make as make_surface_profile
  return make_surface_profile()
 elif name=='CARRIER':
  s=None
  for x in centers:
   ring=(cx(rout,x-3,x+3)-cx(rin,x-4,x+4)) & block(x-4,x+4,-30,30,-50,-20)
   ring=ring+block(x-3,x+3,-26.5,-17.5,-50,-15.5)+block(x-3,x+3,17.5,26.5,-50,-15.5)+block(x-5,x+5,*c['flange_y_mm'],-15.5,-11.5)
   for hx,hy in c['deck_holes_xy_mm']:
    if hx==x:ring=ring-cz(1.7,-16,-11,hx,hy)
   for y in [-22.5,22.5]:ring=ring-cz(1.5,-50.1,-41.5,x,y)
  ...
  s=s+frame
  # Drill after fusing rails: otherwise the rail fills the upper board bores.
  for y,z in c['board_holes_yz_mm']:s=s-cx(1.5,-13.9,-6.9,y,z)
  color=(.70,.59,.38)
