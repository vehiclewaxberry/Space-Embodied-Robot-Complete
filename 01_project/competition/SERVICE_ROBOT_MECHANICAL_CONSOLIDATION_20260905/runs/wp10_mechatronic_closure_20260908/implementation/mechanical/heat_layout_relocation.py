...
from bottom_radiator_common import assembly as bottom_assembly
...
def upper_deck():
    ...
    for y in [36,69]:s=s-Pos(106,y,-10)*Cylinder(1.7,20)
    s.label='upper_equipment_deck_B__DUAL_NEW_BORES_OLD_BORES_RETAINED';return s

def dual_parts():
    out={}
    for key in DUAL_IDS:
        s=Pos(6,-10,0)*import_step(str(V6/(key+'.step')));s.label=key;out[key]=s
    return out
...
def route(name):
    if name=='PROP_PWR':
        ...
        r=Route((78,-15,45));r.line((78,25,45));r.arc((99,25,45),(0,0,-1),math.pi/2)
...
def assembly():
    children=[heat_assembly(),bottom_assembly(),upper_deck(),*dual_parts().values(),route('PROP_PWR')[0],route('PROP_DATA')[0]]
