...
def run():
    dest=HERE/'SPREADER_GEOMETRY_V33.json';assert not dest.exists()
    p=read(HERE/'SPREADER_PARAMETERS_V33.json')
    assert all(sha(A/rel)==h for rel,h in p['source_lock'].items())
    baseline=step(A/'mechanical/bottom_radiator.step');new=step(HERE/'spreader_v33.step')
    ...
    # Independent full-width prism minus two beam slots, CHB seat, two feet,
    # two radius4 support bosses and four radius3.5 MIPS pockets.
    x0,y0,x1,y1=p['region_xy_mm']
    expected=(((x1-x0)*(y1-y0))-(2*14.5*(y1-y0))-(58*62)-(2*58*14)-(2*math.pi*4**2)-(4*math.pi*3.5**2))*p['height_mm']
    ...
    current=read(HERE/'SPREADER_INSTANCE_PLAN_V28.json')
    lineage={}
    for state,rows in parent['states'].items():
        actual={r['id']:r for r in current['states'][state]['rows']}
        assert len(actual)==974 and {r['id'] for r in rows}==set(actual)
        for r in rows:
            if r['id']==p['parent_instance_id']:continue
            for key in ['step_path','source_sha256','T_S_step','representation_role','is_ground_only']:
                assert r[key]==actual[r['id']][key],(state,r['id'],key)
        lineage[state]=973
    ...
    out=dict(schema='WP10_V33_SPREADER_NATIVE_DELTA',script_sha256=sha(__file__),
        generator_sha256=sha(HERE/'spreader_v33.step.py'),parameter_sha256=sha(HERE/'SPREADER_PARAMETERS_V33.json'),
        baseline_step_sha256=sha(A/'mechanical/bottom_radiator.step'),new_step_sha256=sha(HERE/'spreader_v33.step'),
        ...
        increment_vs_V28_volume_mm3=volume(increment),increment_vs_V28_mass_kg=volume(increment)*2700e-9,
        current_plan_sha256=current_plan_sha,unchanged_host_rows_by_state=lineage,added_mass_kg=addv*2700e-9,new_mass_kg=volume(new)*2700e-9,added_bbox_S_mm=bb,
