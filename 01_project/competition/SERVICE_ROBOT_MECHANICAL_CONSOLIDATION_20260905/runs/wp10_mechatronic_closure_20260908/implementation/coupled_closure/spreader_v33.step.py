...
def gen_step():
    d=json.loads((HERE/'SPREADER_PARAMETERS_V33.json').read_text())
    for path,h in d['source_lock'].items():
        assert hashlib.sha256((A/path).read_bytes()).hexdigest()==h,'SOURCE_DRIFT: '+path
    spec=importlib.util.spec_from_file_location('wp10_bottom_source_v33',A/'mechanical/bottom_radiator_common.py')
    ...
    s.label='WP10_V33_BOTTOM_RADIATOR_INTEGRAL_WIDE_SPREADER_3p5__THERMAL_CLOSURE_OPEN'
