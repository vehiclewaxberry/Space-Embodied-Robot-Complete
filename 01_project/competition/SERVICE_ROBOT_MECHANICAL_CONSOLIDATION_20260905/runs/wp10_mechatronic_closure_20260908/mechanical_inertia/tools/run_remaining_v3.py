"""Serial bounded source batches using the declared V3 practical integration method."""
from pathlib import Path
import subprocess,sys,json,psutil
M=Path(__file__).resolve().parents[1];results=[]
for start in range(19,237,12):
    end=min(start+12,237);name=f'native_delta_inertia_v3batch{start:03d}'
    assert not any((M/'results'/f'G{i:03d}.json').exists() for i in range(start,end))
    cmd=[sys.executable,'-B','-X','utf8',str(M/'tools/native_delta_guard.py'),'--timeout','60',name,'--',sys.executable,'-B','-X','utf8',str(M/'tools/read_material_inertia_v3.py'),'--start',str(start),'--end',str(end)]
    ...
    (M/'results/SERIAL_BATCH_PROGRESS_V3.json').write_text(json.dumps(results,indent=2),encoding='utf-8')
