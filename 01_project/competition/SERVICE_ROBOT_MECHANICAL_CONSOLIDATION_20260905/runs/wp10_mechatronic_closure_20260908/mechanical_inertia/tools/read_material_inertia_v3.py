...
def props(shape,eps=None,reference=None):
    ...
    if eps is None:BRepGProp.VolumeProperties_s(shape,p)
        ...
        err=BRepGProp.VolumeProperties_s(shape,p,eps,True,False)
        assert err>=0,'Adaptive volume quadrature failed'
    ...
    r={'volume_mm3':p.Mass(),'COM_local_mm':[c.X(),c.Y(),c.Z()],
       ...
       'method':'BRepGProp.VolumeProperties(default,no triangulation)' if eps is None else 'BRepGProp.VolumeProperties(adaptive face quadrature,OnlyClosed=True,SkipShared=False; eps estimates volume, NOT inertia error)'}
...
for job in jobs['rows'][args.start:args.end]:
    ...
    start=time.monotonic();r={'worker_sha256':sha(__file__),'method_revision':'V3_DEFAULT_FACE_WITH_ADAPTIVE_COMPARISON','index':job['index'],'source':job['source'],'instances':job['instances'],'status':'RUNNING'}
    try:
        ...
        r.update(default=d0,adaptive_eps1e7=d7,adaptive_eps1e9=d9,primary_property_key='default',primary_method='OCCT_DEFAULT_FACE_GAUSS_QUADRATURE',comparison_method='VOLUME_CONTROLLED_ADAPTIVE_FACE_QUADRATURE_NOT_A_STRICT_INERTIA_BOUND')
        r['convergence']={'volume_relative_change':abs(d9['volume_mm3']-d7['volume_mm3'])/d9['volume_mm3'],
            ...
            'strict_volume_COM_inertia_error_bound':None,'status':'OBSERVED_VOLUME_TOLERANCE_REFINEMENT_CHANGE_NOT_INERTIA_ERROR_BOUND'}
        ...
        representative=bodies[job['instances'][0]];world=[];dpose=d0
        for state,g in representative['geometry_by_state'].items():
            ...
            transformed=shape.Moved(TopLoc_Location(tr));center=rot@np.asarray(dpose['COM_local_mm'])+tmm
            pw,dw=props(transformed,None,center.tolist())
            target_I=rot@np.asarray(dpose['I_COM_local_mm5'])@rot.T
            ...
            verr=abs(dw['volume_mm3']-dpose['volume_mm3'])/dpose['volume_mm3']
            axis_checks=[]
            I_origin=target_I+dpose['volume_mm3']*(np.dot(center,center)*np.eye(3)-np.outer(center,center))
            for i in range(3):
            ...
            world.append({'state':state,'representative_instance':representative['id'],'volume_relative_error':verr,'COM_max_abs_error_mm':cerr,'I_COM_relative_max_error':irel,'axis_origin_MomentOfInertia_relative_errors':axis_checks,'actual_BRep_rigid_placement_reintegrated':True,'pose_check_integration_method':'default face quadrature against default local integrals; primary default integrals checked by algebra in collector'})
            transformed=pw=None
