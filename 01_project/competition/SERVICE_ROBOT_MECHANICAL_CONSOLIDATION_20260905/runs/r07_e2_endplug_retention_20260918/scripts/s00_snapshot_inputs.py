# -*- coding: utf-8 -*-
"""R07-E2 run: 输入快照与 sha256 清单（LF 二进制写）。
...
def main():
    ...
    # 备份烟测将覆盖的现存实例回执（E1 后现行版，431 实例）
    cur = ENG / 'results/service_structure_instances.json'
    if cur.exists():
        shutil.copy2(cur, RUN / 'logs' / 'service_structure_instances.json.pre_e2_smoke_bak')
    manifest = {
        'run': 'r07_e2_endplug_retention_20260918',
        'created_by': 'WP03_R07_E2_builder (single CAD builder)',
        'role': 'pre_change_input_snapshot',
        'note': '改件前输入快照（E1 已闭环后状态，整装基线 431 实例）；sha256 对原文件字节计算。WP02 文件只读不改，快照仅为来源绑定。',
        'files': man,
    }
