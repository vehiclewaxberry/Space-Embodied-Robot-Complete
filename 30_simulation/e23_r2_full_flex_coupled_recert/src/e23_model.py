...
E22_DIR = HERE.parents[1]  # e23 package dir (name kept for minimal diff)
...
def rom5_falsifier_summary(ctx: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "P2_max_modes_per_wing_historical": 5,
        "P2_max_modes_per_wing": 7,
        "disposition": (
...
def rom7_family_confirmation(ctx: dict[str, Any]) -> bool:
    """Confirm the Round4 seven-mode ROM family is exactly the independent
    falsifier's passing 7D subset {HF 0-based 0,1,2,3,4,5,7} with error <=1%."""
    doc = ctx["rom5_falsifier"]
    best7 = doc["dimension_combination_falsifier"]["best_seven_dimensional_subset_preserving_three_bending_modes"]
    falsifier_set = set(int(x) for x in best7["hf_indices_0based"])
    contract = ctx["rom_contract"]
    rom_set = set(int(x)-1 for x in contract["hf_mode_indices_1based"])
    expected_labels = ["BENDING_1", "BENDING_2", "BENDING_3",
                       "TORSION_1", "TORSION_2", "TORSION_3", "TORSION_4"]
    return bool(
        falsifier_set == {0, 1, 2, 3, 4, 5, 7} and
        rom_set == falsifier_set and
        list(contract["mode_labels"]) == expected_labels and
        float(best7["global_max_relative_error"]) <= 0.01 and
        int(doc["dimension_combination_falsifier"]["minimum_passing_dimension_preserving_all_three_bending_modes"]) == 7
    )
...
def load_context(config_path: Path | None = None) -> dict[str, Any]:
    config_path = config_path or E22_DIR / "config" / "e23_config.yaml"
    ...
    rom_npz = np.load(PROJECT_ROOT / pins["round4_rom_npz"]["path"])
    rom_contract = _read_json(PROJECT_ROOT / pins["round4_rom_contract"]["path"])
    envelope = _read_json(PROJECT_ROOT / pins["round4_validity_envelope"]["path"])
    ...
    round3_gate = _read_json(PROJECT_ROOT / pins["round4_gate"]["path"])  # Round4 V3 gate (key name kept)
...
def build_authority_audits(ctx: dict[str, Any]) -> dict[str, Any]:
    ...
    shape_ok = (z["M"].shape == (183, 183) and z["Phi_rom"].shape == (183, 7) and
                z["Gamma_t_L"].shape == (7, 3) and z["Gamma_r_L"].shape == (7, 3) and
    ...
    criteria = [
        ...
        {"id": "PF13_ROM7_FAMILY_CONFIRMATION", "pass": bool(rom7_family_confirmation(ctx)), "detail": "Round4 seven-mode family equals falsifier passing 7D subset with error<=1%"},
    ]
...
def assemble_chaser_model(
    ...
    if not 1 <= n <= 7:
        raise ValueError("modes_per_wing must be 0..7")
    # Pre-registered nested order by nominal frequency, mapped from stored
    # [B1,B2,B3,T1,T2,T3,T4] to [B1,T1,T2,B2,T3,T4,B3].
    nested_all = np.array([0, 3, 4, 1, 5, 6, 2], dtype=int)
    idx = nested_all[:n]
    labels_all = np.array(["BENDING_1", "TORSION_1", "TORSION_2", "BENDING_2",
...
def _linearity_metrics(ctx: dict[str, Any], eta_contact: np.ndarray,
    ...
    for iw, wing in enumerate(("LEFT", "RIGHT")):
        eta7 = np.zeros((7, histories.shape[1]))
        start = iw*modes_per_wing
        nested_all = np.array([0, 3, 4, 1, 5, 6, 2], dtype=int)
        eta7[nested_all[:modes_per_wing]] = histories[start:start+modes_per_wing]
        checks = {}
        for name, (Rop, limit_key) in names.items():
            peak = float(np.max(np.abs(Rop @ eta7))) if histories.shape[1] else 0.0
...
def _run_post_ode(
    ...
    return {
        ...
        "omega_plus_equiv_semantics": "E23_DIAGNOSTIC_PROXY__Icg_inverse_times_L_about_combined_CG",
...
def run_hf_to_rom_dynamic_validation(
    ...
    z = ctx["rom"]
    Rfull = full_reconstruction_operators(ctx)
    Phi5 = z["Phi_rom"]  # (183, 7) seven-mode Round4 basis
    op_backcheck = {
    ...
    for wing in ("L", "R"):
        ...
        trunc = {}
        nested_all = np.array([0, 3, 4, 1, 5, 6, 2], dtype=int)
        nested_labels = ["BENDING_1", "TORSION_1", "TORSION_2", "BENDING_2",
                         "TORSION_3", "TORSION_4", "BENDING_3"]
        for nm in (5, 6, 7):
            idx = nested_all[:nm]
            Phi = Phi5[:, idx]
        ...
        all_seven_errors.append(trunc["7"]["max_relative_error"])

        idx7 = nested_all
        Phi_ordered = Phi5[:, idx7]
        Mr5 = z["Mrom"][np.ix_(idx7, idx7)]
        Kr5 = z["Krom_NOMINAL"][np.ix_(idx7, idx7)]
        Cr5 = np.zeros_like(z["Crom_NOMINAL"][np.ix_(idx7, idx7)])
    ...
    seven_max = float(max(all_seven_errors))
    cross_max = float(max(cross_errors))
    subset_pass = seven_max <= limit and cross_max <= float(cfg["diagnostic_thresholds"]["solver_cross_relative_max"])
    ...
    if not rom7_family_confirmation(ctx):
        raise RuntimeError("ROM7_FAMILY_FALSIFIER_SUBSET_MISMATCH")
    return jsonable({
        ...
        "wings": out_wings,
        "seven_mode_max_relative_error_vs_183dof": seven_max,
