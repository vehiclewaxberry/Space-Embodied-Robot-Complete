"""Build the complete E23 R2 coupled recertification evidence package (seven-mode ROM), serially."""
...
from e23_model import (E22_DIR, PROJECT_ROOT, jsonable, load_context,
                       rom5_falsifier_contract_pass, rom5_falsifier_summary,
                       rom7_family_confirmation, run_hf_to_rom_dynamic_validation,
...
def build() -> dict[str, Any]:
    ...
    for sid in scenarios:
        for corner in corners:
            flex_main.append(simulate_case(
                ctx, sid, "SOLAR_R2_FLEX_CURRENT", corner=corner, modes_per_wing=7,
    ...
    for sid in scenarios:
        for Tc in cfg["physics"]["contact_window_ms_sweep"]:
            contact_sweep.append(simulate_case(
                ctx, sid, "SOLAR_R2_FLEX_CURRENT", corner="NOMINAL", modes_per_wing=7,
    ...
    for sid in scenarios:
        for corner in corners:
            rad = simulate_case(ctx, sid, "SOLAR_R2_FLEX_CURRENT", corner=corner,
                                modes_per_wing=7, post_window_s=0.5, method="Radau",
            ...
            bdf = simulate_case(ctx, sid, "SOLAR_R2_FLEX_CURRENT", corner=corner,
                                modes_per_wing=7, post_window_s=0.5, method="BDF",
                                post_propagator="IMPLICIT_ODE", damping_lane="UNDAMPED_CONSERVATION_GATE")
    ...
    # Coupled m=5/6/7 trend uses an identical five-second horizon and nested
    # [B1,T1,T2,B2,T3,T4,B3] ordering.
    truncation_summary = {}
    trunc_passes = []
    for sid in scenarios:
        cs = {int(x["modes_per_wing"]): x for x in truncation_cases if x["scenario"] == sid}
        c56, c67 = compare_cases(cs[5], cs[6]), compare_cases(cs[6], cs[7])
        pass67 = c67["max_relative_difference"] <= float(cfg["diagnostic_thresholds"]["rom_truncation_relative_max"])
        trunc_passes.append(pass67)
        truncation_summary[sid] = {"nested_order": ["BENDING_1", "TORSION_1", "TORSION_2", "BENDING_2", "TORSION_3", "TORSION_4", "BENDING_3"],
                                    "m5_to_m6": c56, "m6_to_m7": c67,
                                    "m6_to_m7_under_1pct": pass67}
    ...
    original = (PROJECT_ROOT / ctx["pins"]["round4_gate"]["path"]).read_bytes()
    hash_mutation_rejected = __import__("hashlib").sha256(original+b"E22_NEGATIVE_CONTROL").hexdigest().upper() != ctx["pins"]["round4_gate"]["sha256"]
    try:
    ...
    rom7_binding_pass = rom7_family_confirmation(ctx)
    mutated_contract = copy.deepcopy(ctx["rom_contract"])
    mutated_contract["mode_labels"][3] = "TORSION_9"
    rom7_family_mutation_rejected = not rom7_family_confirmation({**ctx, "rom_contract": mutated_contract})
    ...
    negative_controls = {
        ...
        "rom7_family_binding_pass": rom7_binding_pass,
        "rom7_family_mutation_rejected": rom7_family_mutation_rejected,
        ...
        "pass": all((nan_rejected, null_rejected, hash_mutation_rejected, source_lane_rejected,
                    ...
                    falsifier_field_mutation_rejected, rom7_binding_pass, rom7_family_mutation_rejected,
                    guard_mutation_rejected,
    ...
    anchors_covered = all(
        any(x["scenario"] == sid for x in rigid) and
        any(x["scenario"] == sid and x["corner"] == "NOMINAL" for x in flex_main)
        for sid in scenarios)
    current_scope_comparisons = {
        sid: compare_cases(
            next(x for x in rigid if x["scenario"] == sid),
            next(x for x in flex_main if x["scenario"] == sid and x["corner"] == "NOMINAL"))
        for sid in scenarios}
    legacy_reference_guarded = len(legacy) == 2 and all(
        x["lane"] == "LEGACY_R1_FLEX_HISTORICAL_REFERENCE_ONLY" and
        x["numeric_comparison_authorized"] is False for x in legacy)
    three_lane_copresentation_complete = bool(anchors_covered and legacy_reference_guarded)
    cases_doc = {
        "schema": "E23_R2_COUPLED_CASES_V1",
        ...
        "three_lane_two_scenario_comparison": {
            "state": "COMPLETE_CURRENT_SCOPE__LEGACY_HISTORICAL_REFERENCE_GUARDED" if three_lane_copresentation_complete else "PARTIAL_FAIL_CLOSED",
            "complete": three_lane_copresentation_complete,
            "current_scope_numeric_comparisons_rigid_vs_flex7": current_scope_comparisons,
            "legacy_numeric_comparison": "NOT_AUTHORIZED_INCOMPATIBLE_SEMANTICS",
            "reason": (
                ...
                "historical reproduction only; legacy metrics are co-presented at both "
                "anchors with an explicit non-causal guard, and the binding comparison is "
                "Rigid vs Solar-R2-flex-seven-mode at identical current scope"
            ),
        },
    }
    sensitivity_doc = {
        "schema": "E23_R2_SENSITIVITY_AND_VALIDATION_V1",
    ...
    authority_doc = {
        "schema": "E23_SOURCE_AND_AUTHORITY_AUDIT_V1",
    ...
    write_json(RESULTS/"E23_SOURCE_AND_AUTHORITY_AUDIT_V1.json", authority_doc)
    write_json(RESULTS/"E23_R2_COUPLED_CASES_V1.json", cases_doc)
    write_json(RESULTS/"E23_R2_SENSITIVITY_AND_VALIDATION_V1.json", sensitivity_doc)
    ...
    with (RESULTS/"E23_CASE_MATRIX_V1.csv").open("w", newline="", encoding="utf-8") as f:
    ...
    criteria = [
        ...
        {"id": "G10", "name": "nested coupled m6->m7 truncation <=1%", "pass": all(trunc_passes)},
        {"id": "G11", "name": "183-DOF HF to seven-mode dynamic truncation <=1%", "pass": bool(hf_validation["pass"])},
        ...
        {"id": "G17", "name": "three-lane/two-scenario co-presentation: current-scope numeric comparison complete, legacy reference guarded non-causal, no empty comparison set", "pass": three_lane_copresentation_complete and all(v["relative_differences"] for v in current_scope_comparisons.values())},
    ...
    evidence_paths = [
        E22_DIR/"config/e23_config.yaml", E22_DIR/"src/e23_model.py", E22_DIR/"src/build_e23.py",
        E22_DIR/"README.md", E22_DIR/"tests/test_e23.py", E22_DIR/"tests/independent_recompute.py",
        ...
        RESULTS/"R2_HF_TO_ROM_DYNAMIC_VALIDATION_V1.json", RESULTS/"E23_CASE_MATRIX_V1.csv",
    ]
    evidence_hashes = {str(p.relative_to(PROJECT_ROOT)).replace("\\", "/"): sha256(p) for p in evidence_paths}
    gate = {
        "schema": "E23_R2_FULL_FLEX_COUPLED_GATE_V1",
        "technical_verdict": verdict,
        "scope": "CHECKPOINT-A recertification of current R2 rigid/flexible coupled diagnostics with the Round4 seven-mode ROM; no mission release",
        ...
        "key_metrics": {
            ...
            "hf_to_rom_seven_mode_max_relative": hf_validation["seven_mode_max_relative_error_vs_183dof"],
            ...
            "executed_rom_dimension_preserving_three_bending_modes": 7,
            "linearity_all_current_R2": linearity_ok,
            "three_lane_comparison_state": "COMPLETE_CURRENT_SCOPE__LEGACY_HISTORICAL_REFERENCE_GUARDED" if three_lane_copresentation_complete else "PARTIAL_FAIL_CLOSED",
        ...
        "risk_and_followup": {
            "root_cause_E22": "CONTRACT_BOUNDED_ROM_DIMENSION_INSUFFICIENT_FOR_1PCT_DYNAMIC_RECONSTRUCTION",
            "resolution_executed": "Owner TMC-01 authorized ROM reselection to 7D B1-B3+T1-T4; Round4 V3 gate 17/17 PASS_WITH_DECLARED_PROVISIONAL_PHYSICS; this E23 package is the coupled recertification",
            "independent_falsifier": hf_validation["independent_red_team_falsifier_finding"],
            "required_resolution_E22_historical": (
                ...
                "family; EXECUTED here as the 7D family per Owner TMC-01"
            ),
    ...
    write_json(RESULTS/"E23_R2_FULL_FLEX_COUPLED_GATE_V1.json", gate)
    return gate
