...
INDEPENDENT_RESULT = RESULTS / "E22_INDEPENDENT_RECOMPUTE_V1.json"
REPLAY_RESULT = RESULTS / "E22_DETERMINISM_REPLAY_V1.json"
...
def main() -> int:
    ...
    for index in range(1, 6):
        ...
        runs.append({
            ...
            "five_mode_max_relative_independent": dynamic["five_mode_max_relative_independent"],
            "blas_threads": [x["num_threads"] for x in
                             dynamic["determinism_contract"]["threadpools"]],
        })
    hashes = {x["report_sha256"] for x in runs}
    errors = [float(x["five_mode_max_relative_independent"]) for x in runs]
    replay_pass = (
    ...
    output = {
        "schema": "E22_DETERMINISM_REPLAY_V1",
        "contract": (
