"""Independent raw-file recomputation for E22 evidence.

This script intentionally does not import e22_model or build_e22.  It checks the
...
def write_package_manifest() -> dict[str, Any]:
    ...
    gate_path = RESULTS / "E22_R2_FULL_FLEX_COUPLED_GATE_V1.json"
    independent_path = RESULTS / "E22_INDEPENDENT_RECOMPUTE_V1.json"
    required = [
        gate_path,
        independent_path,
        ...
        RESULTS / "E22_CASE_MATRIX_V1.csv",
    ]
    replay_path = RESULTS / "E22_DETERMINISM_REPLAY_V1.json"
    ...
    manifest = {
        "schema": "E22_PACKAGE_MANIFEST_V1",
        ...
        "checkpoint_A_required_pins": {
            ...
            "package_manifest": {
                "path": str((RESULTS / "E22_PACKAGE_MANIFEST_V1.json").relative_to(PROJECT_ROOT)).replace("\\", "/"),
    ...
    write_json(RESULTS / "E22_PACKAGE_MANIFEST_V1.json", manifest)
    return manifest


def main() -> int:
    cfg = yaml.safe_load((E22_DIR / "config" / "e22_config.yaml").read_text(encoding="utf-8"))
    ...
    z = np.load(PROJECT_ROOT / cfg["source_pins"]["round3_rom_npz"]["path"])
    ...
    for wing in hf["wings"].values():
        ...
        for key, value in metrics.items():
            ...
            error_arithmetic_residual = max(
                ...
                abs(absolute-float(five["absolute_error_vs_183dof"][key])),
                abs(relative-float(five["relative_error_vs_183dof"][key])),
            )
    ...
    rom_contract = json.loads((PROJECT_ROOT / cfg["source_pins"]["round3_rom_contract"]["path"]).read_text(encoding="utf-8"))
    operators_full = full_operators(rom_contract["reduced_hf_dof_semantics"])
    stored_order = np.asarray([0, 3, 4, 1, 2], int)
    ...
    for wing in ("L", "R"):
        ...
        writer_rom = hf["wings"][wing]["truncation"]["5"]["metrics"]
        for name in full_ind:
        ...
        dynamic_by_wing[wing] = {
            ...
            "five_mode_metrics_independent": rom_ind,
            "relative_error_independent": relative,
            "torsion_peak_full_rad": full_ind["torsion_peak_rad"],
            "torsion_peak_five_mode_rad": rom_ind["torsion_peak_rad"],
    ...
    checks.append({
        ...
        "pass": (deterministic_pools and independent_max > 0.01 and
                 ...
                 abs(recomputed_hf_max-float(hf["five_mode_max_relative_error_vs_183dof"])) <= 1.0e-14 and
                 abs(independent_max-float(hf["five_mode_max_relative_error_vs_183dof"])) <= 1.0e-10 and
                 writer_match_relative <= 1.0e-9 and
                 hf["linear_subset_verdict"] == "E15_R2_LINEAR_SUBSET_FAIL" and
                 gate["technical_verdict"] == "HOLD_R2_HF_TO_ROM_DYNAMIC_VALIDATION_FAILED" and
        ...
        "five_mode_max_relative_from_writer_arithmetic": recomputed_hf_max,
        "five_mode_max_relative_independent": independent_max,
    ...
    output = {
        "schema": "E22_INDEPENDENT_RECOMPUTE_V1",
        "independence": "raw YAML/JSON/NPZ recomputation; no import from e22_model/build_e22",
        "determinism_contract": determinism_contract,
        "checks": checks,
    ...
    write_json(RESULTS / "E22_INDEPENDENT_RECOMPUTE_V1.json", output)
