"""Executable tests for the E22 R2 coupled diagnostic package."""
...
from e22_model import (assemble_chaser_model, load_context,
                       rom5_falsifier_contract_pass, rom5_falsifier_summary,
                       sha256, simulate_case, target_contract_pass)
from build_e22 import disposition_guards_pass
...
def test_fail_closed_authority_preflight_and_mass_matrix() -> None:
    ...
    assert (preflight["passed"], preflight["total"]) == (12, 12)
    assert ctx["pins"]["round3_gate"]["sha256"] == \
        "F8E5696FF3D9EFAD2737E056CF29C9B9D0940DAB4188AFB095DAE53C01785537"
    mass = ctx["authority_audits"]["coupled_mass_matrix"]
    assert abs(mass["min_eigenvalue"] - 0.21274617164987195) < 1.0e-12
    assert abs(mass["condition_number"] - 147.2632668196728) < 1.0e-9
    assert abs(mass["schur_min_eigenvalue"] - 0.5560347387744818) < 1.0e-12
...
def test_nested_mode_order_and_undamped_gate_lane() -> None:
    ctx = load_context()
    expected_indices = {3: [0, 3, 4], 4: [0, 3, 4, 1], 5: [0, 3, 4, 1, 2]}
    expected_labels = ["BENDING_1", "TORSION_1", "TORSION_2", "BENDING_2", "BENDING_3"]
    for n in (3, 4, 5):
    ...
    damped = assemble_chaser_model(ctx, "C07", "NOMINAL", 5,
...
def test_short_current_r2_case_is_finite_and_classified_from_equivalent_rate() -> None:
    ...
    case = simulate_case(
        ...
        corner="NOMINAL", modes_per_wing=5, post_window_s=0.03,
    ...
    assert case["retained_stored_indices"] == [0, 3, 4, 1, 2]
    ...
    assert case["post_capture"]["omega_plus_equiv_semantics"].startswith(
        "E22_DIAGNOSTIC_PROXY")


def test_final_gate_and_evidence_hashes() -> None:
    gate_path = E22_DIR / "results" / "E22_R2_FULL_FLEX_COUPLED_GATE_V1.json"
    assert gate_path.exists(), "run src/build_e22.py before the final test suite"
    gate = json.loads(gate_path.read_text(encoding="utf-8"))
    assert gate["technical_verdict"] == "HOLD_R2_HF_TO_ROM_DYNAMIC_VALIDATION_FAILED"
    assert gate["authority_dispositions"]["linear_subset"] == "E15_R2_LINEAR_SUBSET_FAIL"
    ...
    assert criteria["G11"] is False
    assert gate["key_metrics"]["hf_to_rom_five_mode_max_relative"] > 0.01
...
def test_round3_hash_negative_control() -> None:
    ctx = load_context()
    path = PROJECT_ROOT / ctx["pins"]["round3_gate"]["path"]
    mutated = hashlib.sha256(path.read_bytes()+b"E22_TEST_MUTATION").hexdigest().upper()
    assert mutated != ctx["pins"]["round3_gate"]["sha256"]
