...
def run_preexecution_tests(run_root: Path) -> dict:
    ...
    base_temp = run_root / "verification" / ".pytest-tmp-r5-preexecution"
    command = [
        sys.executable,
        "-m",
        "pytest",
    ...
    if base_temp.exists():
        try:
            base_temp.resolve().relative_to(run_root.resolve())
    ...
    return {
        ...
        "basetemp": base_temp.relative_to(run_root).as_posix(),
