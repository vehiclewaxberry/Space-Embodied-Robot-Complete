...
def run_s02_case_r5(plant, case: dict, scenario: dict, gates: dict) -> dict:
    ...
    gate_map = {
        "G_P_BALANCE": nominal["metrics"]["epsilon_P_max"] < float(mom["epsilon_P_max"]),
        "G_H_BALANCE": nominal["metrics"]["epsilon_H_max"] < float(mom["epsilon_H_max"]),
    ...
    return {
        ...
        "gates": gate_map,
        "hard_gate_failures": [name for name, value in gate_map.items() if not value],
        "all_gates_pass": bool(all(gate_map.values())),
    }
