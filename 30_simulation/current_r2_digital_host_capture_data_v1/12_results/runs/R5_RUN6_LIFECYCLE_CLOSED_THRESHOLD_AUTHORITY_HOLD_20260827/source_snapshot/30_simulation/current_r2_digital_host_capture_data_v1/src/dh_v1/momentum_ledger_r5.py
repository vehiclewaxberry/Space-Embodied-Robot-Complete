...
def validate_unit_scale_contract(
    *,
    length_to_m: float,
    mass_to_kg: float,
    time_to_s: float,
    ...
    """Validate an explicit interface-unit conversion without repairing it.

    Inertia must scale as mass times length squared.  A caller-provided but
    dimensionally inconsistent inertia factor is a contract failure, never an
    opportunity for an automatic correction.
    """

    values = {
        "length_to_m": float(length_to_m),
        "mass_to_kg": float(mass_to_kg),
        "time_to_s": float(time_to_s),
        "angle_to_rad": float(angle_to_rad),
        "inertia_to_kg_m2": float(inertia_to_kg_m2),
    }
    if not all(np.isfinite(value) and value > 0.0 for value in values.values()):
        raise LedgerContractError("FAIL_UNIT_INCONSISTENCY: all unit scale factors must be positive")
    expected_inertia = values["mass_to_kg"] * values["length_to_m"] ** 2
    if not np.isclose(
        values["inertia_to_kg_m2"], expected_inertia, rtol=1.0e-12, atol=0.0
    ):
        raise LedgerContractError(
            "FAIL_UNIT_INCONSISTENCY: inertia scale must equal mass scale times length scale squared"
        )
    return {**values, "expected_inertia_to_kg_m2": expected_inertia, "validated": True}


def rigid_body_momentum_about_origin(
    mass_kg: float,
    com_position_I_m,
    com_velocity_I_m_s,
    inertia_com_I_kg_m2,
    ...
    """Analytic one-body P/H contribution in SI about inertial origin O."""

    mass = float(mass_kg)
    r = np.asarray(com_position_I_m, dtype=float)
    velocity = np.asarray(com_velocity_I_m_s, dtype=float)
    inertia = np.asarray(inertia_com_I_kg_m2, dtype=float)
    omega = np.asarray(omega_I_rad_s, dtype=float)
    if not np.isfinite(mass) or mass <= 0.0:
        raise LedgerContractError("rigid-body mass must be finite and positive")
    if r.shape != (3,) or velocity.shape != (3,) or omega.shape != (3,):
        raise LedgerContractError("rigid-body position, velocity and angular rate must be 3-vectors")
    if inertia.shape != (3, 3):
        raise LedgerContractError("rigid-body inertia must be 3x3")
    if not all(np.all(np.isfinite(value)) for value in (r, velocity, inertia, omega)):
        raise LedgerContractError("rigid-body momentum input contains non-finite values")
    if not np.allclose(inertia, inertia.T, rtol=0.0, atol=1.0e-12):
        raise LedgerContractError("rigid-body inertia must be symmetric")
    P = mass * velocity
    H_spin = inertia @ omega
    H_orbital = np.cross(r, P)
    return {
        "P_I_kg_m_s": P,
        "H_spin_I_N_m_s": H_spin,
        "H_orbital_O_I_N_m_s": H_orbital,
        "H_O_I_N_m_s": H_spin + H_orbital,
    }


def rigid_body_momentum_from_scaled_interface(
    *,
    mass_value: float,
    com_position_value,
    com_velocity_value,
    ...
    """Convert one explicitly scaled interface record to SI, then evaluate P/H."""

    scales = validate_unit_scale_contract(
        length_to_m=length_to_m,
        mass_to_kg=mass_to_kg,
        time_to_s=time_to_s,
        angle_to_rad=angle_to_rad,
        inertia_to_kg_m2=inertia_to_kg_m2,
    )
    return rigid_body_momentum_about_origin(
        float(mass_value) * scales["mass_to_kg"],
        np.asarray(com_position_value, dtype=float) * scales["length_to_m"],
        np.asarray(com_velocity_value, dtype=float)
        * scales["length_to_m"]
        / scales["time_to_s"],
        np.asarray(inertia_com_value, dtype=float) * scales["inertia_to_kg_m2"],
        np.asarray(omega_value, dtype=float)
        * scales["angle_to_rad"]
        / scales["time_to_s"],
    )


def shift_angular_momentum_reference(
    H_O_I_N_m_s, P_I_kg_m_s, r_O_Oprime_I_m
) -> np.ndarray:
    """Shift angular momentum from O to fixed inertial point O-prime."""

    H = np.asarray(H_O_I_N_m_s, dtype=float)
    P = np.asarray(P_I_kg_m_s, dtype=float)
    r = np.asarray(r_O_Oprime_I_m, dtype=float)
    if H.shape[-1] != 3 or P.shape[-1] != 3 or r.shape != (3,):
        raise LedgerContractError("reference shift requires (...,3) H/P and one 3-vector offset")
    if H.shape != P.shape:
        raise LedgerContractError("reference shift requires matching H and P arrays")
    return H - np.cross(np.broadcast_to(r, P.shape), P)
...
def evaluate_momentum_gate_r5(ledger: dict, momentum_config: dict) -> dict:
    """Evaluate separated P/H diagnostics and the independent authority Gate."""
    ...
    present = forbidden.intersection(momentum_config) | forbidden.intersection(
        ledger.get("metrics", {})
    )
    if present:
        raise LedgerContractError(
            f"mixed-dimension momentum Gate fields are forbidden: {sorted(present)}"
        )
    metrics = ledger.get("metrics", {})
    for field in ("epsilon_P_max", "epsilon_H_max"):
        if field not in metrics or not np.isfinite(float(metrics[field])):
            raise LedgerContractError(f"momentum ledger missing finite {field}")
    numeric = {
        "G_P_BALANCE_DIAGNOSTIC": float(metrics["epsilon_P_max"])
        < float(momentum_config["epsilon_P_max"]),
        "G_H_BALANCE_DIAGNOSTIC": float(metrics["epsilon_H_max"])
        < float(momentum_config["epsilon_H_max"]),
    }
    authority_bound = (
        momentum_config.get("threshold_authority_status")
        not in (None, "", "MISSING", "UNKNOWN")
        and bool(momentum_config.get("relative_metric_hard_gate", False))
    )
    return {
        "numeric_diagnostic_gates": numeric,
        "numeric_diagnostic_all_pass": bool(all(numeric.values())),
        "relative_metric_status": momentum_config.get(
            "relative_metric_status", "PROVISIONAL_DIAGNOSTIC_ONLY"
        ),
        "relative_metric_hard_gate": bool(
            momentum_config.get("relative_metric_hard_gate", False)
        ),
        "threshold_authority_status": momentum_config.get(
            "threshold_authority_status", "MISSING"
        ),
        "threshold_authority_gate": authority_bound,
        "formal_verdict": (
            "PASS_THRESHOLD_AUTHORITY_BOUND"
            if authority_bound
            else momentum_config.get(
                "authority_missing_verdict", "HOLD_THRESHOLD_AUTHORITY_MISSING"
            )
        ),
    }
