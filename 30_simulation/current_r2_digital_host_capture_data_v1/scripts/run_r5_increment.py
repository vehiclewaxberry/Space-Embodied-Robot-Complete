...
R5_SUCCESS_PACKAGE_FILES = R5_REQUIRED_PACKAGE_FILES + (
    "CONTROLLER_TIMESERIES.parquet",
    "EVENT_MONITOR_STATUS.json",
    "HASH_MANIFEST.csv",
)
...
def _lifecycle_stamp(utc_value: str) -> str:
    moment = datetime.fromisoformat(str(utc_value).replace("Z", "+00:00"))
    return moment.astimezone(timezone.utc).strftime("%Y%m%dT%H%M%S%fZ")


def _lifecycle_marker_payload(lifecycle: dict, failure_reason: str | None = None) -> dict:
    return {
        "schema": "R5_INCOMPLETE_RUN_MARKER_V2",
        "task_id": TASK_ID,
        "run_id": lifecycle["run_id"],
        "episode_id": lifecycle["episode_id"],
        "scenario_id": lifecycle["scenario_id"],
        "case_id": lifecycle["case_id"],
        "preopened_before_integration": True,
        "planned_config_sha256": lifecycle["planned_config_sha256"],
        "all_production_source_sha256": lifecycle["all_production_source_sha256"],
...
def preopen_episode_lifecycle(
    *,
    ...
    case_id: str,
    planned_config: dict,
    all_production_source_sha256: str,
) -> dict:
    """Create the same-volume episode workspace before numerical integration starts."""

    started_utc = utc_now()
    planned_sha = sha256_canonical_json(jsonable(planned_config))
    parent = (
        CANONICAL_R5_BASE / _safe_component(scenario_id) / _safe_component(case_id)
    ).resolve()
    ensure_descendant(parent, CANONICAL_R5_BASE)
    parent.mkdir(parents=True, exist_ok=True)
    failure_final = parent / (
        f"{_lifecycle_stamp(started_utc)}_{planned_sha[:8]}_"
        f"{all_production_source_sha256[:8]}"
    )
    temp = parent / (
        f".{failure_final.name}.tmp-{os.getpid()}-{_safe_component(episode_id)}"
    )
    assert_new_output_directory(failure_final)
    ...
    lifecycle = {
        ...
        "case_id": case_id,
        "planned_config_sha256": planned_sha,
        "all_production_source_sha256": all_production_source_sha256,
        "started_utc": started_utc,
        "temp_path": temp,
        "failure_final_path": failure_final,
        "state_history": [
            {"state": "CREATED", "utc": started_utc},
            {"state": "PREEXEC_PASS", "utc": utc_now()},
            {"state": "RUNNING", "utc": utc_now()},
        ],
    ...
    write_json(temp / "INCOMPLETE_RUN_MARKER.json", _lifecycle_marker_payload(lifecycle))
    write_json(
        temp / "FAILURE_CONTEXT.json",
        {
            "failed": None,
            "failure_reason": None,
            "terminal_status": "RUNNING",
    ...
    (temp / "STDOUT.log").write_text(
        f"{episode_id} lifecycle preopened before integration at {started_utc}\n",
        encoding="utf-8",
    )
    return lifecycle


def fail_episode_lifecycle(
    lifecycle: dict, exc: BaseException, terminal_status: str = "FAILED"
) -> Path | None:
    """Publish an incomplete episode package without granting success credit."""

    if lifecycle.get("terminal_status") in ("COMPLETE", "FAILED", "ABORTED"):
        return lifecycle.get("published_path")
    if terminal_status not in ("FAILED", "ABORTED"):
        raise R5RunError(f"invalid lifecycle terminal status: {terminal_status}")
    temp = Path(lifecycle["temp_path"])
    if not temp.is_dir():
        return None
    failed_utc = utc_now()
    lifecycle["state_history"].append({"state": terminal_status, "utc": failed_utc})
    lifecycle["terminal_status"] = terminal_status
    reason = str(exc)
    write_json(temp / "INCOMPLETE_RUN_MARKER.json", _lifecycle_marker_payload(lifecycle, reason))
    write_json(
        temp / "RUN_MANIFEST.json",
        {
            "schema": "R5_EPISODE_FAILURE_MANIFEST_V1",
            "task_id": TASK_ID,
            "run_id": lifecycle["run_id"],
            "episode_id": lifecycle["episode_id"],
            "scenario_id": lifecycle["scenario_id"],
            ...
            "planned_config_sha256": lifecycle["planned_config_sha256"],
            "all_production_source_sha256": lifecycle["all_production_source_sha256"],
            "start_utc": lifecycle["started_utc"],
            "end_utc": failed_utc,
    ...
    with (temp / "STDOUT.log").open("a", encoding="utf-8") as handle:
        handle.write(f"{terminal_status} at {failed_utc}: {reason}\n")
    failure_final = Path(lifecycle["failure_final_path"])
    assert_new_output_directory(failure_final)
    os.replace(temp, failure_final)
    lifecycle["atomic_publish"] = True
    lifecycle["published_path"] = failure_final
    return failure_final


def publish_episode_atomically(
    source_episode: Path, manifest: dict, lifecycle: dict | None = None
) -> Path:
    ...
    if lifecycle is None:
        ...
        temp = Path(lifecycle["temp_path"])
        if not lifecycle.get("preopened_before_integration") or not temp.is_dir():
            raise R5RunError("FAIL_LIFECYCLE_NOT_PREOPENED_BEFORE_INTEGRATION")
        if temp.parent.resolve() != final.parent.resolve():
            raise R5RunError("FAIL_LIFECYCLE_NOT_ON_CANONICAL_SAME_VOLUME_PARENT")
        if not (temp / "INCOMPLETE_RUN_MARKER.json").is_file():
            raise R5RunError("FAIL_LIFECYCLE_MARKER_MISSING_BEFORE_PUBLISH")
    try:
        shutil.copytree(source_episode, temp, dirs_exist_ok=lifecycle is not None)
        ...
        marker = temp / "INCOMPLETE_RUN_MARKER.json"
        if marker.exists():
            marker.unlink()
        expected = set(R5_SUCCESS_PACKAGE_FILES)
        actual = {path.name for path in temp.iterdir() if path.is_file()}
        if actual != expected:
            raise R5RunError(
                f"FAIL_RUN_PACKAGE_SCHEMA_MISMATCH: missing={sorted(expected-actual)} "
                f"extra={sorted(actual-expected)}"
            )
        os.replace(temp, final)
        if lifecycle is not None:
            lifecycle["state_history"].append({"state": "COMPLETE", "utc": utc_now()})
            lifecycle["terminal_status"] = "COMPLETE"
            lifecycle["atomic_publish"] = True
            lifecycle["published_path"] = final
...
def write_episode(
    ...
    if lifecycle is not None:
        manifest["runner_schema_version"] = "R5_ATOMIC_EPISODE_EXPORT_RUNNER_V3"
        manifest["lifecycle_contract"] = {
            "preopened_before_integration": True,
            "state_sequence": ["CREATED", "PREEXEC_PASS", "RUNNING", "COMPLETE"],
            "failure_package_retained_on_exception": True,
            "success_publish_method": "SAME_VOLUME_OS_REPLACE",
            "planned_config_sha256": lifecycle["planned_config_sha256"],
        }
    ...
    canonical = publish_episode_atomically(episode, manifest, lifecycle=lifecycle)
...
def r5_gate_verdict(exit_conditions: dict[str, bool]) -> str:
    """Compose the verdict from evaluated conditions so resolved holds cannot linger in prose."""

    if all(exit_conditions.values()):
        return "R5_FORMAL_GATE_PASS"
    threshold_token = (
        "THRESHOLD_AUTHORITY_BOUND"
        if exit_conditions.get("THRESHOLD_AUTHORITY_BOUND", False)
        else "THRESHOLD_AUTHORITY_HOLD"
    )
    lifecycle_token = (
        "VERSIONED_RUNNER_FAILURE_LIFECYCLE_PASS"
        if exit_conditions.get("FAILURE_LIFECYCLE_PREOPEN_AND_ATOMIC_PUBLISH", False)
        else "VERSIONED_RUNNER_FAILURE_LIFECYCLE_HOLD"
    )
    return (
        "R5_REPEAT_REQUIRED__UNIT_SAFE_P_H_LEDGER_DIAGNOSTIC_EVIDENCE_PRESERVED__"
        f"{threshold_token}__{lifecycle_token}__NO_CONTROL_OR_CONTACT_RELEASE"
    )
...
def execute(args: argparse.Namespace) -> Path:
    ...
    try:
        ...
        manifest_common = {
            ...
            "runner_schema_version": "R5_ATOMIC_EPISODE_EXPORT_RUNNER_V3",
        ...
        for case_index, case in enumerate(s02_cfg["cases"], 1):
            ...
            case_lifecycles = []
            for dt_index, planned_dt in enumerate(s02_cfg["dt_grid_s"], 1):
                episode_id = f"R5-S02-C{case_index:02d}-D{dt_index:02d}"
                lifecycle = preopen_episode_lifecycle(
                    run_id=run_id,
                    episode_id=episode_id,
                ...
                case_lifecycles.append(lifecycle)
                lifecycle_records.append(lifecycle)
                open_lifecycles[episode_id] = lifecycle
            ...
            for dt_index, run in enumerate(result["runs"], 1):
                ...
                _, canonical_episode = write_episode(
                    ...
                    torque_df,
                    lifecycle=case_lifecycles[dt_index - 1],
                )
                published_packages.append(canonical_episode)
                open_lifecycles.pop(episode_id, None)
            print(
        ...
        s03_results = []
        s03_lifecycles = []
        for dt_index, planned_dt in enumerate(s03_cfg["dt_grid_s"], 1):
            ...
            lifecycle = preopen_episode_lifecycle(
                ...
                planned_config={
                    ...
                    "dt_s": float(planned_dt),
                    "seed": scenarios["seed"],
                    "plant_physics_sha256": plant_ids["SERVICER_ARM_TSM_PREBIND"][
                        "plant_physics_sha256"
                    ],
            ...
            s03_lifecycles.append(lifecycle)
            lifecycle_records.append(lifecycle)
            open_lifecycles[episode_id] = lifecycle
        ...
        for dt_index, result in enumerate(s03_results, 1):
            ...
            _, canonical_episode = write_episode(
                ...
                s03_convergence,
                control_df,
                lifecycle=s03_lifecycles[dt_index - 1],
            )
            published_packages.append(canonical_episode)
            open_lifecycles.pop(episode_id, None)
        s03_summary = {
        ...
        expected_success_files = set(R5_SUCCESS_PACKAGE_FILES)
        canonical_packages_complete = len(published_packages) == 12 and all(
            {item.name for item in path.iterdir() if item.is_file()} == expected_success_files
            for path in published_packages
        )
        lifecycle_contracts_complete = len(lifecycle_records) == 12 and all(
            lifecycle["preopened_before_integration"]
            and lifecycle["terminal_status"] == "COMPLETE"
            and lifecycle["atomic_publish"]
            and Path(lifecycle["published_path"]).is_dir()
        ...
        exit_conditions = {
            ...
            "FAILURE_LIFECYCLE_PREOPEN_AND_ATOMIC_PUBLISH": lifecycle_contracts_complete,
        ...
        verdict = r5_gate_verdict(exit_conditions)
        gate_candidate = {
            ...
            "runner_success_package_status": (
                "PASS_EXACT_21_FILE_ATOMIC_SUCCESS_EXPORT"
                if canonical_packages_complete
                else "HOLD_PACKAGE_INCOMPLETE"
            ),
            "runner_failure_lifecycle_status": (
                "PASS_PREOPENED_BEFORE_INTEGRATION__FAILURE_PACKAGE_RETAINED__ATOMIC_SUCCESS_PUBLISH"
                if lifecycle_contracts_complete
                else "HOLD_LIFECYCLE_CONTRACT_INCOMPLETE"
            ),
        ...
        lifecycle_failure_errors = []
        for lifecycle in list(open_lifecycles.values()):
            try:
                fail_episode_lifecycle(lifecycle, exc, terminal_status="FAILED")
            except Exception as lifecycle_exc:  # retain the primary exception and record cleanup failure
                lifecycle_failure_errors.append(
                    {
                        "episode_id": lifecycle.get("episode_id"),
                        "failure_reason": str(lifecycle_exc),
                    }
                )
        if run_root.exists() and not (run_root / "RUN_COMPLETE.json").exists():
            write_json(
                run_root / "RUN_FAILED.json",
                {
                    ...
                    "partial_artifacts_preserved": True,
                    "episode_failure_packages_attempted": len(open_lifecycles),
                    "episode_failure_package_errors": lifecycle_failure_errors,
                },
            )
        raise
